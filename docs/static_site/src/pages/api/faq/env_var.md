...
* MXNET_RTC_VERBOSE
  ...
  - If set to '0', no diagnostic output of RTC use is produced
  - If set to '1', RTC kernel source is output, plus time of fusion graph-pass
  - If set to '2', Level '1' output, plus per-thread local memory use

* MXNET_RTC_SIZE_WARNING
  - 0(false) or 1(true) ```(default=1)```
  - If set to '0', MXNet produces no warning regarding the number of real-time-compilations needed
  - If set to '1', MXNet warns if the number of real-time-compiled kernels reaches 10000
...
* MXNET_CUDNN_ENABLE_WINOGRAD_NONFUSED_5X5
  - Values: 0(false), 1(true) ```(default=0)```
  - The non-fused Winograd cuDNN convolution algorithm is often fastest for 5x5 filters, but with less accuracy.  Users can opt-in to its use with this flag.
  - Value of '0' means that 2D convolutions with 5x5 filters are precluded from using the non-fused Winograd algorithm.
  - Value of '1' means the framework is free to use the non-fused Winograd if it is deemed fasted and meets workspace constraints.
