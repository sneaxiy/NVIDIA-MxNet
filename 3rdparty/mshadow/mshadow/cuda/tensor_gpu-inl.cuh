...
/*! \brief maximum size of grid in the X dimension (in blocks) */
const int kMaxGridDimX = std::numeric_limits<int32_t>::max();  // 2^31 - 1
/*! \brief maximum size of grid in the Y dimension (in blocks) */
const int kMaxGridDimY = 65535;
/*! \brief maximum size of grid in the Z dimension (in blocks) */
const int kMaxGridDimZ = 65535;
/*! \brief maximum number of blocks in a grid */
const int kMaxGridNum = kMaxGridDimX;  // Bigger actually, but safer for now to stay within the range of signed int
...
/*! \brief Create the kernel launch grid configuration, given the total number of blocks */
inline dim3 makeGrid(int64_t numBlocks) {
  return (numBlocks <= kMaxGridDimX) ? dim3{static_cast<uint32_t>(numBlocks), 1, 1}
                                     : dim3{kMaxGridDimX,
                                            static_cast<uint32_t>((numBlocks + kMaxGridDimX - 1) / kMaxGridDimX),
                                            1};
}

inline void CheckLaunchParam(dim3 dimGrid, dim3 dimBlock, const char *estr = "") {
  if (dimGrid.x > kMaxGridDimX ||
      dimGrid.y > kMaxGridDimY ||
      dimGrid.z > kMaxGridDimZ) {
    LOG(FATAL) << estr << " kernel grid too large for launch: attempted grid(dimX, dimY, dimZ) = ("
               << dimGrid.x << "," << dimGrid.y << "," << dimGrid.z << ") blocks, where max limits = ("
               << kMaxGridDimX << "," << kMaxGridDimY << "," << kMaxGridDimZ << ").";
  }

  int threads_per_block = dimBlock.x * dimBlock.y * dimBlock.z;
  if (threads_per_block > static_cast<unsigned>(kMaxThreadsPerBlock)) {
    LOG(FATAL) << estr << " kernel has too large a block size: attempted block(dimX, dimY, dimZ) = ("
               << dimBlock.x << "," << dimBlock.y << "," << dimBlock.z << ") with " << threads_per_block
               << "threads, where max limit = " << kMaxThreadsPerBlock;
  }
}
...
inline void MapReduceKeepDim1(expr::Plan<DstExp, DType> dst,
  ...
  dim3 dimGrid(makeGrid(pshape[1]));
...
inline void IndexFill(Tensor<gpu, 2, DType> dst,
  ...
  int64_t grid_dim_x = (src.size(0) * src.size(1) + block_size - 1) / block_size;
  int64_t grid_dim_y = 1;
  while (grid_dim_x > kMaxGridDimX) {
    grid_dim_x = (grid_dim_x + 1) / 2;
    grid_dim_y *= 2;
  }
  dim3 dimBlock(block_dim_x, block_dim_y);
  dim3 dimGrid(static_cast<int>(grid_dim_x), static_cast<int>(grid_dim_y));
