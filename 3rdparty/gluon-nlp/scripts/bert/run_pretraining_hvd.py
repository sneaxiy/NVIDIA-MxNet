...
import mxnet as mx
from mxnet import cuda_utils as cuda
import gluonnlp as nlp

from fp16_utils import FP16Trainer
from pretraining_utils import get_model_loss, get_pretrain_data_npz, get_dummy_dataloader
from pretraining_utils import split_and_load, log, log_noacc, evaluate, forward, get_argparser
...
parser.add_argument('--no_compute_acc', action='store_true',
                    help='If set to True, nsp and msm accuracy will not be computed and displayed '
                         '(loss values will still be computed and shown). This slightly improves '
                         'training speed.')
parser.add_argument('--synthetic_data', action='store_true',
                    help='If set to True, script will use synthetic data (useful for profiling and debugging.')
    ...
    while step_num < num_train_steps:
        ...
        while not end_of_batch:
            ...
            if args.no_compute_acc:
                # Synchronize before the next batch to reduce memory consumption
                mx.nd.waitall()
            ...
            (input_id, _, _, _, _, segment_id, valid_length) = data
            cuda.nvtx_range_push("Step with " +
                                 str(input_id.shape) + ", " +
                                 str(segment_id.shape) + ", " +
                                 str(valid_length.shape))
            # forward
            ...
            if (step_num + 1) % (args.log_interval) == 0 and (batch_num + 1) % accumulate == 0:
                if args.no_compute_acc:
                    log_noacc(begin_time, running_num_tks, running_mlm_loss / accumulate,
                        running_nsp_loss / accumulate, step_num, trainer, args.log_interval)
                else:
    ...
    if args.data:
        ...
        if not args.synthetic_data:
            ...
            data_batch = [(mx.nd.zeros((7, 512)),
                           ...
                           mx.nd.zeros((7, 512)),
                           mx.nd.ones((7,)) * 512)]
            data_train = DummyIter(data_batch)
