...
ReadyEventList FormReadyEventList(NDArray* input, NDArray* output) {
  ...
  // Get events from input tensor writers
  {
    ...
    if (!sync_obj.writer_event.empty()) {
      ready_event_list.AddReadyEvent(std::make_shared<MXReadyEvent>(sync_obj.writer_event[0].event));
    }
  }
  ...
  // Get events from output tensor reader and writers
  {
    ...
    for (auto& cuda_event : sync_obj.reader_events) {
      ready_event_list.AddReadyEvent(std::make_shared<MXReadyEvent>(cuda_event.event));
    }
    if (!sync_obj.writer_event.empty()) {
      ready_event_list.AddReadyEvent(std::make_shared<MXReadyEvent>(sync_obj.writer_event[0].event));
    }
...
#ifndef HAVE_CUDA
  // Only call on_start for non-GPU builds. Dependencies for GPU builds are handled
  // via ready_event_list to avoid synchronization.
  on_start();
#endif
