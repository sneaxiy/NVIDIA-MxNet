...
#define HOROVOD_ENABLE_NCCL_GROUPS "HOROVOD_ENABLE_NCCL_GROUPS"
...
#if HAVE_GPU
  void PushEventsToStream(gpuStream_t stream) {
    for (auto& e : ready_events_) {
      HVD_GPU_CHECK(gpuStreamWaitEvent(stream, e->event(), 0));
    }
  }
