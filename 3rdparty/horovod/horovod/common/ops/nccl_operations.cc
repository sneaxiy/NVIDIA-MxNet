...
Status NCCLAllreduce::Execute(std::vector<TensorTableEntry>& entries,
  ...
  if (!global_state_->enable_nccl_groups) {
    if (entries.size() > 1) {
    ...
    if (response.prescale_factor() != 1.0) {
      ScaleMemcpyToOutput(entries, response.prescale_factor());
      reduce_output = true; // output contains scaled input, use it in allreduce
    }
  }
  ...
  if (!global_state_->enable_nccl_groups) {
    ...
    for (const auto &e : entries) {
      auto nccl_result = ncclAllReduce(reduce_output ? e.output->data() : e.tensor->data(),
                                       (void*) e.output->data(),
                                       (size_t) e.tensor->size() / DataType_Size(e.tensor->dtype()),
  ...
  if (!global_state_->enable_nccl_groups) {
    ...
    if (response.postscale_factor() != 1.0) {
      ScaleOutput(entries, response.postscale_factor());
    }
  }
...
void NCCLHierarchicalAllreduce::WaitForData(std::vector<TensorTableEntry>& entries) {
  for (auto& e : entries) {
    e.ready_event_list.PushEventsToStream(*gpu_op_context_.stream);
  }
}
