...
#if HAVE_CUDA
void  GPUAllreduce::ScaleMemcpyToOutput(const std::vector<TensorTableEntry>& entries, double scale_factor) {
  auto& first_entry = entries[0];
  ...
  if (entries.size() > 1) {
    ...
    int64_t num_elements = first_entry.tensor->size() / DataType_Size(first_entry.tensor->dtype());
    ScaleBuffer(scale_factor, entries, first_entry.tensor->data(), (void*) first_entry.output->data(), num_elements);
  }
}
#endif
...
#if HAVE_CUDA
void  GPUAllreduce::ScaleOutput(const std::vector<TensorTableEntry>& entries, double scale_factor) {
  auto& first_entry = entries[0];
  ...
  if (entries.size() > 1) {
    ...
    for (auto& e : entries) {
      // Set input/output pointers and sizes
      d2d_params.out[idx % BATCHED_D2D_CAPACITY] = (void*) e.output->data();
      d2d_params.in[idx % BATCHED_D2D_CAPACITY] = (void*) e.output->data();
      d2d_params.sizes[idx % BATCHED_D2D_CAPACITY] = e.output->size();
      ...
      if (idx % BATCHED_D2D_CAPACITY == 0 || idx == (int) entries.size()) {
        // Perform batched d2d memcpy
        BatchedScaledD2DMemcpyCudaImpl(d2d_params, count, scale_factor, first_entry.output->dtype(),
                                       gpu_context_->streams[global_state_->current_nccl_stream][first_entry.device]);
        // TODO: https://github.com/horovod/horovod/issues/2230
        //gpu_context_->ErrorCheck("BatchedScaledD2DMemcpyCudaImpl", cudaGetLastError());
        count = 0;
    ...
    int64_t num_elements = first_entry.output->size() / DataType_Size(first_entry.output->dtype());
    ScaleBuffer(scale_factor, entries, (void*) first_entry.output->data(), (void*) first_entry.output->data(), num_elements);
  }
}
#endif
