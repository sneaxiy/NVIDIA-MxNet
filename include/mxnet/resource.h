...
struct Resource {
  ...
  inline mshadow::Tensor<xpu, ndim, DType> get_space_typed(
    ...
    // Interpret shape.Size() as positive (helps expand useful range when index_t == int32_t)
    using mshadow_shape_size_t = decltype(shape.Size());
    auto num_elems = static_cast<std::make_unsigned_t<mshadow_shape_size_t>>(shape.Size());
    return mshadow::Tensor<xpu, ndim, DType>(
        reinterpret_cast<DType*>(get_space_internal(num_elems * sizeof(DType))),
