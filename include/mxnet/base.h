...
struct Context {
  ...
  /*!
   * Get the GPU streaming multiprocessor (SM) architecture.
   * \param dev the GPU number to query
   * \return The GPU SM arch.
   */
  inline static int32_t GetGPUSMArch(int dev);
  /*!
   * Get the number of asynchronous copy engines for the GPU.
   * \param dev the GPU number to query
   * \return The GPU copy engine count.
   */
  inline static int32_t GetGPUAsyncEngineCount(int dev);
  /*!
...
inline int32_t Context::GetGPUSMArch(int dev) {
#if MXNET_USE_CUDA
  ...
  e = cudaDeviceGetAttribute(&compute_capability_major, cudaDevAttrComputeCapabilityMajor, dev);
  CHECK_EQ(e, cudaSuccess) << " CUDA: " << cudaGetErrorString(e);

  e = cudaDeviceGetAttribute(&compute_capability_minor, cudaDevAttrComputeCapabilityMinor, dev);
  CHECK_EQ(e, cudaSuccess) << " CUDA: " << cudaGetErrorString(e);

  e = cudaSetDevice(curDevice);
  CHECK_EQ(e, cudaSuccess) << " CUDA: " << cudaGetErrorString(e);

  int32_t sm_arch = 10 * compute_capability_major + compute_capability_minor;
  return sm_arch;
#else
  LOG(FATAL)
...
inline int32_t Context::GetGPUAsyncEngineCount(int dev) {
#if MXNET_USE_CUDA

  int32_t num_async_copy_engines;
  cudaError_t e;
  ...
  e = cudaDeviceGetAttribute(&num_async_copy_engines, cudaDevAttrAsyncEngineCount, dev);
  CHECK_EQ(e, cudaSuccess) << " CUDA: " << cudaGetErrorString(e);

  e = cudaSetDevice(curDevice);
  CHECK_EQ(e, cudaSuccess) << " CUDA: " << cudaGetErrorString(e);

  return num_async_copy_engines;
#else
  LOG(FATAL)
      << "This call is only supported for MXNet built with CUDA support.";
  return 0;
