...
/*!
 * \brief Get streaming multiprocessor (SM) architecture of a GPU.
 * \param dev the GPU number to query
 * \param pointer to int that will hold the SM arch.
 * \return 0 when success, -1 when failure happens.
 */
MXNET_DLL int MXGetGPUSMArch(int dev, int* out);

/*!
 * \brief Get the number of async copy engines of a GPU.
 * \param dev the GPU number to query
 * \param pointer to int that will hold the SM arch.
 * \return 0 when success, -1 when failure happens.
 */
MXNET_DLL int MXGetGPUAsyncEngineCount(int dev, int* out);
...
/*!
 * \brief Get the size of the NCCL unique ID.
 */
MXNET_DLL int MXNCCLGetUniqueIdSize(int* size);

/*!
 * \brief Get the NCCL unique ID into the
 * preallocated buffer out. The buffer needs
 * to be able to hold at least the number of
 * bytes obtained by the call to
 * MXNCCLGetUniqueIdSize.
 */
