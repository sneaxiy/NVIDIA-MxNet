...
Graph GraphExecutor::InitGraph(nnvm::Symbol symbol,
  ...
  if (!alm_params.empty()) {
    nnvm::Graph g;
    g.outputs = symbol.outputs;
    g = alm::OptimizeLayout(std::move(g), alm_params.getTargets());
    symbol = nnvm::Symbol();
  ...
  if (use_fusion && default_ctx.dev_mask() == Context::kGPU && simple_bind) {
    ...
    if (!dmlc::GetEnv("MXNET_DISABLE_BNADDRELU_FUSION", false)) {
      g = infer_attr(std::move(g));
      g = FuseBNAmpAddRelu(std::move(g), false);
      g = infer_attr(std::move(g));
      g = FuseBNAddRelu(std::move(g), false);
    }
  ...
  if (use_fusion && default_ctx.dev_mask() == Context::kGPU) {
    ...
    if (common::CheckForInputNameDuplicates(unoptimized_graph.indexed_graph())) {
      if (dmlc::GetEnv("MXNET_DEBUG_PRINT_GRAPH", 0)) {
        ...
        PrintFullGraph(g, "before");
      }
      g = FusePointwise(std::move(g), num_forward_outputs_);
      if (dmlc::GetEnv("MXNET_DEBUG_PRINT_GRAPH", 0)) {
        std::cout << "After the fusion" << std::endl;
        PrintFullGraph(g, "after");
      }
