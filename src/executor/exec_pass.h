...
inline void PrintFullGraph(const nnvm::Graph& g,
  ...
  const std::string& filename = dmlc::GetEnv("MXNET_DEBUG_PRINT_GRAPH_PATH", std::string());
  std::ofstream file_stream;
  std::ostream& stream = (filename == "") ? std::cout : file_stream;
  if (filename != "") {
    file_stream.open(filename + suffix + ".dot");
  }

  stream << "digraph Net {" << std::endl;
  for (size_t i = 0; i < index.num_nodes(); ++i) {
    ...
    if (source != nullptr) {
      ...
      for (size_t j = 0; j < node.inputs.size(); ++j) {
        stream << "    node_" << node.inputs[j].node_id << " -> node_" << i << std::endl;
      }
    } else {
      stream << " [label=\"NODE " << i << "\", shape=ellipse, style=dashed]" << std::endl;
    }
    stream << std::endl;
  }
  stream << "}" << std::endl;
...
/*!
 * \brief Fuse BatchNorm followed by add, amp and relu
 * \param g input graph (needs to be forward graph)
 *
 * \return graph with fused BatchNorm and activation operations
 */
...
bool IsFuseableReLU(const nnvm::ObjectPtr& node);
