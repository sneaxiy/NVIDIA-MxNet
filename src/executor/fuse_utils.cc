...
nnvm::NodeEntryMap<uint32_t> GetNodeEntryCount(const nnvm::Graph& g) {
  ...
  DFSVisit(g.outputs, [&outputs](const nnvm::ObjectPtr& node) {
...
Graph ReplaceNodeEntries(nnvm::Graph&& g,
  ...
  DFSVisit(g.outputs, [&entry_map](const nnvm::ObjectPtr& n) {
...
bool IsFuseableReLU(const nnvm::ObjectPtr& node) {
  if (node->op() == Op::Get("Activation")) {
    const auto& param = nnvm::get<op::ActivationParam>(node->attrs.parsed);
    return param.act_type == op::activation::kReLU;
  }
