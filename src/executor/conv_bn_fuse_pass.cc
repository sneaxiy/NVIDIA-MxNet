...
namespace {
  ...
  void PrintGraph(const Graph& g) {
    ...
    DFSVisit(g.outputs, [&ne_counter](const nnvm::ObjectPtr& n) {
      if (n->op() == nullptr) {
  ...
  bool IsCompatibleBN(const nnvm::ObjectPtr& node, const TShape& shape, const Context& ctx) {
  ...
  void ConvToNormConv(const nnvm::ObjectPtr& node) {
  ...
  void NormConvToConv(const nnvm::ObjectPtr& node) {
  ...
  void FuseBatchNorm(const nnvm::ObjectPtr prev_conv, const nnvm::ObjectPtr bn,
                     const nnvm::ObjectPtr next_conv,
...
Graph FuseConvBN(Graph&& g) {
  ...
  DFSVisit(g.outputs, [&idx, &shape_vec, &dtype_vec, &context_vec](const nnvm::ObjectPtr node) {
  ...
  nnvm::NodeEntryMap<nnvm::NodeEntry> entry_map;
  DFSVisit(g.outputs, [&idx, &shape_vec, &ne_counter,
          &entry_map, &context_vec](const nnvm::ObjectPtr& next) {
    if (next->op() == Op::Get("NormConvolution")) {
      auto node = next->inputs[0].node;
      auto eid = idx.entry_id(idx.node_id(node.get()), 0);
      auto nid = idx.node_id(node.get());
      if (IsCompatibleBN(node, shape_vec[eid], context_vec[nid]) &&
