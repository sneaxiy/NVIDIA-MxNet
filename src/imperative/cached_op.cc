...
struct SetBulkSizeInScope {
  explicit SetBulkSizeInScope(uint32_t size) : prev_size_(Engine::Get()->set_bulk_size(size)) {}
  ~SetBulkSizeInScope() { Engine::Get()->set_bulk_size(prev_size_); }
  uint32_t prev_size_;
};

void CalcInputMap(const nnvm::Graph& original, const nnvm::Graph& current,
                  std::vector<size_t>* input_map, std::vector<size_t>* inverted_input_map) {
  const auto& original_inputs = original.indexed_graph().input_nodes();
  const auto& inputs = current.indexed_graph().input_nodes();
  std::unordered_map<std::string, size_t> name_to_oi;
  CHECK_EQ(inputs.size(), original_inputs.size());
  for (size_t i = 0; i < original_inputs.size(); ++i) {
    auto r = name_to_oi.insert(
        std::make_pair(original.indexed_graph()[original_inputs[i]].source->attrs.name, i));
    CHECK(r.second);
  }
  input_map->resize(inputs.size());
  for (size_t i = 0; i < inputs.size(); ++i) {
    auto it = name_to_oi.find(current.indexed_graph()[inputs[i]].source->attrs.name);
    CHECK(it != name_to_oi.end());
    (*input_map)[i] = it->second;
  }
  *inverted_input_map = InvertBijection(*input_map);
}

bool CachedOp::InferAttrs(FwdGraphInfo* fwd, const std::vector<NDArray*>& inputs,
                          bool* has_dynamic_shapes) {
  fwd->fwd_graph.attrs["context"] = std::make_shared<dmlc::any>(
      std::vector<Context>(fwd->fwd_graph.indexed_graph().num_nodes(), inputs[0]->ctx()));

  ShapeVector shape_inputs(inputs.size());
  nnvm::DTypeVector dtype_inputs(inputs.size());
  StorageTypeVector storage_type_inputs(inputs.size());
  for (size_t i = 0; i < inputs.size(); ++i) {
    shape_inputs[i] = inputs[fwd->input_map[i]]->shape();
    dtype_inputs[i] = inputs[fwd->input_map[i]]->dtype();
    storage_type_inputs[i] = inputs[fwd->input_map[i]]->storage_type();
  }

  bool match = true;
  match &= imperative::CheckAndInferShape(&fwd->fwd_graph, std::move(shape_inputs), true, {0, 0},
                                          {0, 0}, has_dynamic_shapes);
  match &= imperative::CheckAndInferType(&fwd->fwd_graph, std::move(dtype_inputs), true);
  exec::DevMaskVector dev_mask(fwd->fwd_graph.indexed_graph().num_nodes(),
                               inputs[0]->ctx().dev_mask());
  match &= imperative::CheckAndInferStorageType(&fwd->fwd_graph, std::move(dev_mask),
                                                std::move(storage_type_inputs), true);
  return match;
}

void CachedOp::CreateGraph(CachedOpState* state, const std::vector<NDArray*>& inputs) {
  auto set_ctx = [&inputs](nnvm::Graph& g) {
    g.attrs["context"] = std::make_shared<dmlc::any>(
        std::vector<Context>(g.indexed_graph().num_nodes(), inputs[0]->ctx()));
  };

  state->fwd = std::make_unique<FwdGraphInfo>();
  CreateFwdGraph(fwd_graph_.outputs, &state->fwd->fwd_graph);
  state->fwd->input_map.resize(fwd_graph_.indexed_graph().input_nodes().size());
  std::iota(state->fwd->input_map.begin(), state->fwd->input_map.end(), 0);
  state->fwd->inverted_input_map = state->fwd->input_map;
  set_ctx(state->fwd->fwd_graph);

  const bool fusion_supported = IsFusionSupported(inputs[0]->ctx());
  if (config_.static_shape && fusion_supported) {
    RunFwdFusions(state->fwd.get(), inputs);
    CalcInputMap(fwd_graph_, state->fwd->fwd_graph, &state->fwd->input_map,
                 &state->fwd->inverted_input_map);
  }

  state->bwd = std::make_unique<BwdGraphInfo>();
  CreateBwdGraph(state->fwd->fwd_graph, state->fwd->input_map, &state->bwd->grad_graph,
                 &state->bwd->full_graph, &state->bwd->ograd_entries,
                 &state->bwd->fwd_input_to_grad_output, &state->bwd->grad_out_map);

  if (fusion_supported) {
    if (dmlc::GetEnv("MXNET_DEBUG_PRINT_GRAPH", 0)) {
      std::cout << "Before full fusions:\n";
      exec::PrintFullGraph(state->bwd->full_graph, "before_full");
    }
    const size_t num_f_outputs = state->fwd->fwd_graph.indexed_graph().outputs().size();
    state->bwd->full_graph = exec::FusePointwise(std::move(state->bwd->full_graph), num_f_outputs);
    if (dmlc::GetEnv("MXNET_DEBUG_PRINT_GRAPH", 0)) {
      std::cout << "After full fusions:\n";
      exec::PrintFullGraph(state->bwd->full_graph, "after_full");
    }

    state->fwd->fwd_graph = nnvm::Graph();
    state->fwd->fwd_graph.outputs.assign(state->bwd->full_graph.outputs.begin(),
                                         state->bwd->full_graph.outputs.begin() + num_f_outputs);
    state->bwd->grad_graph = nnvm::Graph();
    state->bwd->grad_graph.outputs.assign(state->bwd->full_graph.outputs.begin() + num_f_outputs,
                                          state->bwd->full_graph.outputs.end());
  ...
  SetRefCounts(&state->fwd->fwd_graph, state->bwd->full_graph);

  size_t max_nodes = state->bwd->full_graph.indexed_graph().num_nodes();
  size_t max_entries = state->bwd->full_graph.indexed_graph().num_node_entries();
  set_ctx(state->bwd->full_graph);

  state->buff.resize(max_entries);
  state->arrays.resize(max_entries);
  state->array_reqs.resize(max_entries);
  state->dynamic_entries.resize(max_entries, false);
  state->op_states.resize(max_nodes);
  state->execs.resize(max_nodes);
  state->opr_segs.resize(max_nodes);
}

void CachedOp::RunFwdFusions(FwdGraphInfo* fwd, const std::vector<NDArray*>& inputs) {
  if (dmlc::GetEnv("MXNET_DEBUG_PRINT_GRAPH", 0)) {
    std::cout << "Before forward fusions:\n";
    exec::PrintFullGraph(fwd->fwd_graph, "before_forward");
  }

  bool has_dynamic_shapes = false;
  InferAttrs(fwd, inputs, &has_dynamic_shapes);
  if (!has_dynamic_shapes) {
    fwd->fwd_graph = exec::FuseBNAmpAddRelu(std::move(fwd->fwd_graph), true);
    CalcInputMap(fwd_graph_, fwd->fwd_graph, &fwd->input_map, &fwd->inverted_input_map);

    InferAttrs(fwd, inputs);
    fwd->fwd_graph = exec::FuseBNAddRelu(std::move(fwd->fwd_graph), true);
    CalcInputMap(fwd_graph_, fwd->fwd_graph, &fwd->input_map, &fwd->inverted_input_map);

    InferAttrs(fwd, inputs);
    fwd->fwd_graph = exec::FuseBNActiv(std::move(fwd->fwd_graph));
    CalcInputMap(fwd_graph_, fwd->fwd_graph, &fwd->input_map, &fwd->inverted_input_map);
  } else {
    LOG(WARNING) << "Forward fusions disabled because of dynamic shape ops in a graph";
  }
  if (dmlc::GetEnv("MXNET_DEBUG_PRINT_GRAPH", 0)) {
    std::cout << "After forward fusions:\n";
    exec::PrintFullGraph(fwd->fwd_graph, "after_forward");
  }
}

#if MXNET_USE_CUDA && !defined(_WIN32)
bool CachedOp::IsFusionSupported(const Context& ctx) {
  if (ctx.dev_mask() != kGPU || inlining_ || !dmlc::GetEnv("MXNET_USE_FUSION", true)) return false;
  if (has_duplicate_input_names_) {
    LOG(WARNING)
...
#else
bool CachedOp::IsFusionSupported(const Context&) {
...
CachedOp::CachedOp(
  ...
  CreateFwdGraph(sym.outputs, &fwd_graph_);
  auto grad_graph = nnvm::Graph();
  CreateBwdGraph(fwd_graph_, &grad_graph, &full_graph_, &ograd_entries_);
  ...
  // Set the backward dependency vectors
  {
    ...
    has_duplicate_input_names_ = !common::CheckForInputNameDuplicates(idx);
  }
...
bool CachedOp::CheckDynamicShapeExists(const Context& default_ctx,
  ...
  nnvm::Graph& g = state.fwd->fwd_graph;
  ShapeVector shape_inputs(inputs.size());
  for (size_t i = 0; i < inputs.size(); ++i) {
    shape_inputs[i] = inputs[state.fwd->input_map[i]]->shape();
  }
...
bool CachedOp::SetForwardGraph(FwdGraphInfo* fwd, const bool recording,
                               const std::vector<NDArray*>& inputs) {
  using namespace nnvm;
  using namespace imperative;
  CHECK_EQ(inputs.size(), num_inputs());
  nnvm::Graph& g = fwd->fwd_graph;
...
bool CachedOp::SetBackwardGraph(const FwdGraphInfo& fwd, BwdGraphInfo* bwd,
                                const std::vector<OpReqType>& reqs,
                                const std::vector<NDArray*>& inputs, bool detect_inplace_addto) {
  using namespace nnvm;
  using namespace imperative;
  std::lock_guard<std::mutex> lock(mutex_);
  Context default_ctx = inputs[0]->ctx();
  nnvm::Graph& g = bwd->full_graph;

  if (bwd->bwd_output_reqs != reqs) {
    bwd->bwd_output_reqs = reqs;
    bwd->bwd_input_eid.clear();
    g = nnvm::Graph();
    g.outputs = fwd.fwd_graph.outputs;
    for (size_t i = 0; i < bwd->grad_graph.outputs.size(); ++i) {
      if (bwd->bwd_output_reqs[bwd->grad_out_map[i]] == kNullOp) continue;
      g.outputs.emplace_back(bwd->grad_graph.outputs[i]);
    }
    g.attrs["context"] = std::make_shared<dmlc::any>(
  ...
  if (bwd->bwd_input_eid.size() != inputs.size()) {
    bwd->bwd_input_eid.clear();
    SetBackwardInputEid(bwd_in_dep_, bwd_out_dep_, bwd_ograd_dep_,
                        bwd->ograd_entries, idx, &bwd->bwd_input_eid);
    CHECK_EQ(inputs.size(), bwd->bwd_input_eid.size());
  }

  size_t num_forward_nodes = fwd.fwd_graph.indexed_graph().num_nodes();
  size_t num_forward_entries = fwd.fwd_graph.indexed_graph().num_node_entries();
  ...
  if (!g.attrs.count(AddPrefix(BACKWARD, REF_COUNT))) {
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
      if (bwd->bwd_input_eid[i] != kEidNotExist) {
        ++ref_count[bwd->bwd_input_eid[i]];
      }
    }
  ...
  auto shapes = fwd.fwd_graph.GetAttr<mxnet::ShapeVector>("shape");
  shapes.resize(idx.num_node_entries(), mxnet::TShape());
  auto dtypes = fwd.fwd_graph.GetAttr<DTypeVector>("dtype");
  dtypes.resize(idx.num_node_entries(), -1);
  auto stypes = fwd.fwd_graph.GetAttr<StorageTypeVector>("storage_type");
  stypes.resize(idx.num_node_entries(), -1);

  for (size_t i = 0; i < inputs.size(); ++i) {
    if (bwd->bwd_input_eid[i] == kEidNotExist) continue;
    size_t oi = BwdOriginalInput(fwd.input_map, i);
    shapes[bwd->bwd_input_eid[i]] = inputs[oi]->shape();
    dtypes[bwd->bwd_input_eid[i]] = inputs[oi]->dtype();
    stypes[bwd->bwd_input_eid[i]] = inputs[oi]->storage_type();
  }
...
OpStatePtr CachedOp::GetCachedOpState(const Context& ctx) {
  ...
  auto state_ptr = OpStatePtr::Create<CachedOpState>(ctx);
...
void CachedOp::StaticAllocMemory(
  ...
  nnvm::Graph& g = keep_fwd ? state.bwd->full_graph : state.fwd->fwd_graph;
  const auto& idx = g.indexed_graph();
  ...
  size_t start_eid = keep_fwd ? state.fwd->fwd_graph.indexed_graph().num_node_entries() : 0;
...
void CachedOp::StaticInitExec(
  ...
  size_t start_nid = keep_fwd ? state.fwd->fwd_graph.indexed_graph().num_nodes() : 0;
...
OpStatePtr CachedOp::StaticForward(
  ...
  bool match = SetForwardGraph(state.fwd.get(), recording, inputs);
  ...
  if (config_.static_shape) {
    for (auto i : config_.param_indices) {
      ...
      if (!arrays[idx.entry_id(nid, 0)]->IsSame(*inputs[state.fwd->input_map[i]])) {
        match = false;
        auto ptr = &state.buff[idx.entry_id(nid, 0)];
        CHECK_EQ(arrays[idx.entry_id(nid, 0)], ptr);
        *arrays[idx.entry_id(nid, 0)] = *inputs[state.fwd->input_map[i]];
        state.dynamic_entries[idx.entry_id(nid, 0)] = false;
      }
    }
    for (auto i : config_.data_indices) {
      auto eid = idx.entry_id(idx.input_nodes()[i], 0);
      arrays[eid] = inputs[state.fwd->input_map[i]];
    }
  } else {
...
OpStatePtr CachedOp::DynamicForward(
  ...
  auto& runtime = op_state.get_state<DynamicRuntime>();
  {
    ...
    auto& state = state_ptr.get_state<CachedOpState>();
    std::lock_guard<std::mutex> lock(state.mutex);
    SetForwardGraph(state.fwd.get(), recording, inputs);
    runtime.fwd.fwd_graph = state.fwd->fwd_graph;
    runtime.fwd.input_map = state.fwd->input_map;
  }
  nnvm::Graph& g = runtime.fwd.fwd_graph;
  const auto& idx = g.indexed_graph();
  ...
  CollectInputOutputNDRefs(g, inputs, runtime.fwd.input_map, outputs, &arrays);

  if (!use_naive_run) {
    const auto& mem_plan = g.GetAttr<MemoryPlanVector >(AddPrefix(graph_type, MEM_PLAN));
    CreateGraphNDs(g, default_ctx, mem_plan, outputs, &array_reqs, &arrays);
                  ...
                  dispatch_modes, recording && inlining_, &shapes, monitor_callback_, monitor_all_);
    {
      ...
      std::lock_guard<std::mutex> lock(state.mutex);
      state.fwd->fwd_graph.attrs["shape"] = std::make_shared<dmlc::any>(std::move(copied_shape));
    }
...
OpStatePtr CachedOp::Forward(
  ...
  Context default_ctx = inputs[0]->ctx();
  {
    ...
    if (config_.static_shape && state.fwd && !InferAttrs(state.fwd.get(), inputs)) {
      state.fwd.reset();
      state.bwd.reset();
      LOG(INFO) << "Compute graph will be rebuilt because of the changed attributes.";
    }

    if (!state.fwd) CreateGraph(&state, inputs);

    const auto& idx = state.fwd->fwd_graph.indexed_graph();
    for (size_t i = 0; i < inputs.size(); ++i) {
...
void CachedOp::DynamicBackward(
  ...
  auto& runtime = op_state.get_state<DynamicRuntime>();
  std::vector<size_t> grad_out_map;
  {
    auto state_ptr = GetCachedOpState(default_ctx);
    auto& state = state_ptr.get_state<CachedOpState>();
    std::lock_guard<std::mutex> lock(state.mutex);
    CHECK(state.bwd);
    state.fwd->fwd_graph = runtime.fwd.fwd_graph;
    state.fwd->input_map = runtime.fwd.input_map;
    SetBackwardGraph(*state.fwd, state.bwd.get(), reqs, inputs);
    runtime.bwd.full_graph = state.bwd->full_graph;
    runtime.bwd.bwd_input_eid = state.bwd->bwd_input_eid;
    grad_out_map = state.bwd->grad_out_map;
  }
  nnvm::Graph& g = runtime.bwd.full_graph;
  const auto& idx = g.indexed_graph();
  auto& buff = runtime.buff;
  auto& states = runtime.op_states;

  size_t num_forward_outputs = runtime.fwd.fwd_graph.outputs.size();
  size_t num_forward_nodes = runtime.fwd.fwd_graph.indexed_graph().num_nodes();
  size_t num_forward_entries = runtime.fwd.fwd_graph.indexed_graph().num_node_entries();
  buff.resize(idx.num_node_entries());
  std::vector<NDArray*> arrays;
  arrays.reserve(buff.size());
  for (auto& buffered_array : buff) {
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    if (runtime.bwd.bwd_input_eid[i] == kEidNotExist) continue;
    arrays[runtime.bwd.bwd_input_eid[i]] = inputs[BwdOriginalInput(runtime.fwd.input_map, i)];
  }
  for (size_t i = 0, j = num_forward_outputs; i < reqs.size(); ++i) {
    if (reqs[grad_out_map[i]] == kNullOp) continue;
    const auto eid = idx.entry_id(idx.outputs()[j++]);
    // An input and an output may share the same array.
    INIT_DETACHED(outputs[grad_out_map[i]], arrays[eid]);
    arrays[eid] = outputs[grad_out_map[i]];
  }
  ...
  for (size_t i = 0, j = num_forward_outputs; i < reqs.size(); ++i) {
    if (reqs[grad_out_map[i]] == kNullOp) continue;
    array_reqs[idx.entry_id(idx.outputs()[j++])] = reqs[grad_out_map[i]];
  }
...
void CachedOp::StaticBackward(
  ...
  bool match = SetBackwardGraph(*state.fwd, state.bwd.get(), reqs, inputs, true);

  nnvm::Graph& g = state.bwd->full_graph;
  const auto& idx = g.indexed_graph();
  auto num_forward_nodes = state.fwd->fwd_graph.indexed_graph().num_nodes();
  ...
  state.arrays_with_in_out = state.arrays;
  auto& arrays = state.arrays_with_in_out;
  for (size_t i = 0; i < state.bwd->bwd_input_eid.size(); ++i) {
    auto eid = state.bwd->bwd_input_eid[i];
    if (eid == kEidNotExist || !state.dynamic_entries[eid]) continue;
    arrays[eid] = inputs[BwdOriginalInput(state.fwd->input_map, i)];
  }

  if (config_.static_shape) {
    for (auto i : config_.param_indices) {
      const auto iter = state.bwd->fwd_input_to_grad_output.find(state.fwd->inverted_input_map[i]);
      if (iter == state.bwd->fwd_input_to_grad_output.end()) continue;
      auto entry = state.bwd->grad_graph.outputs[iter->second];
      if (!idx.exist(entry.node.get())) continue;
      auto eid = idx.entry_id(entry);
      if ((!arrays[eid]->IsSame(*outputs[state.bwd->grad_out_map[iter->second]]) &&
            state.array_reqs[eid] != kNullOp) ||
          !(state.array_reqs[eid] == reqs[state.bwd->grad_out_map[iter->second]])) {
        match = false;
        state.array_reqs[eid] = reqs[state.bwd->grad_out_map[iter->second]];
        // An input and an output may share the same array.
        INIT_DETACHED(outputs[state.bwd->grad_out_map[iter->second]], arrays[eid]);
        *arrays[eid] = *outputs[state.bwd->grad_out_map[iter->second]];
    ...
    for (size_t i = 0; i < state.bwd->grad_graph.outputs.size(); ++i) {
      auto entry = state.bwd->grad_graph.outputs[i];
      if (!idx.exist(entry.node.get())) continue;
      auto eid = idx.entry_id(entry);
      state.array_reqs[eid] = reqs[i];
      // An input and an output may share the same array.
      INIT_DETACHED(outputs[state.bwd->grad_out_map[i]], arrays[eid]);
      arrays[eid] = outputs[state.bwd->grad_out_map[i]];
    }
  }
...
void CachedOp::Backward(
  ...
  SetBulkSizeInScope sbs(config_.backward_bulk_size);
