...
OpStatePtr CachedOpThreadSafe::GetCachedOpState(
  ...
  // Upstream's approach- not consistent with current CachedOpState constructor
...
OpStatePtr CachedOpThreadSafe::DynamicForward(const Context& default_ctx,
  ...
  auto &runtime = op_state.get_state<DynamicRuntime>();
  {
    ...
    SetForwardGraph(state.fwd.get(), false, inputs);
  ...
  size_t max_nodes = runtime.fwd.fwd_graph.indexed_graph().num_nodes();
