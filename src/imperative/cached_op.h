...
#include "../executor/alm.h"

/*
 * The merge of PR https://github.com/apache/incubator-mxnet/pull/16654
 * is not quite complete.  A high-level synopsis of the PR is that is
 * modified the CachedOp class so that it could be a base class to a
 * new CachedOpThreadSafe class used for inference.  Many routines
 * formerly in cached_op.cc were moved to cached_op.h, and the
 * CreateFullGraph() method now calls new routines CreateForwardGraph() and
 * CreateBackwardGraph().  The new CachedOpThreadSafe class calls
 * CreateForwardGraph().
 *
 * To complete merge:
 *
 * - get CachedOpThreadSafe() to call NVIDIA's slightly different routine CreateFwdGraph()
 * - resolve differences in GraphInfo and remove commented-out code
 * - get ./tests/cpp/mxnet_unit_tests to pass the ThreadSafety tests, which currently fail
 *
 * A more detailed description of some of the PR's refactorings:
 *
 *  Moved from cached_op.cc to cached_op.h:
 *
  ...
  void SetRefCounts(nnvm::Graph* fwd_graph,
                    const nnvm::Graph& full_graph) { }
 ...
 *
  ...
  CachedOp set up now as a potential base class, with virtual member functions
    Gradient(), Forward(), Backward() and BackwardStorageType().

  
  Added CachedOp subclass CachedOpThreadSafe in cached_op_threadsafe.{h,cc}
...
static void PrepareOutputs(const nnvm::Graph& g, const Context& default_ctx,
  ...
  for (size_t i = 0; i < outputs.size(); ++i) {
    ...
    const auto& attrs = idx[idx.outputs()[i].node_id].source->attrs;
...
void CollectInputOutputNDRefsNoInputMap(const nnvm::Graph& g,
                              const std::vector<NDArray*>& inputs,
                              const std::vector<NDArray*>& outputs,
                              std::vector<NDArray*>* arrays) DMLC_ATTRIBUTE_UNUSED;
void CollectInputOutputNDRefsNoInputMap(const nnvm::Graph& g,
...
void CreateGraphNDs(const nnvm::Graph& g,
  ...
  PrepareOutputs(g, default_ctx, outputs, arrays, false);
...
void CreateFwdGraph(const std::vector<nnvm::NodeEntry>& outputs, nnvm::Graph* fwd_graph) {
  nnvm::Graph original;
  original.outputs = outputs;
  nnvm::Graph init;
  common::CopyGraph(&init, original, true);
  auto& alm_params = common::ALMParams::getALMParams();
  if (!alm_params.empty()) init = alm::OptimizeLayout(std::move(init), alm_params.getTargets());

  static const auto copy_op = nnvm::Op::Get("_copy");
  nnvm::NodeEntryMap<size_t> dedup_out;
  for (const auto& nodeEntry : init.outputs) {
    if (dedup_out.find(nodeEntry) != dedup_out.end()) {
      nnvm::ObjectPtr copy_node = nnvm::Node::Create();
      ...
      if (copy_op->attr_parser != nullptr) copy_op->attr_parser(&(copy_node->attrs));
  ...
  if (dmlc::GetEnv("MXNET_ELIMINATE_COMMON_EXPR", true))
...
std::vector<size_t> InvertBijection(const std::vector<size_t>& m) {
  std::vector<size_t> ret(m.size());
  for (size_t i = 0; i < m.size(); ++i) ret[m[i]] = i;
  return ret;
}

std::vector<size_t> GradOutMap(
    const std::vector<size_t>& input_map,
    const std::unordered_map<uint32_t, uint32_t>& fwd_input_to_grad_output) {
  std::map<size_t, size_t> original_order;
  for (auto p : fwd_input_to_grad_output)
    original_order.insert(std::make_pair(input_map[p.first], p.second));

  std::vector<size_t> m;
  for (auto p : original_order) m.push_back(p.second);
  return InvertBijection(m);
}

void CreateBwdGraph(const nnvm::Graph& fwd_graph, const std::vector<size_t>& input_map,
                    nnvm::Graph* grad_graph, nnvm::Graph* full_graph,
                    std::vector<nnvm::NodeEntry>* ograd_entries,
                    std::unordered_map<uint32_t, uint32_t>* fwd_input_to_grad_output,
  ...
  for (size_t i = 0; i < indexed_graph.input_nodes().size(); ++i) {
    ...
    if (indexed_graph.mutable_input_nodes().count(node_id)) continue;
    if (fwd_input_to_grad_output) (*fwd_input_to_grad_output)[i] = xs.size();
    xs.emplace_back(indexed_graph[node_id].weak_ref.lock());
  }
  // Input reordering causes grad out reordering. Create the grad out mapping, similar to input_map.
  if (grad_out_map) *grad_out_map = GradOutMap(input_map, *fwd_input_to_grad_output);

  CHECK(!xs.empty()) << "There are no inputs in computation graph that require gradients.";

  static const std::vector<const nnvm::Op*> zero_ops{nnvm::Op::Get("zeros_like"),
                                                     nnvm::Op::Get("_zeros")};
  *grad_graph =
      nnvm::pass::MXGradient(fwd_graph, fwd_graph.outputs, xs, *ograd_entries,
                             exec::AggregateGradient, nullptr, nullptr, zero_ops, "_copy");
...
void CreateBwdGraph(const nnvm::Graph& fwd_graph, nnvm::Graph* grad_graph, nnvm::Graph* full_graph,
                    std::vector<nnvm::NodeEntry>* ograd_entries) {
  std::vector<size_t> input_map(fwd_graph.indexed_graph().input_nodes().size());
  std::iota(input_map.begin(), input_map.end(), 0);
  CreateBwdGraph(fwd_graph, input_map, grad_graph, full_graph, ograd_entries, nullptr, nullptr);
}

/* \brief construct grad_graph from fwd_graph and ograd_entries*/

/*  Unmerged code from PR https://github.com/apache/incubator-mxnet/pull/16654
...
class CachedOp {
 ...
 protected:
  ...
  struct FwdGraphInfo {
    ...
    std::vector<size_t> inverted_input_map;  // the new index of an original input
  };
  ...
  struct BwdGraphInfo {
    ...
    std::vector<nnvm::NodeEntry> ograd_entries;
    std::unordered_map<uint32_t, uint32_t> fwd_input_to_grad_output;
    std::vector<size_t> grad_out_map;  // the original index of a grad output
    std::vector<OpReqType> bwd_output_reqs;
    std::vector<uint32_t> bwd_input_eid;
  };

  struct CachedOpState {
    explicit CachedOpState(const Context& ctx) : context(ctx) {}

    std::mutex mutex;
    Context context;

    // TODO(vcherepanov): change to std::optional once c++17 is required?
    std::unique_ptr<FwdGraphInfo> fwd;
    std::unique_ptr<BwdGraphInfo> bwd;
  ...
  */
  ...
  static bool InferAttrs(FwdGraphInfo* fwd, const std::vector<NDArray*>& inputs,
                         bool* has_dynamic_shapes = nullptr);

  void CreateGraph(CachedOpState* state, const std::vector<NDArray*>& inputs);
  bool IsFusionSupported(const Context& ctx);
  void RunFwdFusions(FwdGraphInfo* fwd, const std::vector<NDArray*>& inputs);
 ...
 private:
  ...
  bool dynamic_shape_checked_ = false;
  bool has_duplicate_input_names_ = false;
