...
/*!
 * \brief Return the kernel stack limit the given GPU device
 * \param device_id The device index of the cuda-capable gpu of interest
 *         (default is the calling thread's current device)
 * \return the device's per-thread stack limit in bytes
 */
inline size_t GetDeviceStackLimit(int device_id = -1) {
  DeviceStore(device_id, true);
  size_t stackLimit;
...
/*!
 * \brief Get the per-thread local memory use of a compiled-in CUDA kernel
 * \param f the Kernel
 * \return the per-thread local memory use of the kernel in bytes
 */
template <class T>
inline int GetLocalMemUse(T* f) {
    cudaFuncAttributes func_attributes;

    CUDA_CALL(cudaFuncGetAttributes(&func_attributes, f));
    return func_attributes.localSizeBytes;
}

/*!
 * \brief Get the per-thread local memory use of a CUfunction
 * \param f the CUfunction
 * \return the per-thread local memory use of the CUfunction in bytes
 */
template <>
inline int GetLocalMemUse<std::remove_pointer<CUfunction>::type>(CUfunction f) {
    int local_size_bytes = -1;
    CUDA_DRIVER_CALL(cuFuncGetAttribute(&local_size_bytes,
...
/*!
 * \brief Get Size of L2 cache in bytes. 0 if the device doesn't have L2 cache
 * \param device_id The device index of the cuda-capable gpu of interest.
 * \return size of L2 cache in bytes. 0 if the device doesn't have L2 cache
 */
inline int L2CacheSize(int device_id) {
  static std::vector<int32_t> l2_cache_size(kMaxNumGpus, -1);
  return cudaAttributeLookup(device_id, &l2_cache_size,
                             cudaDevAttrL2CacheSize, "L2CacheSize");
}
...
/*!
 * \brief Return the maximum `x` that can be passed to
 *        cudaFuncSetAttribute(f, cudaFuncAttributeMaxDynamicSharedMemorySize, x)
 * \param device_id The device index of the cuda-capable gpu of interest.
 * \return the shared memory size per streaming multiprocessor.
 */
inline int MaxSharedMemoryPerBlockOptin(int device_id) {
  static std::vector<int32_t> max_smem_limit_with_optin(kMaxNumGpus, -1);
  return cudaAttributeLookup(device_id, &max_smem_limit_with_optin,
                             cudaDevAttrMaxSharedMemoryPerBlockOptin,
                             "MaxSharedMemoryPerBlockOptin");
}

/*!
 * \brief Return the number of copy engines that can be active during kernel executions
 * \param device_id The device index of the cuda-capable gpu of interest.
 * \return the number of asynchronous copy engines
 */
inline int AsyncEngineCount(int device_id) {
  static std::vector<int32_t> num_async_engines(kMaxNumGpus, -1);
  return cudaAttributeLookup(device_id, &num_async_engines,
...
using cubFunc = cudaError_t (*)(void *d_temp_storage, size_t &temp_storage_bytes,  // NOLINT(*)
                                InputT *d_in, OutputT *d_out, int num_items,
                                cudaStream_t stream, bool debug_synchronous);

template<typename InputT, typename OutputT>
size_t GetCublasMemorySize(int num_items, mshadow::Stream<mxnet::gpu> *s,
                           cubFunc<InputT, OutputT> cublasFunc) {
  cudaStream_t stream = mshadow::Stream<mxnet::gpu>::GetStream(s);
  size_t temp_storage_bytes = 0;
  (*cublasFunc)(nullptr, temp_storage_bytes, nullptr, nullptr, num_items, stream, false);
