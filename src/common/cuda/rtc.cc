...
CUfunction get_function(const std::string &parameters,
  ...
  // log per-thread local mem use
  bool output_local_mem_use = false;
  if (kinfo.ptx.size() == 0) {
    ...
    auto verbose_level = dmlc::GetEnv("MXNET_RTC_VERBOSE", 0);
    if (verbose_level > 0) {
              LOG(INFO) << "\n" << std::string(80, '-') << "\n" << (parameters + code);
    // Actual output of mem use info is performed below after CUfunction is created
        if (verbose_level > 1)
            output_local_mem_use = true;
    }
  ...
  if (output_local_mem_use) {
    LOG(INFO) << "Kernel per-thread local memory use = "
              << GetLocalMemUse(kinfo.functions[dev_id]) << " bytes";
    if (kinfo.mangled_name.find("batch_norm") != std::string::npos) {
        LOG(WARNING) << "Spills in " << kinfo.mangled_name.c_str()
            << GetLocalMemUse(kinfo.functions[dev_id]) << " bytes\n";
    }
  }
