...
inline void BilinearSamplerForward(const Tensor<gpu, 4, DType> &output,
    ...
    // Kernel has integer indexing
    CHECK_LE(output.shape_.Size(), std::numeric_limits<int32_t>::max());
    auto num_blocks = (output.shape_.Size() + kMaxThreadsPerBlock - 1) / kMaxThreadsPerBlock;
    dim3 grid_dim(makeGrid(num_blocks));
    dim3 threads_per_block(kMaxThreadsPerBlock);
    CheckLaunchParam(grid_dim, threads_per_block, "bilinear sampler forward");
    cudaStream_t stream = Stream<gpu>::GetStream(output.stream_);
    cuda::BilinearSamplerForwardKernel<DType> << <grid_dim, threads_per_block, 0, stream >> >(
...
inline void BilinearSamplerBackward(const Tensor<gpu, 4, DType> &input_grad,
  ...
  using namespace cuda;
  const int64_t num_blocks = (output_grad.shape_.Size() / o_c + kMaxThreadsPerBlock - 1)
                             / kMaxThreadsPerBlock;
  dim3 grid_dim(makeGrid(num_blocks));
  dim3 threads_per_block(kMaxThreadsPerBlock);
  CheckLaunchParam(grid_dim, threads_per_block, "bilinear sampler backward");
  ...
  MXNET_REQ_TYPE_SWITCH(data_req, Req1, {
    MXNET_REQ_TYPE_SWITCH(grid_req, Req2, {
      ...
      <<<grid_dim, threads_per_block, 0, stream >>>(
