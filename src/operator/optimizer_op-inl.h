  ...
  MSHADOW_XINLINE static void Map(index_t i, const MultiSGDKernelParam<DType, MPDType>& param,
    ...
    for (int index = 0; index < param.count; ++index) {
      if (i < static_cast<index_t>(param.sizes[index])) {
        ...
        MPDType mom = has_momentum ? param.mom[index][i] : MPDType(0);
        mom = param.momentum * mom;
        const auto g = static_cast<MPDType>(param.grads[index][i]);
        MPDType g_rescaled = g * param.rescale_grad;
        if (param.clip_gradient >= 0.0f) {
          g_rescaled = mshadow_op::clip::Map(g_rescaled, param.clip_gradient);
        }
        const auto temp = g_rescaled + param.wds[index] * w;
        mom -= param.lrs[index] * temp;
        w = w + param.momentum * mom - param.lrs[index] * temp;
        if (has_momentum) {
          param.mom[index][i] = mom;
        }
        if (has_mixed_precision) {
...
template<typename xpu,
         ...
         int input_stride = 2>
void FillMultiSGDKernelParam(const nnvm::NodeAttrs& attrs,
                             const OpContext &ctx,
                             const std::vector<TBlob> &inputs,
                             const std::vector<TBlob> &outputs,
                             MultiSGDKernelParam<DType, MPDType> *in_param) {
...
template<typename xpu,
         ...
         typename MPDType,
         int input_stride = 3>
void FillMultiSGDMomKernelParam(const nnvm::NodeAttrs& attrs,
                                const OpContext &ctx,
                                const std::vector<TBlob> &inputs,
                                const std::vector<TBlob> &outputs,
                                MultiSGDKernelParam<DType, MPDType> *in_param) {
  ...
  FillMultiSGDKernelParam<xpu,
                          ...
                          MPDType,
                          MultiSGDMomParam,
                          input_stride>(attrs, ctx, inputs, outputs, &param);
...
template<typename xpu, template<typename> class MPTypeChooser, int input_stride,
         template <typename, bool, bool> class Optimizer>
inline void MultiMomUpdate(const nnvm::NodeAttrs& attrs,
                           const OpContext &ctx,
                           const std::vector<TBlob> &inputs,
                           const std::vector<OpReqType> &req,
                           const std::vector<TBlob> &outputs) {
    ...
    for (index_t j = 0; j < row_length; j++) {
      ...
      if (clip_gradient >= 0.0f) {
        mom_data[data_i] = momentum * mom_data[data_i]
                - wd * weight_data[data_i]
                - mshadow_op::clip::Map(rescale_grad * grad_data[grad_i],
                                        clip_gradient);
      } else {
        mom_data[data_i] = momentum * mom_data[data_i]
                  - wd * weight_data[data_i]
                  - rescale_grad * grad_data[grad_i];
      }
