...
template<typename xpu, typename InputT, typename OutputT>
size_t GetAdditionalMemorySize(const int num_items, mshadow::Stream<xpu> *s);
...
void ApproxGradient(const nnvm::NodeAttrs& attrs,
  ...
  const auto num_items = in_array0.Size();
  ...
  const auto extra = GetAdditionalMemorySize<xpu, float, float>(num_items, ctx.get_stream<xpu>());
  const auto extraBytes = (sizeof(float) + extra) * (param.batched_mode? param.index : 1);

  auto workspaceMem = AllocMemory<xpu, float>(ctx, num_items, extraBytes);
  ApproxGradientAction(ctx.get_stream<xpu>(), workspaceMem, extra,
