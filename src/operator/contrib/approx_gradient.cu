...
template<>
size_t GetAdditionalMemorySize<gpu, float, float>(const int num_items, mshadow::Stream<gpu> *s) {
  return GetCublasMemorySize<float, float>(num_items, s, cub::DeviceReduce::Sum);
}
