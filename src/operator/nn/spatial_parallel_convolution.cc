...
ListArguments(const SpatialParallelConvolutionParam& param_) {
  if (!param_.no_bias) {
    return {"data", "weight", "dummy", "bias"};
  } else {
    return {"data", "weight", "dummy"};
  }
}

static bool SPConvChangeLayout(nnvm::NodeAttrs* attrs, mshadow::LayoutFlag target_layout,
                             std::vector<alm::Transpose>* in_axes,
                             std::vector<alm::Transpose>* out_axes) {
  const auto& param = nnvm::get<SpatialParallelConvolutionParam>(attrs->parsed);
...
static bool SPConvolutionShape(const nnvm::NodeAttrs& attrs,
  ...
  if (!param_.no_bias) {
    CHECK_EQ(in_shape->size(), 4U) << "Input:[data, weight, dummy, bias]";
  } else {
    CHECK_EQ(in_shape->size(), 3U) << "Input:[data, weight, dummy]";
  }
...
.set_attr<nnvm::FListInputNames>("FListInputNames",
  ...
  const SpatialParallelConvolutionParam& params =
    nnvm::get<SpatialParallelConvolutionParam>(attrs.parsed);
  if (params.no_bias)
    return std::vector<std::string>{"data", "weight", "dummy"};
  else
    return std::vector<std::string>{"data", "weight", "dummy", "bias"};
})
...
.set_attr<mxnet::alm::FChangeLayout>("FChangeLayout", SPConvChangeLayout)
...
.add_argument("dummy", "NDArray-or-Symbol", "Dummy parameter needed to make sure no "
                                            "2 spatial parallel convolutions are performed "
                                            "at the same time.")
