...
static bool BatchNormAddReluShape(const nnvm::NodeAttrs& attrs,
  ...
  CHECK_EQ(in_shape->size(), 6U) << "Input:[data, gamma, beta, MovingMean, MovingVar, addend]";
...
#define ROUND_UP_TO_MULTIPLE(x, multiple) (((x + multiple - 1) / multiple) * multiple)
  ...
  const int MAX_C_ELEMENTS_PER_CTA = 256;
  const int int32_bits = 32;
  const int effective_c = ROUND_UP_TO_MULTIPLE(channelCount, MAX_C_ELEMENTS_PER_CTA);
...
static bool BatchNormAddReluType(const nnvm::NodeAttrs& attrs,
  ...
  std::vector<std::string> args{"data", "gamma", "beta", "mean", "var", "addend"};
...
std::vector<nnvm::NodeEntry> BatchNormAddReluGrad(const nnvm::ObjectPtr& n,
  ...
  in_grad[batchnormaddrelu::kAddend] = nnvm::NodeEntry{gnode, batchnormaddrelu::kInMovingMean, 0};
  return in_grad;
}
...
.set_attr<nnvm::FListInputNames>("FListInputNames",
  ...
  return std::vector<std::string>{"data", "gamma", "beta", "moving_mean", "moving_var", "addend"};
})
...
.set_attr<nnvm::FInplaceOption>("FInplaceOption", [](const NodeAttrs& attrs){
  // input addend can share its tensor with output (y)
  return std::vector<std::pair<int, int> >{{batchnormaddrelu::kAddend, batchnormaddrelu::kOut}};
})
