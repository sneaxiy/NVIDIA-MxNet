...
void LayerNormGPUContig(const LayerNormParam param,
  ...
  CHECK_LE(nbatch, std::numeric_limits<int32_t>::max());
  int nchannel = data_shape[1];
  float eps = param.eps;
  int ngrid_x = (nbatch > kMaxGridDimX) ? (nbatch + kBaseGridNum - 1) / kBaseGridNum : nbatch;
  int ngrid_y = (nbatch > kMaxGridDimX) ? kBaseGridNum : 1;
  int nthread_y;
