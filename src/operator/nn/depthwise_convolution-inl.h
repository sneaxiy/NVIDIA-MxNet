...
void DepthwiseConv2dForwardGpu(mshadow::Stream<gpu> *stream,
  ...
  if (CanLaunchDepthwiseConv2dGPUSmall(args)) {
    ...
    auto num_output = out_data[conv::kOut].shape_.Size();
    // Kernel uses integer indexing
    CHECK_LE(num_output, std::numeric_limits<int32_t>::max());
    auto num_blocks = (num_output +
                       mshadow::cuda::kBaseThreadNum + 1) / mshadow::cuda::kBaseThreadNum;
    ...
    if (args.filter_height == 3 && args.filter_width == 3) {
      DepthwiseConv2dForwardKernel<DType, 3, 3>
          <<<num_blocks, mshadow::cuda::kBaseThreadNum, 0, s>>>(data.dptr_,
                                                               weight.dptr_,
                                                               args,
                                                               num_output,
                                                               out.dptr_);
...
void DepthwiseConv2dBackwardDataGpu(mshadow::Stream<gpu> *stream,
  ...
  if (CanLaunchDepthwiseConv2dGPUSmall(args)) {
    ...
    auto num_in_grad = in_grad[conv::kData].shape_.Size();
    // Kernel uses integer indexing
    CHECK_LE(num_in_grad, std::numeric_limits<int32_t>::max());
    auto s = mshadow::Stream<gpu>::GetStream(stream);
    auto num_blocks = (num_in_grad +
                       mshadow::cuda::kBaseThreadNum + 1) / mshadow::cuda::kBaseThreadNum;
    DepthwiseConv2dBackwardDataKernel<DType>
        <<<num_blocks, mshadow::cuda::kBaseThreadNum, 0, s>>>(args,
                                                             out_g.dptr_,
                                                             weight.dptr_,
                                                             in_data_g.dptr_,
                                                             num_in_grad);
...
void DepthwiseConv2dBackwardFilterGpu(mshadow::Stream<gpu> *stream,
  ...
  if (TryLaunchDepthwiseConv2dBackwardFilterGPUSmall<DType>(stream, args,
    ...
    auto num_out_grad = out_grad[conv::kOut].shape_.Size();
    // Kernel uses integer indexing
    CHECK_LE(num_out_grad, std::numeric_limits<int32_t>::max());
    auto s = mshadow::Stream<gpu>::GetStream(stream);
    int block_num = args.out_channel * args.batch;
