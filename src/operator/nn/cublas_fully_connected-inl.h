...
/**
 * \brief NVIDIA Container only:
 *        This is the implementation of fully connected operator for cuBLAS.
 *        It gives more algo and precision tuning options, not avaible upstream.
 */
    ...
    switch (output_request) {
      ...
      case kWriteInplace: {
          auto blas_handle = mshadow::Stream<gpu>::GetBlasHandle(s);
            ...
            auto new_math_mode = (algo >= CUBLAS_GEMM_DFALT_TENSOR_OP) ? CUBLAS_TENSOR_OP_MATH
                                                                       : VERSION_ADJUSTED_TF32_MATH;
            if (handle_math_mode != new_math_mode)
              CUBLAS_CALL(cublasSetMathMode(blas_handle, new_math_mode));
          #endif
