...
class CuDNNNormConvolutionOp {
 public:
  ...
  void Forward(const OpContext &ctx,
    ...
    auto workspace = AllocateTempWorkspace(ctx, fwd_workspace_byte_, norm_conv::kTempSpace);
  ...
  void Backward(const OpContext &ctx,
    ...
    auto workspace = AllocateTempWorkspace(ctx, backward_workspace_byte, norm_conv::kTempSpace);
    ...
    if (req[norm_conv::kData] != kNullOp) {
      ...
      if (!dgrad_as_gemm_) {
        ...
        int GEMM_N = static_cast<int>(GetNHW(fwd_in_data[norm_conv::kData].shape_));
        int output_features = static_cast<int>(Features(out_grad[norm_conv::kOut].shape_));
        ...
        mshadow::Tensor<gpu, 2, DType> wmat =
          fwd_in_data[input_weight_idx].get_with_shape<gpu, 2, DType>(Shape2(GEMM_M, GEMM_K), s);
        mshadow::Tensor<gpu, 2, DType> grad =
          out_grad[norm_conv::kOut].get_with_shape<gpu, 2, DType>(Shape2(GEMM_K, GEMM_N), s);
        mshadow::Tensor<gpu, 2, DType> gdata =
          in_grad[norm_conv::kData].get_with_shape<gpu, 2, DType>(Shape2(GEMM_M, GEMM_N), s);

        int32_t algo_precision = -1;  // No params to set policy for this, so pseudo-fp16.
