...
class NhwcBatchNormAddReluOp {
 public:
  ...
  void Init(const BatchNormAddReluParam &param) {
    if (!nhwc_layer_)
      nhwc_layer_.reset(new NhwcBatchNormAddRelu(param.bn_group));
  ...
  void Forward(const OpContext &ctx,
    ...
    Init(in_data[nhwcbatchnormaddrelu::kData], s, ctx.run_ctx.ctx.dev_id, ctx.is_train, true);
    ...
    MSHADOW_REAL_TYPE_SWITCH(dtype_param_, DTypeParam, {
      ...
      if (ctx.is_train) {
        ...
        nhwc_layer_->fwdInference(s, ctx.run_ctx.ctx.dev_id);
      }
    })
  }
  ...
  void Backward(const OpContext &ctx,
    ...
    Stream<gpu> *s = ctx.get_stream<gpu>();
    Init(in_data, s, ctx.run_ctx.ctx.dev_id);
 ...
 */
 ...
 private:
  ...
  void Init(const TBlob &in_data, mshadow::Stream<gpu> *s,
            ...
            const bool& is_fwd = false) {
      dim_ = in_data.ndim();
      CHECK(dim_ >= 3 && dim_ <= 5) << "NHWC BatchNorm Op data dim " << dim_
                                    << " not in supported range [3,5].";
      param_.axis = batchnorm::GetRealAxis(in_data.shape_, param_.axis);
      auto supported_axis = dim_ - 1;
      ...
      workspace_bytes_ = nhwc_layer_->numWorkspaceBytes(is_fwd);
