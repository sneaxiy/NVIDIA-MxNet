 ...
 * \author Shankara Rao Thejaswi Nanditale, Dick Carter, Kartikeya Goyal
*/
...
class NhwcBatchNorm {
 public:
  ...
  NhwcBatchNorm() = delete;  // will never be generated
  explicit NhwcBatchNorm(int gbn) :
  ...
  inline void fwd(cudaStream_t stream, bool use_relu, int device_id);
  inline void dgrad(cudaStream_t stream, bool use_relu, int device_id);
  template <typename gpu>
  inline void fwdInference(mshadow::Stream<gpu> *s, bool use_relu, int device_id);
  ...
  void useHeuristicHelper(bool is_bwd, bool is_add, size_t m, int c, const int& device_id,
  ...
  void useHeuristicFwd(size_t m, int c, int device_id, bool gbn);
  void useHeuristicBwd(size_t m, int c, int device_id, bool gbn);

  void useHeuristic(size_t m, int c, int device_id, bool gbn) {
    useHeuristicFwd(m, c, device_id, gbn);
    useHeuristicBwd(m, c, device_id, gbn);
  ...
  void setInputOutputParams(size_t input_size, int c, int bn_group, int device_id) {
    m_ = input_size / c;
  ...
  inline void setWorkspacePointers(const std::vector<void*>& workspace);
 ...
 protected:
  ...
  void _fwdKernelLauncher(cudaStream_t stream, NhwcBatchNormFwdParams params,
    ...
    std::string code_params = NhwcBatchNormCode::get_bhwc_batch_norm_params(THREADS_PER_PIXEL_FWD,
                                              ...
                                              PIXELS_PER_THREAD_IN_REGISTERS_FWD,
                                              pixels_per_thread_in_smem_fwd, ELEMENTS_PER_LDG_FWD,
                                              USE_ONLINE_APPROACH, outer_loops, params.sync_iters,
                                              use_relu, false, occupancy);
    std::string old_name = "nhwc_batch_norm_fwd";
    std::string new_name = "nhwc_batch_norm_fwd_" +
                           std::string(use_relu ? "relu_" : "") +
    ...
    auto kernel = mxnet::common::cuda::rtc::get_function(code_params,
                                ...
                                NhwcBatchNormCode::get_nhwc_batch_norm_kernel(
                                    ...
                                    new_name),
                                device_id);
    checkCudaStatus(name_ + " mxnet::common::cuda::rtc::get_function");
    ...
    ENSURE_MAX_DYN_SMEM_LIMIT(kernel, dynamic_smem_size_fwd, static_smem_size_fwd,
                                  device_id, name_ + " fwd coop serial kernel");
    auto err = cuLaunchCooperativeKernel(kernel, grid_dim.x, grid_dim.y, grid_dim.z,
            THREADS_PER_CTA, 1, 1, dynamic_smem_size_fwd,
            stream,  const_cast<void**>(args.data()));
    ...
    if (err != CUDA_SUCCESS) {
        ...
        cuOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocks,
                kernel, THREADS_PER_CTA, dynamic_smem_size_fwd);
  ...
  void _bwdKernelLauncher(cudaStream_t stream, NhwcBatchNormBwdParams params,
    ...
    std::string code_params = NhwcBatchNormCode::get_bhwc_batch_norm_params(THREADS_PER_PIXEL_BWD,
                                          ...
                                          ELEMENTS_PER_LDG_BWD, USE_ONLINE_APPROACH,
    ...
    if (use_relu) {
      code_s = std::string(NhwcBatchNormCode::nhwc_batch_norm_common) +
               std::string(NhwcBatchNormCode::nhwc_batch_norm_bwd_relu);
      old_name = "nhwc_batch_norm_bwd_relu";
    } else {
      code_s = std::string(NhwcBatchNormCode::nhwc_batch_norm_common) +
               std::string(NhwcBatchNormCode::nhwc_batch_norm_bwd);
      old_name = "nhwc_batch_norm_bwd";
    }
    new_name = "nhwc_batch_norm_bwd_" +
               std::string(use_relu ? "relu_" : "") +
    ...
    auto kernel = mxnet::common::cuda::rtc::get_function(code_params,
                                ...
                                device_id);
    cuFuncSetAttribute(kernel, CU_FUNC_ATTRIBUTE_PREFERRED_SHARED_MEMORY_CARVEOUT, 100);
    checkCudaStatus(name_ + new_name + " ser coop kernel (cuFuncSetAttribute carveout)");
    ENSURE_MAX_DYN_SMEM_LIMIT(kernel, dynamic_smem_size_bwd, static_smem_size_bwd,
                                  device_id, name_ + new_name + " coop serial kernel");
    auto err = cuLaunchCooperativeKernel(kernel, grid_dim.x, grid_dim.y, grid_dim.z,
            THREADS_PER_CTA, 1, 1,
            dynamic_smem_size_bwd, stream, const_cast<void**>(args.data()));
    checkCudaStatus(name_ + new_name + " ser coop kernel");
    if (err != CUDA_SUCCESS) {
        const char* error_string;
        cuGetErrorString(err, &error_string);
        int numBlocks = 0;
        cuOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocks, kernel,
                THREADS_PER_CTA, dynamic_smem_size_bwd);
        LOG(FATAL) << "cuLaunchCooperativeKernel failed: "
               << err << " " << error_string << ": "
               << new_name << " "
               << "numBlocks = " << numBlocks << " "
 ...
 private:
  ...
  int get_max_smem_fwd(int device_id) {
    ...
    if (sm_arch < 70) {
        smem_per_sm = MaxSharedMemoryPerBlockOptin(device_id);
    }
...
const std::vector<size_t> NhwcBatchNorm::numWorkspaceBytes(const bool& is_fwd) const {
  assert(c_ > 0);
...
template <typename gpu>
void NhwcBatchNorm::fwdInference(mshadow::Stream<gpu> *s, bool use_relu, int device_id) {
  bool ptrs_are_set =
      scale_ != nullptr
      && bias_ != nullptr
...
void NhwcBatchNorm::init_fwd_params(int new_pixels_per_thread_in_smem_fwd) {
...
void NhwcBatchNorm::init_bwd_params(int new_pixels_per_thread_in_smem_bwd) {
...
dim3 NhwcBatchNorm::calc_fwd_grid(int device_id, int *loop, bool use_group_bn) {
...
dim3 NhwcBatchNorm::calc_bwd_grid(int device_id, int *loop, bool use_group_bn) {
  // set smem config to max for current device
  ...
  if (grid_dim.x <= max_grid_x) {
    *loop = 1;
    ...
    int pixels_per_iteration = PIXELS_PER_THREAD_IN_REGISTERS_BWD * PIXELS_PER_LDG_BWD * grid_dim.x;
    *loop = div_up(nhw_in_regs, pixels_per_iteration);
  }
  return grid_dim;
}
