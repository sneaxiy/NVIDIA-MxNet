...
class CuDNNNormalizedConvolutionOp {
 public:
  ...
  void Forward(const OpContext &ctx,
    ...
    auto workspace = AllocateTempWorkspace(ctx, fwd_workspace_byte_,
                                           normalized_conv::kTempSpace);
  ...
  void Backward(const OpContext &ctx,
    ...
    if (req[normalized_conv::kData] != kNullOp) {
      ...
      if (!dgrad_as_gemm_) {
        ...
        int GEMM_N = static_cast<int>(GetNHW(in_data[normalized_conv::kData].shape_));
        int output_features = static_cast<int>(Features(out_grad[normalized_conv::kOut].shape_));
        ...
        mshadow::Tensor<gpu, 2, DType> wmat =
          in_data[weight_idx].get_with_shape<gpu, 2, DType>(Shape2(GEMM_M, GEMM_K), s);
        mshadow::Tensor<gpu, 2, DType> grad =
          out_grad[normalized_conv::kOut].get_with_shape<gpu, 2, DType>(Shape2(GEMM_K, GEMM_N), s);
        mshadow::Tensor<gpu, 2, DType> gdata =
          in_grad[normalized_conv::kData].get_with_shape<gpu, 2, DType>(Shape2(GEMM_M, GEMM_N), s);
