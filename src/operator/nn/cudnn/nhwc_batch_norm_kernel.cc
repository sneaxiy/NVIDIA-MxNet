...
const char NhwcBatchNormCode::nhwc_batch_norm_common[] = R"code(
...
DEVICE_FUNCTION void ldg(int (&dst)[4], const uint16_t *gmem) {
    int4 tmp = __ldg((const int4*) gmem);
    dst[0] = tmp.x;
    dst[1] = tmp.y;
    dst[2] = tmp.z;
    dst[3] = tmp.w;
...
DEVICE_FUNCTION void ldg_stream(int (&dst)[4], const uint16_t *gmem) {
    ...
    asm volatile ("ld.global.cs.nc.v4.s32 {%0,%1,%2,%3}, [%4];"
        : "=r"(tmp.x), "=r"(tmp.y), "=r"(tmp.z), "=r"(tmp.w) : "l"((const int4 *)gmem));
    dst[0] = tmp.x;
    dst[1] = tmp.y;
    dst[2] = tmp.z;
    dst[3] = tmp.w;
...
DEVICE_FUNCTION void stg_stream(uint16_t *gmem, int (&src)[4]) {
    asm volatile ("st.global.cs.v4.s32 [%0], {%1,%2,%3,%4};"
        :: "l"((uint *)gmem) , "r"(src[0]), "r"(src[1]), "r"(src[2]), "r"(src[3]));
}
...
DEVICE_FUNCTION void read_from_gmem(float (&dst)[8], const float *gmem, int idx) {
    float4 tmp = __ldg(reinterpret_cast<const float4*>(&gmem[8*idx]));
    dst[0] = tmp.x;
    dst[1] = tmp.y;
    dst[2] = tmp.z;
    dst[3] = tmp.w;

    float4 tmp1 = __ldg(reinterpret_cast<const float4*>(&gmem[8*idx + 4]));
    dst[4] = tmp1.x;
    dst[5] = tmp1.y;
    dst[6] = tmp1.z;
    dst[7] = tmp1.w;
...
DEVICE_FUNCTION void read_from_smem(float (&x)[8], const float *smem, int idx) {
    ...
    float4 tmp1 = *(const float4*) &smem[8*idx + 4];
    x[4] = tmp1.x;
    x[5] = tmp1.y;
    x[6] = tmp1.z;
    x[7] = tmp1.w;
...
DEVICE_FUNCTION void read_from_smem(int (&x)[4], const int *smem, int idx) {
    int4 tmp = *(const int4*) &smem[4*idx];
    x[0] = tmp.x;
    x[1] = tmp.y;
    x[2] = tmp.z;
    x[3] = tmp.w;
...
DEVICE_FUNCTION void scaled_write_to_gmem(float *gmem, int idx, const float (&src)[4],
                                        const float coeff) {
    reinterpret_cast<float4*>(&gmem[4*idx])[0] =
        make_float4(src[0]*coeff, src[1]*coeff, src[2]*coeff, src[3]*coeff);
}
DEVICE_FUNCTION void scaled_write_to_gmem(float *gmem, int idx, const float (&src)[8],
                                        const float coeff) {
    reinterpret_cast<float4*>(&gmem[8*idx])[0] =
        make_float4(src[0]*coeff, src[1]*coeff, src[2]*coeff, src[3]*coeff);
    reinterpret_cast<float4*>(&gmem[8*idx+4])[0] =
        make_float4(src[4]*coeff, src[5]*coeff, src[6]*coeff, src[7]*coeff);
}
...
DEVICE_FUNCTION void write_to_smem(float *smem, int idx, const float (&x)[8]) {
    reinterpret_cast<float4*>(&smem[8*idx])[0] = make_float4(x[0], x[1], x[2], x[3]);
    reinterpret_cast<float4*>(&smem[8*idx+4])[0] = make_float4(x[4], x[5], x[6], x[7]);
}
...
template< int THREADS_PER_CTA, int ELEMENTS_PER_LDG >
DEVICE_FUNCTION void parallel_sums_32x1(float *smem, float (&x)[ELEMENTS_PER_LDG], int nhw,
                                        void* params_my_data, void** pair_data, int off,
                                        const int sync_iters) {
    // The number of warps in a CTA.
    const int THREADS_PER_PIXEL = 32;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
template< int THREADS_PER_CTA, int ELEMENTS_PER_LDG >
DEVICE_FUNCTION void parallel_sums_16x2(float *smem, float (&x)[8], int nhw,
                                        void* params_my_data, void** pair_data, int off,
                                        const int sync_iters) {
    // The number of warps in a CTA.
    ...
    if (warp_id == 0) {
        ...
        // This shuffle result in both half warp with final result
        #pragma unroll
        for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
            x[i] += __shfl_sync(0xffffffffU, x[i], THREADS_PER_PIXEL+lane_id);
        }
...
DEVICE_FUNCTION void parallel_sums_16x2(float *smem, float (&x)[4], int nhw,
    ...
    if (warp_id == 0) {
        ...
        // Each thread has ELEMENTS_PER_LDG(4) elements to exchange, and
        // we need THREADS_PER_PIXEL(16) threads(half warp) to do the work.
        // Since each half warp now have the final data, we let both of
        // them handle half the data, which is ELEMENTS_PER_LDG / 2 == 2.
        // We may need to re-visit this if constants are changed

        // 2 half warp get different x
        float x_half[2];  // copy in data from x
        x_half[0] = x[2*(threadIdx.x / THREADS_PER_PIXEL)];
        x_half[1] = x[2*(threadIdx.x / THREADS_PER_PIXEL)+1];

        for (int sync_iter=0; sync_iter < sync_iters; ++sync_iter) {
            void * params_pair_data = pair_data[sync_iter];
            ...
            + ELEMENTS_PER_LDG*(threadIdx.x%THREADS_PER_PIXEL)*2;
            ...
            if (blockIdx.x == 0) {
                ...
                // 2 half warp get different buffer
                write_data = write_data + 4*(threadIdx.x / THREADS_PER_PIXEL);

                do {
                    asm volatile ("ld.volatile.global.v4.b32 {%0, %1, %2, %3}, [%4];"
                        : "=f"(other[0]), "=r"(other_flag_a), "=f"(other[1]),
                        "=r"(other_flag_b) : "l"(write_data));
                } while (other_flag_a != 0);

                // write the data to memory region to be reflected to other GPU
                asm volatile ("st.global.wt.v4.b32 [%0], {%1,%2,%3,%4};"
                    :: "l"(write_data) , "f"(x_half[0]), "r"(gridDim.x),
                    "f"(x_half[1]), "r"(gridDim.x));
            }
            ...
            // 2 half warp get different buffer
            read_data = read_data + 4*(threadIdx.x / THREADS_PER_PIXEL);

            do {
                asm volatile ("ld.volatile.global.v4.b32 {%0, %1, %2, %3}, [%4];"
                    : "=f"(other[0]), "=r"(other_flag_a), "=f"(other[1]),
                    "=r"(other_flag_b) : "l"(read_data));
            } while (other_flag_a == 0);

            atomicSub(const_cast<int*>(reinterpret_cast<volatile int*>(read_data + 1)), 1);
        ...
        // offset second half warp by 2
        smem = smem + 2*(threadIdx.x / THREADS_PER_PIXEL);
        // 2* here to get offset into smem back to by 4 elements, since x_half now has 2
        write_to_smem(smem, 2*(threadIdx.x%THREADS_PER_PIXEL), x_half);
    }
}
...
template< int THREADS_PER_CTA>
DEVICE_FUNCTION void parallel_sums_8x8(float *smem, float (&x)[8], int nhw) {
    // The size of a warp.
    const int THREADS_PER_WARP = 32;
    // The number of warps in a CTA.
    const int WARPS_PER_CTA = THREADS_PER_CTA / THREADS_PER_WARP;
...
template< int THREADS_PER_CTA>
DEVICE_FUNCTION void parallel_sums_4x8(float *smem, float (&x)[8], int nhw) {
    // The size of a warp.
    const int THREADS_PER_WARP = 32;
    // The number of warps in a CTA.
    const int WARPS_PER_CTA = THREADS_PER_CTA / THREADS_PER_WARP;
    // The number of threads per pixel.
    const int THREADS_PER_PIXEL = 4;
    // The number of elements per ldg.
    const int ELEMENTS_PER_LDG = 8;
    // The warp decomposition.
    const int warp_id = threadIdx.x / THREADS_PER_WARP;
    ...
    for (int i = 0; i < ELEMENTS_PER_LDG; ++i) {
        x[i] += __shfl_sync(0xffffffffU, x[i], THREADS_PER_PIXEL+lane_id);
        x[i] += __shfl_sync(0xffffffffU, x[i], THREADS_PER_PIXEL*2+lane_id);
        x[i] += __shfl_sync(0xffffffffU, x[i], THREADS_PER_PIXEL*4+lane_id);
    }
...
struct ParallelSums<16, 4> {
    ...
    DEVICE_FUNCTION void dispatch(float *smem, float (&x)[4], int nhw) {
        parallel_sums_16x2<THREADS_PER_CTA, 4>(smem, x, nhw, 0, 0, 0, 0);
    }

    template< int THREADS_PER_CTA >
    DEVICE_FUNCTION void dispatchX(float *smem, float (&x)[4], int nhw,
                                void* params_my_data, void** pair_data,
                                int off, const int sync_iters) {
        parallel_sums_16x2<THREADS_PER_CTA, 4>(smem, x, nhw, params_my_data, pair_data,
                                            off, sync_iters);
    }
};

template<>
struct ParallelSums<16, 8> {
    template< int THREADS_PER_CTA >
    DEVICE_FUNCTION void dispatch(float *smem, float (&x)[8], int nhw) {
        parallel_sums_16x2<THREADS_PER_CTA, 8>(smem, x, nhw, 0, 0, 0, 0);
    }

    template< int THREADS_PER_CTA >
    DEVICE_FUNCTION void dispatchX(float *smem, float (&x)[8], int nhw,
                                void* params_my_data, void** pair_data,
                                int off, const int sync_iters) {
        parallel_sums_16x2<THREADS_PER_CTA, 8>(smem, x, nhw, params_my_data, pair_data,
                                            off, sync_iters);
    }
};

template<>
struct ParallelSums<32, 8> {
    template< int THREADS_PER_CTA >
    DEVICE_FUNCTION void dispatch(float *smem, float (&x)[8], int nhw) {
        parallel_sums_32x1<THREADS_PER_CTA, 8>(smem, x, nhw, 0, 0, 0, 0);
    }
    ...
    DEVICE_FUNCTION void dispatchX(float *smem, float (&x)[8], int nhw,
                                void* params_my_data, void** pair_data,
                                int off, const int sync_iters) {
        // TODO(kgoyal): Enable GBN for 8 elements per thread
    }
};
...
template<>
struct ParallelSums<8, 8> {
    template< int THREADS_PER_CTA >
    DEVICE_FUNCTION void dispatch(float *smem, float (&x)[8], int nhw) {
        parallel_sums_8x8<THREADS_PER_CTA>(smem, x, nhw);
    }
    template< int THREADS_PER_CTA >
    DEVICE_FUNCTION void dispatchX(float *smem, float (&x)[8], int nhw,
                                void* params_my_data, void** pair_data,
...
template<>
struct ParallelSums<4, 8> {
    template< int THREADS_PER_CTA >
    DEVICE_FUNCTION void dispatch(float *smem, float (&x)[8], int nhw) {
        parallel_sums_4x8<THREADS_PER_CTA>(smem, x, nhw);
    }
    template< int THREADS_PER_CTA >
    DEVICE_FUNCTION void dispatchX(float *smem, float (&x)[8], int nhw,
                                void* params_my_data, void** pair_data,
...
DEVICE_FUNCTION int div_up(int m, int n) {
    return (m + n - 1) / n;
}
...
const char NhwcBatchNormCode::nhwc_batch_norm_fwd_inference[] =
    R"code(
...
const char NhwcBatchNormCode::nhwc_batch_norm_fwd[] =
    R"code(
...
__global__ __launch_bounds__(THREADS_PER_CTA, DESIRED_OCCUPANCY)
    ...
    for (int c_blk_index = blockIdx.y; c_blk_index < params.c_blks; c_blk_index += gridDim.y) {
        ...
        if (!is_valid_c)
            thread_c = params.c - ELEMENTS_PER_LDG;
        ...
        for (int loop_i = 0; loop_i < OUTER_LOOPS; ++loop_i) {
            ...
            if (USE_ONLINE_APPROACH) {
                ...
                #pragma unroll
                for (int i = 0; i < PIXELS_PER_THREAD_IN_REGISTERS; ++i) {
                    // kgoyal const int idx = nhw_regs + thread_in_cta_nhw + i*PIXELS_PER_LDG;
                    // kgoyal if (((unsigned int)idx < (unsigned int)params.nhw) && is_valid_c) {
                        // Convert to float.
                        float x_math[ELEMENTS_PER_LDG];
        ...
        if (SYNC_ITERS) {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatchX<THREADS_PER_CTA>(
                smem, m1, thread_in_cta_nhw, params.my_data, params.pair_data, 4*c_blk_index+3,
                params.sync_iters);
        } else {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<THREADS_PER_CTA>(
                smem, m1, thread_in_cta_nhw);
        ...
        if (SYNC_ITERS) {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatchX<THREADS_PER_CTA>(
                smem, m2, thread_in_cta_nhw, params.my_data, params.pair_data, 4*c_blk_index+2,
                params.sync_iters);
        } else {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<THREADS_PER_CTA>(
                smem, m2, thread_in_cta_nhw);
        ...
        const int RELU_MULT = C_ELEMENTS_PER_CTA / 32;
        uint16_t *const gmem_dst = &params.gmem_dst[thread_c];
        unsigned int *const gmem_relu_bitmask = params.gmem_relu_bitmask +
                                    ((params.nhw + 31) & ~31) * RELU_MULT * c_blk_index;
        ...
        for (int loop_i = OUTER_LOOPS-1; loop_i >= 0; --loop_i) {
            ...
            for (int i = 0; i < PIXELS_PER_THREAD_IN_REGISTERS; ++i) {
                ...
                if (USE_ADD_RELU) {
                    ...
                    if (ELEMENTS_PER_LDG == 8 && THREADS_PER_PIXEL == 4) {
                        const bool is_valid_nhw_relu = 
                            static_cast<unsigned int>(idx) < static_cast<unsigned int>(((params.nhw+31) & ~31));
                        if (is_valid_nhw_relu && (lane_id < ELEMENTS_PER_LDG)) {
                            gmem_relu_bitmask[(idx & ~0x1) + lane_id] = relu_mask;
                        }
                    } else {
                        if (is_valid_nhw && (lane_id < ELEMENTS_PER_LDG)) {
                            gmem_relu_bitmask[(idx * RELU_MULT) + lane_id] = relu_mask;
                        }
                    }
                } else if (USE_RELU) {
                    relu_activation(x_math);
...
const char NhwcBatchNormCode::nhwc_batch_norm_bwd[] =
    R"code(
...
// We scale the mean by the number of elements. It brings more stability.
        ...
        if (SYNC_ITERS) {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatchX<THREADS_PER_CTA>(
                smem, dscale, thread_in_cta_nhw, params.my_data, params.pair_data, 4*c_blk_index+1,
                params.sync_iters);
        } else {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<THREADS_PER_CTA>(
                smem, dscale, thread_in_cta_nhw);
        ...
        if (SYNC_ITERS) {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatchX<THREADS_PER_CTA>(
                smem, dbias, thread_in_cta_nhw, params.my_data, params.pair_data, 4*c_blk_index+0,
                params.sync_iters);
        } else {
            ParallelSums<THREADS_PER_PIXEL, ELEMENTS_PER_LDG>::dispatch<THREADS_PER_CTA>(
                smem, dbias, thread_in_cta_nhw);
...
const char NhwcBatchNormCode::nhwc_batch_norm_bwd_relu[] =
    R"code(
struct NhwcBatchNormBwdParams {
    // The input/output tensors.
    uint16_t *gmem_src, *gmem_dy, *gmem_dst, *gmem_dst1;
...
const char NhwcBatchNormCode::nhwc_batch_norm_bwd_add_relu[] =
    R"code(
struct NhwcBatchNormBwdParams {
    // The input/output tensors.
    uint16_t *gmem_src, *gmem_dy, *gmem_dst, *gmem_dst1;
...
__global__ __launch_bounds__(THREADS_PER_CTA, DESIRED_OCCUPANCY)
    ...
    // The number of pixels loaded in a single LDG.
    constexpr int PIXELS_PER_LDG = THREADS_PER_CTA / THREADS_PER_PIXEL;
    // The number of pixels computed per CTA stored in registers.
    constexpr int PIXELS_PER_CTA_IN_REGISTERS = PIXELS_PER_THREAD_IN_REGISTERS * PIXELS_PER_LDG;
    // The number of pixels computed per CTA stored in SMEM.
    constexpr int PIXELS_PER_CTA_IN_SMEM = PIXELS_PER_THREAD_IN_SMEM*PIXELS_PER_LDG;
        ...
        if (pixels_in_smem > 0) {
            ...
            for (int i = 0; i < PIXELS_PER_THREAD_IN_SMEM; ++i) {
                ...
                if (ELEMENTS_PER_LDG == 8 && THREADS_PER_PIXEL == 4) {
                    const bool is_valid_nhw_relu =
                        static_cast<unsigned int>(idx) < static_cast<unsigned int>(((params.nhw+31) & ~31));
                    if (is_valid_nhw_relu) {
                        if (lane_id < ELEMENTS_PER_LDG) {
                            relu_mask = gmem_relu_bitmask[(idx & ~0x1) + lane_id];
                        }
                    }
                } else {
                    if (is_pixel_valid_nhw) {
                        if (lane_id < ELEMENTS_PER_LDG) {
                            relu_mask = gmem_relu_bitmask[idx * RELU_MULT + lane_id];
                        }
                    }
                }
        ...
        for (int loop_i = 0; loop_i < OUTER_LOOPS; ++loop_i) {
            ...
            for (int i = 0; i < PIXELS_PER_THREAD_IN_REGISTERS; ++i) {
                ...
                if (ELEMENTS_PER_LDG == 8 && THREADS_PER_PIXEL == 4) {
                    ...
                    if (is_valid_nhw_relu) {
                        if (lane_id < ELEMENTS_PER_LDG) {
                            relu_mask[i] = gmem_relu_bitmask[(idx & ~0x1) + lane_id];
                        }
                    }
                } else {
                    if (is_valid_nhw) {
                        if (lane_id < ELEMENTS_PER_LDG) {
                            relu_mask[i] = gmem_relu_bitmask[idx * RELU_MULT + lane_id];
                        }
                    }
                }
            }
