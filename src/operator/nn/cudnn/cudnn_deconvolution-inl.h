...
/*!
 * \brief The Operator used to perform deconvolution using cuDNN kernels.
 */
...
class CuDNNDeconvolutionOp {
 ...
 public:
  CuDNNDeconvolutionOp() : back_bias_get_workspace_performed_(false) {
  ...
  void Init(const DeconvolutionParam& param,
  ...
  void Forward(const OpContext &ctx,
    ...
    bool perform_cuda_forward_bias = perform_forward_bias &&
                                     FeaturesLastLayout() &&
                                     dmlc::GetEnv("MXNET_DECONV_CUDA_FORWARD_BIAS", false);
  ...
  void Backward(const OpContext &ctx,
    ...
    CHECK_NE(req[kWeight], kWriteInplace);
    if (!param_.no_bias) {
      CHECK_NE(req[kBias], kWriteInplace);
    }
    CHECK_NE(req[kData], kWriteInplace);

    bool perform_backward_bias = !param_.no_bias && (req[kBias] != kNullOp);
    bool perform_cuda_backward_bias = perform_backward_bias &&
                                      FeaturesLastLayout() &&
                                      dmlc::GetEnv("MXNET_DECONV_CUDA_BACKWARD_BIAS", true);
    if (perform_cuda_backward_bias && !back_bias_get_workspace_performed_) {
      ...
      auto back_workspace_byte_bias = AddBiasGradWorkspaceSizeBytes(in_grad[kBias],
                                                                     ...
                                                                     req[kBias],
                                                                     output_features, ctx);
      backward_workspace_byte_ = std::max(backward_workspace_byte_,
    ...
    if (req[kWeight] != kNullOp) {
      ...
      CUDNN_CALL(cudnnConvolutionBackwardFilter(
          ...
          workspace.dptr_,
          workspace_size,
          req[deconv::kWeight] == kAddTo ? &beta_add : &beta,
          filter_desc_,
          gwmat_ptr));
    }
    ...
    if (req[kData] != kNullOp) {
        CUDNN_CALL(cudnnConvolutionForward(s->dnn_handle_,
                                           ...
                                           workspace.dptr_,
                                           workspace_size,
                                           req[deconv::kData] == kAddTo ? &beta_add : &beta,
                                           in_desc_,
                                           gdata_ptr));
    }
  }
 ...
 */
  ...
  void CuDNNAlgoSetter(const RunContext& rctx,
    ...
    if (add_to_weight_ && Features(out_shape[kOut]) >= cudnn_find_issue_max_features) {
