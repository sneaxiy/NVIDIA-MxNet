  ...
  MSHADOW_XINLINE static void Map(int i, index_t row_length, DType* out_data,
    ...
    if (clip_gradient >= 0.0f) {
      mom_data[i] = momentum * mom_data[i]
              - wd * weight_data[i]
              - mshadow_op::clip::Map(rescale_grad * grad, clip_gradient);
    } else {
      mom_data[i] = momentum * mom_data[i]
                  - wd * weight_data[i]
                  - rescale_grad * grad;
...
.set_attr<FCompute>("FCompute<gpu>", MultiMomUpdate<gpu, type_identity, 3, MultiSGDKernel>);
...
.set_attr<FCompute>("FCompute<gpu>", MultiMomUpdate<gpu, single_precision, 4, MultiSGDKernel>);
...
NNVM_REGISTER_OP(multi_nag_mom_update)
.set_attr<FCompute>("FCompute<gpu>", MultiMomUpdate<gpu, type_identity, 3, MultiNAGKernel>);

NNVM_REGISTER_OP(multi_mp_nag_mom_update)
.set_attr<FCompute>("FCompute<gpu>", MultiMomUpdate<gpu, single_precision, 4, MultiNAGKernel>);
