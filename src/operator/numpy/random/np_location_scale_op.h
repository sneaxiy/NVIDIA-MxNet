...
inline void LocationScaleReparamBackwardImpl(const OpContext& ctx,
  ...
  size_t workspace_size_l = ReduceWorkspaceSize(s, lgrad.shape_, req[0], ograd.shape_,
                                                lhs.shape_, rhs.shape_, sizeof(DType));
  size_t workspace_size_r = ReduceWorkspaceSize(s, rgrad.shape_, req[1], ograd.shape_,
                                                lhs.shape_, rhs.shape_, sizeof(DType));
...
inline void ScalarLocationScaleReparamBackwardImpl(const OpContext& ctx,
  ...
  size_t workspace_size = ReduceWorkspaceSize(s, igrad.shape_, req[0],
                                              ograd.shape_, sizeof(DType));
  Tensor<xpu, 1, char> workspace =
    ctx.requested[0].get_space_typed<xpu, 1, char>(Shape1(workspace_size), s);
  if constexpr (std::is_same<xpu, cpu>::value) {
    if (loc_is_tensor) {
