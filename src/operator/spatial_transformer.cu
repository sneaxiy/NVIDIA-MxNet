...
inline void BilinearSamplingForward(const Tensor<gpu, 4, DType> &output,
    ...
    CheckLaunchParam(grid_dim, threads_per_block, "spatial transformer forward");
...
inline void BilinearSamplingBackward(const Tensor<gpu, 4, DType> &input_grad,
  ...
  using namespace cuda;
  // Kernel has integer indexing
  CHECK_LE(output_grad.shape_.Size() / o_c, std::numeric_limits<int32_t>::max());
  const int num_blocks = (output_grad.shape_.Size() / o_c + kMaxThreadsPerBlock - 1)
                        / kMaxThreadsPerBlock;
  dim3 grid_dim(makeGrid(num_blocks));
  dim3 threads_per_block(kMaxThreadsPerBlock);
  CheckLaunchParam(grid_dim, threads_per_block, "spatial transformer backward");
