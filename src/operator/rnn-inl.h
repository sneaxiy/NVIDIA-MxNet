 ...
 private:
  inline void Init(const OpContext &ctx,
    ...
    if (!init_cudnn_) {
      ...
      // Only update the probability by passing in a null states ptr
      ...
      // 'rnn_math_prec' appears as 'dtype_with_fallback_' in upstream code

      // adopt pseudo-fp16 for all architectures
      cudnnDataType_t rnn_math_prec =
