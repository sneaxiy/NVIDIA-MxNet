...
NDArray::Chunk::~Chunk() {
  ...
  if (auto engine = engine_ref_.lock()) {
    engine->DeleteVariable([mem, skip_free, var = this->var](RunContext s) mutable {
#if MXNET_USE_CUDA
    ...
    Engine::Get()->PushAsync(
      [&](RunContext rctx,
        ...
        on_start();
        {
          ...
          bool has_writer = !sync_obj.writer_event.empty();
          for (auto ev : sync_obj.reader_events) {
            auto event = ev.event;
            if (has_writer) {
              auto w_ev = sync_obj.writer_event[0];
              if (w_ev.stream == ev.stream) {
                event = w_ev.pool_index > ev.pool_index ? w_ev.event : ev.event;
                has_writer = false;
              }
            }
          ...
          if (has_writer) {
           CUDA_CALL(cudaEventSynchronize(sync_obj.writer_event[0].event));
          }
        }
