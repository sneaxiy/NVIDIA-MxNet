...
int MXGetGPUSMArch(int dev, int* out) {
  API_BEGIN();
  *out = Context::GetGPUSMArch(dev);
  API_END();
}

int MXGetGPUAsyncEngineCount(int dev, int* out) {
  API_BEGIN();
  *out = Context::GetGPUAsyncEngineCount(dev);
  API_END();
}
...
int MXInitializeALM() {
  API_BEGIN();
  auto& alm = mxnet::common::ALMParams::getALMParams();
  alm.setTargets({{"Convolution", "NHWC"},
                  {"Deconvolution", "NHWC"}});
