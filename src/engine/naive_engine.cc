...
class NaiveEngine final : public Engine {
 public:
  ...
  void Push(OprHandle op, Context exec_ctx, int priority = 0, bool profiling = false) override {
    ...
    this->PushAsync([&](RunContext ctx, CallbackOnStart on_start,
 ...
 */
  void PushAsync(AsyncFn exec_fun,
    ...
    CallbackOnStart on_start = CreateOnStart(
        NaiveEngine::OnStart, &promise);
  ...
  void DeleteVariable(SyncFn delete_fn, Context exec_ctx, VarHandle var) override {
    ...
    this->PushAsync([delete_fn, naive_var](RunContext ctx,
