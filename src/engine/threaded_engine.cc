...
void ThreadedEngine::DeleteOperator(OprHandle op) {
  ...
  this->PushAsync([threaded_opr](RunContext,
...
void ThreadedEngine::DeleteVariable(SyncFn delete_fn,
  ...
  this->PushAsync([delete_fn, threaded_var](RunContext ctx,
                                            CallbackOnStart on_start,
                                            CallbackOnComplete on_complete) {
...
void ThreadedEngine::WaitForVar(VarHandle var) {
  ...
  this->PushAsync([this, &done](RunContext,
                                CallbackOnStart on_start,
                                CallbackOnComplete on_complete) {
      on_start();
      if (engine_info_) {
...
void ThreadedEngine::OnStartCPU(Engine *engine, void *opr_block,
  ...
  for (auto event : event_per_stream) {
    MSHADOW_CUDA_CALL(cudaEventSynchronize(event.second.event));
  }
}
...
void ThreadedEngine::OnStartGPU(Engine *engine, void *sync_info,
  ...
  for (auto* read_var : threaded_opr->const_vars) {
    ...
    for (auto& writer : sync_obj.writer_event) {
      if (writer.stream != worker_stream->stream_) {
        ...
        // the ongoing sync
        bool found = false;
        for (const auto& reader : sync_obj.reader_events) {
  ...
  for (auto* write_var : threaded_opr->mutable_vars) {
    ...
    if (!sync_obj.writer_event.empty()
        && worker_stream->stream_ != sync_obj.writer_event[0].stream) {
      AddEventHelper(&event_per_stream, sync_obj.writer_event[0]);
    }
  }
  for (auto event : event_per_stream) {
    MSHADOW_CUDA_CALL(cudaStreamWaitEvent(worker_stream->stream_, event.second.event));
  }
}
...
void ThreadedEngine::OnCompleteGPU(Engine *engine, void *sync_info,
  ...
  MSHADOW_CUDA_CALL(cudaEventRecord(event, worker_stream->stream_));
  for (auto* read_var : threaded_opr->const_vars) {
    auto &sync_obj = read_var->sync_object;
    std::lock_guard<std::mutex> l(sync_obj.mutex);
