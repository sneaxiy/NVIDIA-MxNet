...
from ..ndarray import (sgd_update, sgd_mom_update, adam_update, rmsprop_update, rmspropalex_update,
                       ...
                       multi_mp_sgd_mom_update, lars_multi_sgd_update,
                       lars_multi_sgd_mom_update, lars_multi_mp_sgd_update,
                       lars_multi_mp_sgd_mom_update, lamb_update_phase1, lamb_update_phase2,
                       mp_lamb_update_phase1, mp_lamb_update_phase2, multi_nag_mom_update,
                       multi_mp_nag_mom_update)
from ..ndarray.contrib import (multi_lamb_update, multi_mp_lamb_update)
from ..ndarray import sparse
from ..random import normal
from ..util import is_np_array
from ..engine import bulk as engine_bulk
...
class SGD(Optimizer):
    ...
    **lazy updates** are applied by::
    ...
    def __init__(self, momentum=0.0, lazy_update=True, **kwargs):
        ...
        self.bulk_size = max(1, int(os.getenv('MXNET_OPTIMIZER_MAX_BULK_SIZE', '1')))
    ...
    def _update_impl(self, indices, weights, grads, states, multi_precision=False):
        ...
        if aggregate:
            current_index = 0
            bulk_size = min(self.bulk_size, math.ceil(len(indices) / self.aggregate_num))
            with engine_bulk(bulk_size):
...
class LARS(Optimizer):
    ...
    def _update_impl(self, indices, weights, grads, states, multi_precision=False):
        ...
        if aggregate:
            # LARS SGD does not do momentum correction by default,
            # so need to modify the momentum if this option is set
            # to emulate. The correction term is current_lr / previous_lr.
            if self.momentum > 0:
            ...
            if nb_lars > 0:
                ...
                multi_lars(new_lrs[:nb_lars], w_sum_sq, g_sum_sq, new_wds[:nb_lars],
                           ...
                           out=new_lrs[:nb_lars])
            # Same as usual using LARS sgd functions
            sidx = 0
            ...
            if self.momentum > 0:
                ...
                # so need to modify the momentum if this option is not set
                # to emulate. The correction term is previous_lr / current_lr.
                kwargs['momentum'] = (self.momentum * (self.last_lr / self.cur_lr)) \
                                     if ((not self.momentum_correction) and self.cur_lr != 0) else \
                                     self.momentum
        ...
        if aggregate:
            ...
            with engine_bulk(bulk_size):
                while current_index < len(indices):
                    ...
                    if not multi_precision:
                        if self.momentum > 0:
                            multi_nag_mom_update(*_flatten_list(zip(weights[sidx:eidx],
                                                                    grads[sidx:eidx],
                                                                    states[sidx:eidx])),
                                                 out=weights[sidx:eidx],
                                                 num_weights=len(weights[sidx:eidx]),
                        ...
                        if self.momentum > 0:
                            multi_mp_nag_mom_update(*_flatten_list(zip(weights[sidx:eidx],
                                                                       grads[sidx:eidx],
                                                                       *zip(*states[sidx:eidx]))),
                                                    out=weights[sidx:eidx],
                                                    num_weights=len(weights[sidx:eidx]),
            ...
            for weight, grad, state, lr, wd in zip(weights, grads, states, lrs, wds):
                if not multi_precision:
                    if state is not None:
                        nag_mom_update(weight, grad, state, out=weight,
                                       lazy_update=self.lazy_update, lr=lr, wd=wd, **kwargs)
                    else:
                        sgd_update(weight, grad, out=weight, lazy_update=self.lazy_update,
                                   lr=lr, wd=wd, **kwargs)
