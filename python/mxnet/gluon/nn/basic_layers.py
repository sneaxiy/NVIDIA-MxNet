...
__all__ = ['Sequential', 'HybridSequential', 'Dense', 'Dropout', 'Embedding',
           'BatchNorm', 'BatchNormReLU', 'InstanceNorm', 'InstanceNormV2', 'LayerNorm', 'GroupNorm',
...
class Dense(HybridBlock):
    ...
    def __init__(self, units, activation=None, use_bias=True, flatten=True,
                 ...
                 in_units=0, cublas_algo_fwd_prec=None, **kwargs):
        ...
        with self.name_scope():
            ...
            if activation is not None:
                ...
                self.act = None
            self.cublas_algo_fwd_prec = cublas_algo_fwd_prec
    ...
    def hybrid_forward(self, F, x, weight, bias=None):
        ...
        act = fc(x, weight, bias, no_bias=bias is None, num_hidden=self._units,
                 flatten=self._flatten, cublas_algo_fwd_prec=self.cublas_algo_fwd_prec, name='fwd')
...
class _BatchNorm(HybridBlock):
    ...
    act_type : string, optional, default None
        Fused activation function to be applied
    xbuf_ptr : uint64, default 0
...
class InstanceNormV2(HybridBlock):
    r"""
    Applies instance normalization to the n-dimensional input array.
    This operator takes an n-dimensional input array where (n>2) and normalizes
    the input using the following formula:
    ...
    def __init__(self, axis=1, epsilon=1e-5, center=True, scale=False,
                 beta_initializer='zeros', gamma_initializer='ones',
                 in_channels=0, xbuf_group=1, act_type=None, xbuf_ptr=0, **kwargs):
        super(InstanceNormV2, self).__init__(**kwargs)
        self._kwargs = {'eps': epsilon, 'axis': axis,
                        'act_type': act_type, 'xbuf_group': xbuf_group, 'xbuf_ptr': xbuf_ptr}
        # 'center' and 'scale' are not parameters of the underlying object
        self._repr_kwargs = {**self._kwargs, 'center': center, 'scale': scale}
        self.gamma = self.params.get('gamma', grad_req='write' if scale else 'null',
                                     shape=(in_channels,), init=gamma_initializer,
                                     allow_deferred_init=True)
        self.beta = self.params.get('beta', grad_req='write' if center else 'null',
    ...
    def hybrid_forward(self, F, x, gamma, beta):
        return F.InstanceNormV2(x, gamma, beta,
                                name='fwd', **self._kwargs)
    ...
    def cast(self, dtype):
        if np.dtype(dtype).name == 'float16':
            dtype = 'float32'
        super(InstanceNormV2, self).cast(dtype)
