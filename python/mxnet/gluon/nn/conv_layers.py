...
__all__ = ['Conv1D', 'Conv2D', 'Conv3D',
           ...
           'ReflectionPad2D', 'SpatialParallelConv3D',
...
class _SpatialParallelHelper(object):
    class _DummyParam(HybridBlock):
        def __init__(self, **kwargs):
            super(type(self), self).__init__(**kwargs) # pylint: disable=bad-super-call
            with self.name_scope():
                self.p = self.params.get('dummy', shape=(1,), grad_req='null',
                                         allow_deferred_init=False,
                                         differentiable=False)
                # Remove the warning about dummy param being already initialized when
                # multiple spatial parallel convolutions are used
                from ... import initializer
                import types
                old_initialize = self.p.initialize
                self.p.initialize = types.MethodType(lambda self, init=None, ctx=None, \
                                                            default_init=initializer.Uniform(), \
                                                            force_reinit=False: \
                                                            old_initialize(init, ctx, \
                                                                           default_init, True),
                                                     self.p)
        def hybrid_forward(self, F, x):
    ...
    @staticmethod
    def init(num_gpus):
        """Initialize dummy variable to track dependencies
        between the convolutions and communicate
        the NCCL unique id
        """
        cls = _SpatialParallelHelper
        if not cls._init:
            cls.dummy = cls._DummyParam()
            cls._init = True
            import ctypes
...
class _SpatialParallelConv(HybridBlock):
    ...
    def __init__(self, num_gpus, channels, kernel_size, strides, padding,
        ...
        with self.name_scope():
            ...
            if kernel_size == (1, 1, 1):
                ...
                self._spatial = True
            if self._spatial:
                self.dummy_param = helper.dummy.params.get('dummy', grad_req='null',
                                                           differentiable=False)
                self.params._params[self.dummy_param.name] = self.dummy_param
            self.weight = self.params.get('weight', shape=wshapes[1],
                                          init=weight_initializer,
                                          allow_deferred_init=True)
            if use_bias:
    ...
    def hybrid_forward(self, F, x, weight, dummy_param=None, bias=None):
        if is_np_array():
            F = F.npx
        if self._spatial:
            if bias is None:
                act = getattr(F, self._op_name)(x, weight=weight, dummy=dummy_param, name='fwd',
                                                **self._kwargs)
            else:
                act = getattr(F, self._op_name)(x, weight=weight, dummy=dummy_param, bias=bias,
...
class SpatialParallelConv3D(_SpatialParallelConv):
    ...
    def __init__(self, num_gpus, channels, kernel_size,
        ...
        if isinstance(kernel_size, numeric_types):
            kernel_size = (kernel_size,)*3
        assert len(kernel_size) == 3, "kernel_size must be a number or a list of 3 ints"
        assert (kernel_size in ((3, 3, 3), (1, 1, 1))), "Only supports 1x1x1 and 3x3x3 convolution for now"
        assert kernel_size == (1, 1, 1) or padding == (1, 1, 1) or padding == 1, \
               "Only supports padding of 1 for now"
