...
def gpu_async_engine_count(device_id=0):
    """Query CUDA for the number of async copy engines provided by the given GPU
    ...
    Returns
    -------
    async copy engine count

    """
    async_engine_count = ctypes.c_int()
    dev_id = ctypes.c_int(device_id)
    check_call(_LIB.MXGetGPUAsyncEngineCount(dev_id, ctypes.byref(async_engine_count)))
