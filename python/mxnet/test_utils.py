...
def is_float(dtype):
    ...
    return dtype in (np.float16, np.float32, np.float64, 'float16', 'float32', 'float64')

def is_float16(dtype):
    """Check if data type is float16."""
    return dtype in (np.float16, 'float16')
...
def rand_ndarray(shape, stype='default', density=None, dtype=None, modifier_func=None,
    ...
    ctx = ctx if ctx else default_context()
    if stype == 'default':
        val = random_arrays(shape)
        if modifier_func is not None:
            val = assign_each(val, modifier_func)
        arr = mx.nd.array(val, dtype=dtype, ctx=ctx)
    else:
...
def assert_almost_equal(a, b, rtol=None, atol=None, names=('a', 'b'), equal_nan=False,
    ...
    ----------
                       Supported formats:
    a, b: on CPU/GPU: np.ndarray, mx.nd.array
          on CPU: string, bool, int, int16, int32, int64, float16, float32, float64
    ...
    if a_str or b_str:
        # Comparing instances at least one of which is a string
        if a_str == b_str: # both are strings?
            if a == b:
                return
            errMsg = ""
        else:
            errMsg = "'assert_almost_equal' cannot compare {} and {}".format(type(a), type(b))
...
def _parse_location(sym, location, ctx, dtype=default_dtype()):
    ...
    assert dtype == "asnumpy" or is_float(dtype)
...
def numeric_grad(executor, location, grad_nodes, aux_states=None, eps=1e-4,
    ...
    for k, v in location.items():
        ...
        if not (k in grad_nodes and v.dtype.kind == 'f'):
            continue
        ...
        if loc_src is not None:
            ...
            for i in range(len):
                # inplace update of diagonal only
            ...
            if use_approx_grad:
                ...
                # make sure the forward() and copy() are complete before overwriting the 'v' input below
                mx.nd.waitall()
            else:
            ...
            if use_approx_grad:
                ...
                # make sure the approx_gradient calc is complete before overwriting the 'v' input
                mx.nd.waitall()
            else:
...
def check_gradient(grad_req, fd_grad, sym_grad, orig_grad, rtol, atol, name, names, equal_nan=False):
    ...
    if grad_req == 'write':
        ...
        if isinstance(sym_grad, mx.nd.NDArray) and isinstance(orig_grad, np.ndarray):
...
def check_numeric_gradient(sym, location, aux_states=None, numeric_eps=None, rtol=None,
    ...
    location_npy = None if use_approx_grad else {k:v.asnumpy() for k, v in location.items()}
    ...
    if isinstance(sym, np_symbol):
        proj = proj.as_np_ndarray() # convert to np symbol for using element-wise multiplication
        out = sym * proj
        out = out.as_nd_ndarray()   # convert to classic symbol so that make_loss can be used
    else:
        out = sym * proj
    ...
    if use_approx_grad:
        ...
        location = location_npy

    numeric_gradients = numeric_grad(
        executor, location, grad_nodes, aux_states, use_approx_grad=use_approx_grad,
        eps=eps, use_forward_train=use_forward_train, dtype=dtype, loc_src=location_src)
...
def run_and_check_results(sym, executor, expected, rtol=1E-4, atol=None, equal_nan=False, backward_data=None):
    """Launch forward/backward execution and compare results with the expected ones.

    Parameters
    ----------
    sym : Symbol
        Symbol to run the test.
    executor : executor for forward or backward path.
    rtol : float, optional
        Relative tolerance used in result comparison
    atol : float or None, optional
        Absolute tolerance used in result comparison
    equal_nan : boolean, optional
        The flag determining how to treat NAN values in comparison
    backward_data : None or list of np.ndarray or dict of str to np.ndarray
    ...
    if backward_data is None:
        ...
        executor.backward(backward_data)
        direction = 'BACKWARD'

    outputs = executor.outputs
    for output_name, expect, output in zip(sym.list_outputs(), expected, outputs):
        assert_almost_equal(expect, output, rtol, atol,
                            ("EXPECTED_%s"%output_name, "%s_%s"%(direction, output_name)),
                            equal_nan=equal_nan)
    return outputs
...
def check_symbolic_forward(sym, location, expected, rtol=None, atol=None,
    ...
    return run_and_check_results(sym, executor, expected, rtol=rtol, atol=atol, equal_nan=equal_nan)
