...
def get_gpu_count():
    """Get the number of visible cuda GPUs."""
    size = ctypes.c_int()
    check_call(_LIB.MXGetGPUCount(ctypes.byref(size)))
    return size.value


def is_enough_gpus(gpus, use_all=False, do_exit=True):
    """Check if computer has enough GPUs to run test.
    Parameters
    ----------
    GPUs  : list OR number of gpus, which supposed to be assigned to run test
    use_all : when True, the number of existing gpus will be returned
    do_exit : when True, program will be terminated with corresponding error message

    Returns
    -------
    number of gpus, which actually could be used
    """
    import sys

    n_gpus = get_gpu_count()
    if isinstance(gpus, str):
        gpus = [i for i in gpus.split(',')]
    n_GPUs = len(gpus) if isinstance(gpus, list) else gpus
    if n_gpus >= n_GPUs:
        # TODO When GPUs is a list of indices, some additional checks need to be made
        # to make sure that mapping of GPUs into [0, ..., num_gpus-1] exists
        return n_GPUs

    if use_all:
        return n_gpus
    if do_exit:
        sys.exit("To run this test %d gpu's needed, but only %d found on this computer" % (n_GPUs, n_gpus))
    return -1


def get_gpu_sm_arch(gpu_dev_id):
    """Get the streaming multiprocessor arch of the cuda GPU."""
    sm_arch = ctypes.c_int()
    check_call(_LIB.MXGetGPUSMArch(gpu_dev_id))
    return sm_arch.value


def get_gpu_memory(gpu_dev_id):
    """Get the free and total global memory size (in bytes) of the cuda GPU."""
...
def get_cuda_compute_capability(ctx):
    ...
    max_supported_compute_capability = 86
    cc = min(cc, max_supported_compute_capability)
    return cc
