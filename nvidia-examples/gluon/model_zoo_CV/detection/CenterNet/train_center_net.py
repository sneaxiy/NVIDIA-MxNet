...
def validate(net, val_data, ctx, eval_metric, dtype, flip_test=False):
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    trainer = gluon.Trainer(
                net.collect_params(), 'adam', optimizer_params,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_data):
            ...
            with autograd.record():
                ...
                for x, heatmap_target, wh_target, wh_mask, center_reg_target, center_reg_mask in zip(*split_data):
                    heatmap_pred, wh_pred, center_reg_pred = net(x.astype(args.dtype, copy=False))
                    wh_preds.append(wh_pred)
                    center_reg_preds.append(center_reg_pred)
                    wh_losses.append(wh_loss(wh_pred.astype('float32', copy=False), wh_target, wh_mask))
                    center_reg_losses.append(center_reg_loss(center_reg_pred.astype('float32', copy=False),
                                                             center_reg_target, center_reg_mask))
                    heatmap_losses.append(heatmap_loss(heatmap_pred.astype('float32', copy=False), heatmap_target))
        ...
        if (epoch % args.val_interval == 0) or (args.save_interval and epoch % args.save_interval == 0) or (epoch == args.epochs - 1):
            ...
            map_name, mean_ap = validate(net, val_data, ctx, eval_metric, args.dtype, flip_test=args.flip_validation)
