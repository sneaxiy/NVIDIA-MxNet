...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    if args.horovod:
        ...
        trainer = gluon.Trainer(
            net.collect_params(), 'sgd', optimizer_params,
        ...
        for i, batch in enumerate(train_data):
            ...
            with autograd.record():
                for ix, x in enumerate(data):
                    x = x.astype(args.dtype, copy=False)
                    gt_box = gt_boxes[ix].astype(args.dtype, copy=False)
                    fixed_target = [ft[ix].astype(args.dtype, copy=False) for ft in fixed_targets]
                    obj_loss, center_loss, scale_loss, cls_loss = net(x, gt_box, *fixed_target)
