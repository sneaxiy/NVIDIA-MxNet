...
def get_dataloader(net, train_dataset, val_dataset, data_shape, dtype, batch_size, num_workers, ctx):
    """Get dataloader."""
    width, height = data_shape, data_shape
    # use fake data to generate fixed anchors for target generation
    with autograd.train_mode():
        x = mx.nd.zeros((1, 3, height, width), ctx)
        _, _, anchors = net(x.astype(dtype, copy=False))
    anchors = anchors.as_in_context(mx.cpu()).astype('float32')
...
def validate(net, dtype, val_data, ctx, eval_metric):
    ...
    for batch in val_data:
        ...
        for x, y in zip(data, label):
            ...
            ids, scores, bboxes = net(x.astype(dtype, copy=False))
            det_ids.append(ids)
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    if args.horovod:
        ...
        trainer = hvd.DistributedTrainer(net.collect_params(), 'sgd', optimizer_params)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_data):
            ...
            with autograd.record():
                ...
                box_preds = []
                for x in data:
                    cls_pred, box_pred, _ = net(x.astype(args.dtype, copy=False))
                    cls_preds.append(cls_pred.astype('float32', copy=False))
                    box_preds.append(box_pred.astype('float32', copy=False))
        ...
        if (not args.horovod or hvd.rank() == 0):
            ...
            if (epoch % args.val_interval == 0) or (args.save_interval and epoch % args.save_interval == 0):
                ...
                if args.dtype != 'float16':  # For some reason validation is not working for float16
                    map_name, mean_ap = validate(net, args.dtype, val_data, ctx, eval_metric)
...
if __name__ == '__main__':
    ...
    if args.dali:
        ...
        train_data, val_data = get_dali_dataloader(
            async_net, train_dataset, val_dataset, args.data_shape, args.dtype, args.batch_size, args.num_workers,
            devices, ctx[0], args.horovod)
    else:
        train_dataset, val_dataset, eval_metric = get_dataset(args.dataset, args)
        batch_size = (args.batch_size // hvd.size()) if args.horovod else args.batch_size
        train_data, val_data = get_dataloader(async_net, train_dataset, val_dataset, args.data_shape, args.dtype,
