...
class Trainer(object):
    def __init__(self, args, logger):
        ...
        if TRAIN_DATA_DIR is None:
            if args.dataset=='ade20k':
                ...
                data_set = 'mhp/LV-MHP-v1'
            _TARGET_DIR = os.path.expanduser('~/.mxnet/datasets/' + data_set)
        else:
            _TARGET_DIR = TRAIN_DATA_DIR
            if args.dataset=='mhpv1' and not os.path.exists(_TARGET_DIR + '/train_list.txt'):
                _TARGET_DIR += '/LV-MHP-v1'

        trainset = get_segmentation_dataset(args.dataset, root=_TARGET_DIR,
                                            split=args.train_split,
                                            mode='train',
                                            **data_kwargs)
        valset = get_segmentation_dataset(args.dataset, root=_TARGET_DIR,
                                          split='val',
                                          mode='val',
                                          **data_kwargs)
        ...
        self.optimizer = gluon.Trainer(self.net.module.collect_params(), args.optimizer, optimizer_params,
    ...
    def training(self, epoch):
        ...
        self.logger.info('Epoch {}: Training cost: {:.3f} '.format(epoch, time.time() - start))
        # save every epoch
        if self.args.no_val:
