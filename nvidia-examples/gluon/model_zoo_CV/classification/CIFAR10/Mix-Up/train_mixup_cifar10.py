...
def main():
    ...
    num_gpus = is_enough_gpus(opt.num_gpus)
    ...
    def train(epochs, ctx, use_amp):
        ...
        trainer = gluon.Trainer(net.collect_params(), optimizer, optimizer_params,
                                update_on_kvstore=(False if use_amp else None))
        if use_amp:
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(train_data):
                ...
                output_softmax = [nd.SoftmaxActivation(out.astype('float32', copy=False)) for out in output]
            ...
            logging.info('[Epoch %d] train=%f val=%f loss=%f time: %f' %
                (epoch, acc, val_acc, train_loss, epoch_time))
        ...
        logging.info("Train total time: %f" % total_time)

    if opt.mode == 'hybrid':
        net.hybridize()
    train(opt.num_epochs, context, opt.amp)
