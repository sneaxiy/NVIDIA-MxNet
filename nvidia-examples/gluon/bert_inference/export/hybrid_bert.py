...
from gluonnlp.model.transformer import BaseTransformerEncoderCell, BasePositionwiseFFN
...
def fast_init(self, base_cell, query_units, key_units, value_units, num_heads, use_bias=True,
    ...
    with self.name_scope():
        ...
        self.proj_in = nn.Dense(units=self._proj_in_units * 3, use_bias=self._use_bias,
                                ...
                                prefix='proj_in', cublas_algo_fwd_prec=cublas_gemms_compute_type)
...
def fast_forward(self, F, query, key, value, mask=None):
    ...
    output = F.contrib.interleaved_matmul_selfatt_valatt(qkv_proj, att_weights, heads=self._num_heads)
    return output, att_weights

def BaseTransformerEncoderCell_init(self, attention_cell='multi_head', units=128,
    ...
    with self.name_scope():
        ...
        self.proj = nn.Dense(units=units, flatten=False,
                            ...
                            prefix='proj_', cublas_algo_fwd_prec=cublas_gemms_compute_type)

        self.proj_bias_alone = self.params.get('proj_bias_alone', allow_deferred_init=True)
...
def BaseTransformerEncoderCell_forward(self, F, inputs, mask=None, proj_bias_alone=None):
    outputs, attention_weights =\
        self.attention_cell(inputs, inputs, inputs, mask)
    outputs = self.proj(outputs)
    outputs =  outputs + F.broadcast_like(proj_bias_alone,outputs)
    if self._dropout:
...
def ffw_init(self, units=512, hidden_size=2048, dropout=0.0, use_residual=True,
    ...
    self.ffn1_bias_alone = self.params.get('ffn_1_bias_alone', allow_deferred_init=True)
    self.ffn2_bias_alone = self.params.get('ffn_2_bias_alone', allow_deferred_init=True)

    with self.name_scope():
        self.ffn_1 = nn.Dense(units=hidden_size, flatten=False,
                                weight_initializer=weight_initializer,
                                use_bias=False,
                                prefix='ffn_1_', cublas_algo_fwd_prec=cublas_gemms_compute_type)
        self.activation = self._get_activation(activation) if activation else None
        self.ffn_2 = nn.Dense(units=units, flatten=False,
                                weight_initializer=weight_initializer,
                                use_bias=False,
                                prefix='ffn_2_', cublas_algo_fwd_prec=cublas_gemms_compute_type)
        if dropout:
            self.dropout_layer = nn.Dropout(rate=dropout)
        self.layer_norm = _get_layer_norm(use_bert_layer_norm, units,
                                            layer_norm_eps=layer_norm_eps)
def ffw_forward(self, F, inputs, ffn1_bias_alone, ffn2_bias_alone):
        outputs = self.ffn_1(inputs)
        outputs =  outputs + F.broadcast_like(ffn1_bias_alone,outputs)
        if self.activation:
            outputs = self.activation(outputs)
        if self._dropout and self._ffn1_dropout:
            outputs = self.dropout_layer(outputs)
        outputs = self.ffn_2(outputs)
        outputs =  outputs + F.broadcast_like(ffn2_bias_alone,outputs)
        if self._dropout:
            outputs = self.dropout_layer(outputs)
        if self._use_residual:
            outputs = outputs + inputs
...
def init_fast_multiheadattn_and_softmax(gemms_compute_type):
    global cublas_gemms_compute_type
    cublas_gemms_compute_type = gemms_compute_type
...
def detach_addbias_and_set_gemms_compute_type(gemms_compute_type):
    global cublas_gemms_compute_type
    cublas_gemms_compute_type = gemms_compute_type
    nlp.model.transformer.BaseTransformerEncoderCell.__init__ = BaseTransformerEncoderCell_init
    nlp.model.transformer.BaseTransformerEncoderCell.hybrid_forward = BaseTransformerEncoderCell_forward
    nlp.model.transformer.BasePositionwiseFFN.__init__ = ffw_init
    nlp.model.transformer.BasePositionwiseFFN.hybrid_forward = ffw_forward
