...
parser.add_argument('--activate_AMP',
                    action='store_true',
                    help='Whether to activate Automac Mixed Precision')

parser.add_argument('--use_FP16',
                    action='store_true',
                    help='Whether to use FP16 (If not AMP). By default FP32')
...
net = hybrid_bert.HybridBERTForQA(bert=bert)
if model_parameters:
    # load complete BertForQA parameters
    #@Moises castType set to None for now, not working otherwise
    #nlp.utils.load_parameters(net, model_parameters, ctx=ctx, cast_dtype=True)
    nlp.utils.load_parameters(net, model_parameters, ctx=ctx, cast_dtype=None)
...
sym, arg_params, aux_params = mx.model.load_checkpoint('bert_base_FP16_'+str(max_seq_length), 0)
if args.use_FP16:
    executor = sym.simple_bind(ctx=mx.gpu(0), data0=(test_batch_size,max_seq_length), data1=(test_batch_size,max_seq_length), data2=(test_batch_size,), type_dict={'data0' : 'float32', 'data1' : 'float32', 'data2' : 'float16'})
else:
    executor = sym.simple_bind(ctx=mx.gpu(0), data0=(test_batch_size,max_seq_length), data1=(test_batch_size,max_seq_length), data2=(test_batch_size,), type_dict={'data0' : 'float32', 'data1' : 'float32', 'data2' : 'float32'})
...
def evaluate():
    ...
    inputs_fp32 = mx.nd.zeros((test_batch_size,max_seq_length))
    token_types_fp32 = mx.nd.zeros((test_batch_size,max_seq_length))
    if args.use_FP16:
        valid_length_fp16 = mx.nd.zeros((test_batch_size,), dtype='float16')
    else:
        valid_length_fp32 = mx.nd.zeros((test_batch_size,), dtype='float32')
    ...
    for data in dev_dataloader:
        ...
        if valid_length.shape[0]!=test_batch_size:
            break #last batch size may not match the binding size 

        mx.nd.cast(inputs, dtype='float32', out=inputs_fp32)
        mx.nd.cast(token_types, dtype='float32', out=token_types_fp32)
        if args.use_FP16:
            mx.nd.cast(valid_length, dtype='float16', out=valid_length_fp16)
        else:
            mx.nd.cast(valid_length, dtype='float32', out=valid_length_fp32)

        if args.use_FP16:
            out= executor.forward(data0=inputs_fp32,
                                  data1=token_types_fp32,
                                  data2=valid_length_fp16) 
        else:
            out = net(inputs.astype('float32').as_in_context(ctx),
                    token_types.astype('float32').as_in_context(ctx),
                    valid_length.astype('float32').as_in_context(ctx))

        output = mx.nd.split(out[0], axis=2, num_outputs=2)
...
if __name__ == '__main__':
    if args.use_FP16 and args.activate_AMP:
        raise ValueError("use_FP16 & activate_AMP cannot be active simultaneously")
    if args.activate_AMP:
        amp.init()
    if not only_predict:
        train()
        evaluate()
