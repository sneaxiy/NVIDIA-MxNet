...
from utils import report_train_time, get_model

nlp.utils.check_version('0.9', warning_only=True)

def log_file_name(args):
    return 'log_{0}.txt'.format(args)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='The name of the task to fine-tune. Choices include MRPC, QQP, '
             'QNLI, RTE, STS-B, CoLA, MNLI, WNLI, SST, XNLI, LCQMC, ChnSentiCorp.')
    parser.add_argument(
        '--bert_model',
        type=str,
    ...
    fh = logging.FileHandler(log_file_name(args.task_name))
    ...
    best_model = not args.no_best_model
    ...
    bert, vocabulary = get_model(model_name, dataset, get_pretrained, ctx, not use_roberta, logging)
...
def train(metric):
    """Training function."""
    def get_param_file_name(task_name, epoch_id, remove_prev=True):
        if remove_prev and epoch_id > 1:
            params_saved = get_param_file_name(task_name, epoch_id - 1, remove_prev=True)
            if os.path.exists(params_saved):
                os.remove(params_saved)

        ckpt_name = 'model_bert_{0}_{1}.params'.format(task_name, epoch_id)
        return os.path.join(output_dir, ckpt_name)
    ...
    for epoch_id in range(epoch_number):
        ...
        if not only_inference:
            # save params
            if best_model:
                params_saved = get_param_file_name(task_name, epoch_id, remove_prev=False)
    ...
    if not only_inference and best_model:
...
if __name__ == '__main__':
    ...
    report_train_time(start, log_file_name, args.task_name)
