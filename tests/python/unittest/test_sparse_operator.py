...
def my_gamma(x):
    # gamma function is discontinuous at negative integers and 0
    mask = np.logical_and(x == np.round(x), x < 0)
    ret = scipy_special.gamma(x)
    ret[mask] = np.nan
    return ret
...
def get_result_type_2(call, dflt_stype):
    ...
    if call is None or dflt_stype is None or dflt_stype == 'default':
        return 'default'
...
def get_result_type_3(call, dflt_stype):
    ...
    for moon in [zero]:
        for outer in [zero]:
            for inner in [zero]:
                res_lst = call(moon, outer, inner)
                for res in res_lst:
                    result  = do_normalize(res)
                    if not almost_equal(result, zero, equal_nan=True):
                        return dflt_stype
...
def initiate_nd(shape, stype, density, modifier_func,
                force_lr_overlap, shuffle_csr_indices, dtype=default_dtype(), rand_array = True):
    if stype == 'default':
        if rand_array:
...
def scipy_psi_func():
    ...
    return np.vectorize(lambda x: np.inf if float(x).is_integer() and x <= 0 else scipy_special.psi(x))
...
LHS_STYPE = 'lhs_stype'
RHS_STYPE = 'rhs_stype'
LHS_GRAD_STYPE = 'lhs_grad_stype'
RHS_GRAD_STYPE = 'rhs_grad_stype'
LHS_DENSITY = 'lhs_density'
RHS_DENSITY = 'rhs_density'
LHS_MODIFIER = 'lhs_modifier'
RHS_MODIFIER = 'rhs_modifier'
MODIFIER = 'modifier'
RHS_IS_SCALAR = 'rhs_is_scalar'
OGRAD_DENSITY = 'ograd_density'
FORCE_LR_OVERLAP = 'force_lr_overlap'
FORCE_GRAD_OVERLAP = 'force_grad_overlap'
SHUFFLE_CSR_IDX = 'shuffle_src_idx'
RESULT_STYPE = 'result_stype'
SKIP_GRAD_CHECK = 'skip_grad_check'
VERBOSE = 'verbose'
...
def check_operator(OpDescr, shape, OptParam = None, print_test_names = False):
    # Unpack data which could be represented in different formats
    if isinstance(OpDescr, list):
        OptDict = OpDescr[1]
        OpDescr = OpDescr[0]
    else:
        OptDict = None
    ...
    func_name = None
    len_op_descr = len(OpDescr)
    assert(len_op_descr in (3, 4))
    if len_op_descr == 4:
        func_name, forward_mxnet_call, forward_numpy_call, backward_numpy_call = OpDescr
    else:
        forward_mxnet_call, forward_numpy_call, backward_numpy_call = OpDescr

    if func_name is None:
        func_name = forward_mxnet_call.__name__

    if print_test_names:
        print("Function: \"{}\"".format(func_name))

    assert(callable(forward_mxnet_call) and callable(forward_numpy_call))
    assert(backward_numpy_call is None or callable(backward_numpy_call)) # When defined, it should be a function

    # Setting optional parameters
    dtype = OptParam[DTYPE] if OptParam and DTYPE in OptParam else default_dtype()
    lhs_stype = OptParam[LHS_STYPE] if OptParam and LHS_STYPE in OptParam else 'default'
    rhs_stype = OptParam[RHS_STYPE] if OptParam and RHS_STYPE in OptParam else 'default'
    lhs_grad_stype = OptParam[LHS_GRAD_STYPE] if OptParam and LHS_GRAD_STYPE in OptParam else None
    rhs_grad_stype = OptParam[RHS_GRAD_STYPE] if OptParam and RHS_GRAD_STYPE in OptParam else None
    lhs_density = OptParam[LHS_DENSITY] if OptParam and LHS_DENSITY in OptParam else 0.5
    rhs_density = OptParam[RHS_DENSITY] if OptParam and RHS_DENSITY in OptParam else 0.5
    force_lr_overlap = OptParam[FORCE_LR_OVERLAP] if OptParam and FORCE_LR_OVERLAP in OptParam else False
    force_grad_overlap = OptParam[FORCE_GRAD_OVERLAP] if OptParam and FORCE_GRAD_OVERLAP in OptParam else False
    ograd_density = OptParam[OGRAD_DENSITY] if OptParam and OGRAD_DENSITY in OptParam else 0.0
    shuffle_csr_indices = OptParam[SHUFFLE_CSR_IDX] if OptParam and SHUFFLE_CSR_IDX in OptParam else True
    expected_result_storage_type = OptParam[RESULT_STYPE] if OptParam and RESULT_STYPE in OptParam else None
    rhs_arg = OptParam[RHS_ARG] if OptParam and RHS_ARG in OptParam else True
    rhs_is_scalar = OptParam[RHS_IS_SCALAR] if OptParam and RHS_IS_SCALAR in OptParam else False

    # Function which modify randomly assigned lh/rh side parameters to make them acceptable by tested operator
    lhs_modifier_func = OptParam[LHS_MODIFIER] if OptParam and LHS_MODIFIER in OptParam else None
    rhs_modifier_func = OptParam[RHS_MODIFIER] if OptParam and RHS_MODIFIER in OptParam else None
    modifier_func = OptParam[MODIFIER] if OptParam and MODIFIER in OptParam else None
    skip_gradient_check = OptParam[SKIP_GRAD_CHECK] if OptParam and SKIP_GRAD_CHECK in OptParam else False
    verbose = OptParam[VERBOSE] if OptParam and VERBOSE in OptParam else False

    # Some default values used in tensor comparisons
    rtol_symb = OptParam[RTOL_SYMB] if OptParam and RTOL_SYMB in OptParam else 1e-3
    atol_symb = OptParam[ATOL_SYMB] if OptParam and ATOL_SYMB in OptParam else 1e-20
    rtol_grad = OptParam[RTOL_GRAD] if OptParam and RTOL_GRAD in OptParam else 1e-2
    atol_grad = OptParam[ATOL_GRAD] if OptParam and ATOL_GRAD in OptParam else None

    if OptDict: # Some optional parameters could be passed/redefine in a dictionary
        if SKIP_GRAD_CHECK in OptDict: skip_gradient_check = OptDict[SKIP_GRAD_CHECK]
        if RTOL_SYMB in OptDict: rtol_symb = OptDict[RTOL_SYMB]
        if ATOL_SYMB in OptDict: atol_symb = OptDict[ATOL_SYMB]
        if RTOL_GRAD in OptDict: rtol_grad = OptDict[RTOL_GRAD]
        if ATOL_GRAD in OptDict: atol_grad = OptDict[ATOL_GRAD]
    elif is_float16(dtype):
        atol_grad = 1e-2
        atol_symb = 1e-2

    if modifier_func and lhs_modifier_func is None and rhs_modifier_func is None:
        lhs_modifier_func = rhs_modifier_func  = modifier_func

    assert(lhs_modifier_func is None or callable(lhs_modifier_func))
    assert(rhs_modifier_func is None or callable(rhs_modifier_func))
    ...
    if verbose is True:
        print("testing: {}  lhs={}, rhs={}, lhs_grad_stype={}, rhs_grad_stype={}"
              .format(func_name, lhs_stype, rhs_stype, lhs_grad_stype, rhs_grad_stype))
    ...
    lhs_nd = initiate_nd(shape, lhs_stype, lhs_density, lhs_modifier_func,
                         force_lr_overlap, shuffle_csr_indices, dtype=dtype)
    lhs_np = lhs_nd.asnumpy()
    if verbose is True: print("lhs input: {}".format(lhs_np))

    if rhs_arg:
        if rhs_is_scalar:
            val = 10.
            tmp = random.uniform(-val, val)
            rhs_np = rhs = rhs_nd = float(np.array(tmp).astype(dtype)[()])
        else:
            rhs = mx.symbol.Variable('rhs', stype=rhs_stype)
            rhs_nd = initiate_nd(shape, rhs_stype, rhs_density, rhs_modifier_func,
                             force_lr_overlap, shuffle_csr_indices, dtype=dtype)
            rhs_np = rhs_nd.asnumpy()

        if verbose is True: print("rhs input: {}".format(rhs_np))

        out_np = forward_numpy_call(lhs_np, rhs_np)
        out = forward_mxnet_call(lhs, rhs)
        args = location = {'lhs': lhs_nd, 'rhs': rhs_nd}
    else:
        rhs_is_scalar = True
        out_np = forward_numpy_call(lhs_np)
        out = forward_mxnet_call(lhs)
        args = list()
        args.append(lhs_nd)
    ...
    if rhs_is_scalar:
        ...
        if backward_numpy_call:
            arr_grad = initiate_nd(shape, expected_result_storage_type, ograd_density, lambda x: 2,
                               force_grad_overlap, shuffle_csr_indices, dtype=dtype, rand_array=False)
            executor = out.bind(default_context(), args=args, args_grad=[arr_grad])
        else:
            executor = out.bind(default_context(), args=args)
        outputs = run_and_check_results(out, executor, [out_np], rtol=rtol_symb, atol=atol_symb, equal_nan=True)
    else:
        outputs = check_symbolic_forward(out, location, [out_np], dtype=dtype, rtol=rtol_symb, atol=atol_symb, equal_nan=True)
    ...
    grad_stypes = {'lhs' : lhs_grad_stype, 'rhs' : rhs_grad_stype}
    if backward_numpy_call:
        ...
        if rhs_arg:
            ...
            igrads_result = check_symbolic_backward(out, location, [out_grad],
                                                    [ingrad_lhs_np, ingrad_rhs_np],
                                                    dtype=dtype, grad_stypes=grad_stypes,
                                                    atol=atol_grad, equal_nan=True)
            ...
            expected = backward_numpy_call(lhs_np)
            executor.backward(out_grad)

            # When dtype is fp16, expected.dtype is fp32
            # Therefore, for the correct comparison, we must convert
            if is_float16(dtype):
                expected = expected.astype(dtype)

            input_grad = out_grad_np * expected
            if is_float16(dtype):
                # Hiding a possible overflow error for float16
                nan_mask = np.isnan(input_grad)
                if nan_mask.any():
                    nan_mask.shape = -1
                    input_grad.flat[nan_mask] = arr_grad.asnumpy().flat[nan_mask]

            def has_nans(x):
                return np.isnan(np.min(x))
            # If out_grad_np[i] == 0 and expected[i] == inf, then input_grad[i] will be nan.
            # Model may debatably produce 0 instead of nan, so avoid the check.
            if not has_nans(input_grad):
                assert_almost_equal(input_grad, arr_grad, rtol=rtol_grad, atol=atol_grad, equal_nan=True)

    if not (skip_gradient_check or is_float16(dtype)):
        # A smaller numerical eps helps avoid crossing discontinuities in the gradient function
        check_numeric_gradient(out, location, dtype=dtype,  grad_stype_dict=grad_stypes, numeric_eps=1e-4,
                               rtol=rtol_grad, atol=atol_grad, use_batch=True, use_approx_grad=True)

def deactivate_env_vars(env_var_list):
    # Deactivating MXNet operator's output
    env_var_dict = {}
...
def restore_env_vars(env_var_list, env_var_dict):
    # Restore previous OR default values of the environment variables
    for env_var in env_var_list:
        val = env_var_dict[env_var]
        if val is not None:
...
def test_elemwise_ops():
    def move_from_zero(x, atol=0.01):
        return x if abs(x) > atol else atol if x > 0 else -atol

    def add_op_list(lists, list_to_add):
        lists.append(list_to_add)
        return len(lists)

    def run_tests(op_lists, OptParam, idx_no_grad_check,
                  idx_modifier_lst, idx_rhs_scalar, idx_rhs_none, idx_unary_modifier_lst):
        tests_ok = True
        list_idx = 0
        for op_list in op_lists:
            list_idx += 1
            if list_idx in (idx_rhs_none, idx_unary_modifier_lst):
                OptParam.update({RHS_ARG : None})
            elif list_idx == idx_rhs_scalar:
                OptParam.update({RHS_IS_SCALAR : True})
            elif list_idx == idx_no_grad_check:
                OptParam.update({SKIP_GRAD_CHECK : True})

            if list_idx in (idx_modifier_lst, idx_unary_modifier_lst):
                for operation, modifier in op_list:
                    Param = {**OptParam}
                    if isinstance(modifier, (list, tuple)):
                        if modifier[0]: Param.update({LHS_MODIFIER : modifier[0]})
                        if modifier[1]: Param.update({RHS_MODIFIER : modifier[1]})
                    else:
                        Param.update({MODIFIER : modifier})
                    check_operator(operation, shape, Param, print_test_names=True)
            else:
                for operation in op_list:
                    check_operator(operation, shape, OptParam, print_test_names=True)

            if list_idx == idx_no_grad_check:
                OptParam.pop(SKIP_GRAD_CHECK)
            elif list_idx == idx_rhs_scalar:
                OptParam.pop(RHS_IS_SCALAR)
            elif list_idx in (idx_rhs_none, idx_unary_modifier_lst):
                OptParam.pop(RHS_ARG)
        return tests_ok

    env_var_list = ['MXNET_SUBGRAPH_VERBOSE', 'MXNET_STORAGE_FALLBACK_LOG_VERBOSE']
    env_var_dict = deactivate_env_vars(env_var_list)

    sparse = mx.sym.sparse
    intern = mx.sym._internal
    bcast = mx.sym

    # Some specific operator's parameters (rtol, atol, ... etc) could be passed in the dictionary
    # (see intern._power,  intern._rdiv_scalar)
    # List of operations with no data modifiers
    ops_without_modifiers = \
    [(intern._plus,             lambda l, r: l + r, lambda outg, l, r: (outg, outg)),
     (intern._minus,            lambda l, r: l - r, lambda outg, l, r: (outg, -outg)),
     (intern._mul,              lambda l, r: l * r, lambda outg, l, r: (outg * r, outg * l)),
     (sparse.elemwise_add,      lambda l, r: l + r, lambda outg, l, r: (outg, outg)),
     (sparse.elemwise_sub,      lambda l, r: l - r, lambda outg, l, r: (outg, -outg)),
     (sparse.elemwise_mul,      lambda l, r: l * r, lambda outg, l, r: (outg * r, outg * l)),
     [(intern._maximum,         np.maximum,         lambda outg, l, r: (outg * ge(l, r), outg * lt(l, r))), {ATOL_GRAD : 4e-1}],
     [(intern._minimum,         np.minimum,         lambda outg, l, r: (outg * le(l, r), outg * gt(l, r))), {ATOL_GRAD : 4e-1}],
     (bcast.broadcast_add,      lambda l, r: l + r, lambda outg, l, r: (outg, outg)),
     (bcast.broadcast_sub,      lambda l, r: l - r, lambda outg, l, r: (outg, -outg)),
     [(bcast.broadcast_maximum, np.maximum,         lambda outg, l, r: (outg * ge(l, r), outg * lt(l, r))), {ATOL_GRAD : 3e-1}],
     [(bcast.broadcast_minimum, np.minimum,         lambda outg, l, r: (outg * le(l, r), outg * gt(l, r))), {ATOL_GRAD : 3e-1}],
     (bcast.broadcast_logical_and,  np.logical_and, None),
    ]

    # List of operations which requires some data modifiers
    # NOTE:  for mx.sym._internal._power:
    #          a) for now backward testing is excluded because of NaN problem
    #          b) for now numerical gradient testing is excluded because of NaN problem for MXNET_TEST_SEED=2042993855
    #          c) relatively large atol_grad needed for numerical gradient testing with MXNET_TEST_SEED=897556624
    ops_with_modifiers = \
    [([(intern._power,           np.power,           None), {ATOL_SYMB : 1e-3, ATOL_GRAD : 2., SKIP_GRAD_CHECK : True}], (abs)),
     #([(intern._power,           np.power,           lambda outg, l, r: (outg, outg * (l ** r))), {ATOL_GRAD : 1E-1}], (abs)),
     ((intern._div,              lambda l, r: l / r, lambda outg, l, r: (outg * (1/r), outg * (-l/(r*r)))), (None, move_from_zero)),
     ((sparse.elemwise_div,      lambda l, r: l / r, lambda outg, l, r: (outg * (1/r), outg * (-l/(r*r)))), (None, move_from_zero)),
    ]

    # List of unary operations which requires some data modifiers
    ops_unary_with_modifiers = \
    [((sparse.sqrt,      np.sqrt,     lambda x: 1.0/(2.0 * np.sqrt(x))), abs),
     ([(sparse.rsqrt,    lambda x: 1 / np.sqrt(x),  lambda x: -(1.0 / (2.0 * x * np.sqrt(x)))), {ATOL_SYMB : 1e-1}], abs),
     ((sparse.arcsin,    np.arcsin,   lambda x: 1. / (1. - x ** 2) ** (1. / 2.)), lambda x: divmod(x, 1)[1]),
     ((sparse.arccos,    np.arccos,   lambda x: -1. / (1. - x ** 2.) ** (1. / 2.)), lambda x: divmod(x, 1)[1]),
     ((sparse.arccosh,   np.arccosh,  lambda x: 1./(x**2 - 1.)**(1./2.)), lambda x: 1. + abs(x)),
     ((sparse.arctanh,   np.arctanh,  lambda x: -1./(x**2 - 1.)), lambda x: divmod(x, 1)[1]),
     ((sparse.log1p,     np.log1p,    lambda x: 1. / (1.0 + x)), lambda x: abs(x) - 1.),
     ((sparse.log10,     np.log10,    lambda x: 1. / (x * np.log(10.))), abs),
     ((sparse.log2,      np.log2,     lambda x: 1. / (x * np.log(2.))), abs),
    ]

    # List of operators with no gradient check
    ops_with_no_grad_check = \
    [(intern._hypot,       np.hypot,           lambda outg, l, r: (
        outg * assign_each2(l, r, lambda a, b: a/np.sqrt(a * a + b * b)),
        outg * assign_each2(l, r, lambda a, b: b/np.sqrt(a * a + b * b))))
    ]

    # List of operations with rhs operator as a scalar
    ops_rhs_scalar = \
    [(intern._plus_scalar,          lambda l, r: l + r, None),
     (intern._minus_scalar,         lambda l, r: l - r, None),
     (intern._rminus_scalar,        lambda l, r: -(l - r), None),
     (intern._mul_scalar,           lambda l, r: l * r, None),
     (intern._div_scalar,           lambda l, r: l / r, None),
     [(intern._rdiv_scalar,         lambda l, r: 1. / (l / r), None), {RTOL_SYMB : 2e-3}],
     (intern._mod_scalar,           lambda l, r: divmod(l, r)[1], None),
     (intern._rmod_scalar,          lambda l, r: divmod(r, l)[1], None),
    ]

    scipy_psi = scipy_psi_func()

    # List of operations with rhs operator as a scalar
    ops_rhs_none = \
    [(sparse.negative,  np.negative, None),
     (sparse.square,    np.square,   lambda x: 2 * x),
     (sparse.cbrt,      np.cbrt,     lambda x: 1.0/(3.0 * np.cbrt(x) * np.cbrt(x))),
     [(sparse.rint,     np.rint,     None), {ATOL_SYMB : 1}],
     (sparse.fix,       np.fix,      None),
     (sparse.floor,     np.floor,    None),
     (sparse.ceil,      np.ceil,     None),
     [(sparse.round,    np.round,    None), {ATOL_SYMB : 1}],
     (sparse.trunc,     np.trunc,    None),
     (sparse.sign,      np.sign,     lambda x: np.zeros(x.shape)),
     (sparse.expm1,     np.expm1,    np.exp),
     (sparse.sin,       np.sin,      np.cos),
     (sparse.cos,       np.cos,      lambda x: -np.sin(x)),
     (sparse.tan,       np.tan,      lambda x: np.tan(x) ** 2 + 1),
     (sparse.arctan,    np.arctan,   lambda x: 1. / (x ** 2. + 1.)),
     (sparse.degrees,   np.degrees,  lambda x: assign_each(x, lambda a: 180./np.pi)),
     (sparse.radians,   np.radians,  lambda x: assign_each(x, lambda a: np.pi / 180.)),
     (sparse.sinh,      np.sinh,     np.cosh),
     (sparse.cosh,      np.cosh,     np.sinh),
     (sparse.tanh,      np.tanh,     lambda x: 1. - np.tanh(x) ** 2),
     (sparse.arcsinh,   np.arcsinh,  lambda x: 1./(x**2 + 1.)**(1./2.)),
     (sparse.abs,       np.abs,      lambda x: assign_each(x, function=util_sign)),
     [(sparse.gamma,    my_gamma,   lambda x: my_gamma(x) * scipy_psi(x)), {RTOL_SYMB : 5e-4, ATOL_SYMB : 5e-1}],
     (sparse.gammaln,   scipy_special.gammaln, scipy_psi),
    ]

    # Creating a list of operation lists:
    op_lists = []
    add_op_list(op_lists, ops_without_modifiers)
    idx_no_grad_check = add_op_list(op_lists, ops_with_no_grad_check)
    idx_modifier_lst = add_op_list(op_lists, ops_with_modifiers)
    idx_rhs_scalar = add_op_list(op_lists, ops_rhs_scalar)
    idx_rhs_none = add_op_list(op_lists, ops_rhs_none)
    idx_unary_modifier_lst = add_op_list(op_lists, ops_unary_with_modifiers)

    lhs_stype = rhs_stype = 'default'

    tests_ok = True
    dtypes = ['float16', 'float32', 'float64']
    OptParam = {LHS_STYPE : lhs_stype, RHS_STYPE : rhs_stype}
    for dtype in dtypes:
        shape = rand_shape_2d(5, 5)
        print("*** Testing functions for dtype: \"{}\" and shape: {} ***".format(dtype, shape))
        OptParam.update({DTYPE : dtype})
        if not run_tests(op_lists, OptParam, idx_no_grad_check,
                         idx_modifier_lst, idx_rhs_scalar, idx_rhs_none, idx_unary_modifier_lst):
            print ("FAILURE")
            tests_ok = False
            break

    restore_env_vars(env_var_list, env_var_dict)
    assert(tests_ok)
...
def test_elemwise_binary_ops():
    ...
    def check_elemwise_binary_ops(lhs_stype, rhs_stype, shape,
        ...
        OptParam = {LHS_STYPE : lhs_stype, RHS_STYPE : rhs_stype,
                    LHS_GRAD_STYPE : lhs_grad_stype, RHS_GRAD_STYPE : rhs_grad_stype,
                    LHS_DENSITY : lhs_density,  RHS_DENSITY : rhs_density,
                    OGRAD_DENSITY : ograd_density,
                    FORCE_LR_OVERLAP : force_lr_overlap,
                    FORCE_GRAD_OVERLAP : force_grad_overlap
                   }
        check_operator(("elemwise_add",
                        lambda l, r: mx.sym.sparse.elemwise_add(l, r),
                        lambda l, r: l + r,
                        lambda outg, l, r: (outg, outg)),
                        shape, OptParam)

        if ((lhs_stype == 'default' and (rhs_stype == 'row_sparse' or rhs_stype == 'csr')) or
        ...
        NewGradTypes = {LHS_GRAD_STYPE : elemwise_mul_lhs_grad_stype(lhs_stype, rhs_stype),
                        RHS_GRAD_STYPE : elemwise_mul_rhs_grad_stype(lhs_stype, rhs_stype),
                        RESULT_STYPE : elemwise_mul_stype(lhs_stype, rhs_stype)}
        check_operator(("elemwise_mul",
                        lambda l, r: mx.sym.sparse.elemwise_mul(l, r),
                        lambda l, r: l * r,
                        lambda outg, l, r: (outg * r, outg * l)),
                        shape, {**OptParam, **NewGradTypes})

        OptParam.update({SKIP_GRAD_CHECK : True})
        check_operator(("hypot",
...
def check_sparse_mathematical_core(name, stype,
    ...
    result_type_func = get_fw_bw_result_types if rhs_arg is None else \
                       get_fw_bw_result_types_with_scalar if is_scalar(rhs_arg) else \
                       get_fw_bw_result_types_2

    expected_result_type, expected_grad_result_type = \
        result_type_func(forward_numpy_call, stype, backward_numpy_call, temp_input_grad_stype)
...
def test_sparse_dot():
    ...
    def test_infer_forward_stype(lhs_shape, rhs_shape, lhs_density, rhs_density, trans_a, trans_b):
        ...
        lhs_nd = rand_ndarray(lhs_shape, 'default', density=lhs_density, modifier_func=scale_by(0.1))
        rhs_nd = rand_ndarray(rhs_shape, 'default', density=rhs_density, modifier_func=scale_by(0.1))
        ...
        for lhs_stype in all_stypes:
            for rhs_stype in all_stypes:
                for forward_stype in all_stypes:
                    ...
                    assert_almost_equal(out.tostype('default').asnumpy(), out_np, rtol=rtol, atol=atol)
                    ...
                    check_symbolic_forward(out, location, [out_np], rtol=rtol, atol=atol)
    def test_dot_csr(lhs_shape, rhs_shape, rhs_stype, trans_lhs, lhs_density, rhs_density):
        lhs_nd = rand_ndarray(lhs_shape, 'csr', density=lhs_density, shuffle_csr_indices=False, modifier_func=scale_by(0.1))
        lhs_dns = lhs_nd.tostype('default')
        rhs_nd = rand_ndarray(rhs_shape, rhs_stype, density=rhs_density, modifier_func=scale_by(0.1))
    ...
    def test_dot_dns_csr(lhs_shape, rhs_shape, lhs_density, rhs_density, trans_lhs=False, trans_rhs=False):
        lhs_nd = rand_ndarray(lhs_shape, stype='default', density=lhs_density, modifier_func=scale_by(0.1))
        rhs_nd = rand_ndarray(rhs_shape, stype='csr', density=rhs_density, modifier_func=scale_by(0.1))
