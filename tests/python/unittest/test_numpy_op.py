...
from common import assertRaises, with_seed, setup_module, teardown
import random
    ...
    for a_shape, b_shape, axes in tensor_shapes:
        ...
        for hybridize in [False, True]:
            ...
            for dtype in [_np.float32, _np.float64]:
                ...
                assert_almost_equal(a.grad.asnumpy(), np_backward[0], rtol=rtol, atol=atol)
                assert_almost_equal(b.grad.asnumpy(), np_backward[1], rtol=rtol, atol=atol)
                ...
                if (_np.prod(a_shape) > 0 and _np.prod(b_shape) > 0):
                    ...
                    check_numeric_gradient(mx_sym, [a.as_nd_ndarray(), b.as_nd_ndarray()],
                        use_approx_grad=False, rtol=1e-1, atol=1e-1, dtype = dtype)
...
def test_np_dot():
    ...
    for shape_a, shape_b in shapes:
        ...
        if (len(shape_a) > 0 and len(shape_b) > 0 and _np.prod(shape_a) > 0 and _np.prod(shape_b) > 0):
            check_numeric_gradient(mx_sym, {"a": a, "b": b},
                                   use_approx_grad=False, numeric_eps=eps, rtol=1e-2, atol=1e-3)
                ...
                for itype in ['float16', 'float32', 'float64', 'int8', 'int32', 'int64', 'bool']:
                    for dtype in ['float16', 'float32', 'float64', 'int8', 'int32', 'int64']:
                        ...
                        if itype == 'float32' and dtype == 'float32':
                            ...
                            check_numeric_gradient(mx_sym, [x.as_nd_ndarray()], use_approx_grad=False,
            ...
            for dtype in dtypes:
                for lhs_grad_req in ['write', 'add']:
                    for rhs_grad_req in ['write', 'add']:
                        ...
                        out.backward(o_grad)
                        assert_almost_equal(out, gt_out)
                        ...
                        assert_almost_equal(lhs_val.grad, gt_lhs_grad)
                        assert_almost_equal(rhs_val.grad, gt_rhs_grad)
...
def test_npx_batch_norm():
    ...
    def _test_batchnorm_impl(shape, fix_gamma, cudnn_off, output_mean_var,
        ...
        for _ in range(num_iters):
            ...
            # cudnn is used for the most common layouts: NCHW, NHWC, etc.
            def cudnn_employed(cudnn_off, axis, shape):
                ndim = len(shape)
                return (not cudnn_off) and ndim >= 2 and (axis == 1 or axis == ndim-1)

            # cudnn uses m-1 in the denominator of its sample variance calculation, not m
            m = _np.prod(shape) / shape[axis]
            sample_var_adjust = 1.0 if fix_gamma or not cudnn_employed(cudnn_off, axis, shape) \
                                else m / (m-1)
            running_var = running_var * momentum + \
                data_var_flat * sample_var_adjust * (1 - momentum)
            running_var_no_adjust = running_var * momentum + \
    ...
    bools = [False, True]
    ctx = mx.context.current_context()
    cudnn_off_vals = bools if ctx.device_type == 'gpu' else [True,]
    for shape, fix_gamma, cudnn_off, output_mean_var in itertools.product(
            shapes, bools, cudnn_off_vals, bools):
...
#  Test fails on 64-bit values as initially merged
#  ae145cdb1  2020-02-10  hanke580               [Numpy] Add sort op (#17393)
#    dtypes = [np.int8, np.uint8, np.int32, np.int64, np.float32, np.float64]

    dtypes = [np.int8, np.uint8, np.int32, np.float32]
    shapes = [
        (),
        (1,),
        (5,),
    ...
    for hybridize in [True, False]:
        for dtype in dtypes:
            for shape in shapes:
                ...
                if ((s > 1e-5).all() and (L.size == 0 or (L > 1e-1).all())):
