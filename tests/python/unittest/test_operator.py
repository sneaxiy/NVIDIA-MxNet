...
def test_rnnrelu_sym():
    ...
    # If gradients are checked, increase tol to cover the case where
    # the data is at the cusp of relu activation.
...
def check_nearest_upsampling_with_shape(shapes, scale, root_scale):
    ...
    for k in range(len(shapes)):
        name = 'arg_%d'%k
        assert_allclose(arr[name].asnumpy()*root_scale**2*scale**(2*k), arr_grad[name], rtol=1e-4)
...
def test_batchnorm_training():
    def check_batchnorm_training(stype):
        for shape in [(2, 3), (3, 2, 4), (2, 3, 2, 2), (2, 8, 2, 2)]:
...
def test_convolution_grouping():
    for dim in [1, 2, 3]:
        ...
        for num_group in [1, 2, 4]:
            kernel = (3,) * dim
            xshape = (n, c) + (9,) * dim if dim != 3 else (n, c, 6, 5, 4)
            in_features_per_filter = c // num_group
            num_filter_per_group = num_filter // num_group
            wshape = (num_filter, in_features_per_filter) + kernel
            ...
            y2 = mx.sym.Concat(*[mx.sym.Convolution(data=xslice[i], weight=wslice[i], bias=bslice[i],
                                                    num_filter=num_filter_per_group, kernel=kernel)
                            for i in range(num_group)])

            exe1 = y1.simple_bind(default_context(), x=xshape, w=wshape, b=(num_filter,))
            exe2 = y2.simple_bind(default_context(), x=xshape, w=wshape, b=(num_filter,))
            ...
            if default_context().device_type == "gpu":
                ndim = len(xshape)
                transposed_layouts = {3:'NWC', 4:'NHWC', 5:'NDHWC'}
                # NCW -> NWC axes=(0,2,1); NCHW -> NHWC axes=(0,2,3,1); NCDHW -> NDHWC axes=(0,2,3,4,1);
                axes1 = (0,) + tuple(range(2,ndim)) + (1,)
                # NWC -> NCW axes=(0,2,1); NHWC -> NCHW axes=(0,3,1,2); NDHWC -> NCDHW axes=(0,4,1,2,3);
                axes2 = (0, ndim-1) + tuple(range(1,ndim-1))

                y3 = mx.sym.Convolution(data=mx.sym.transpose(x, axes=axes1),
                                        weight=mx.sym.transpose(w, axes=axes1),
                                        bias=b, num_filter=num_filter, num_group=num_group, kernel=kernel,
                                        layout=transposed_layouts[ndim])
                y3 = mx.sym.transpose(y3, axes=axes2)
                exe3 = y3.simple_bind(default_context(), x=xshape, w=wshape, b=(num_filter,))
                for arr1, arr3 in zip(exe1.arg_arrays, exe3.arg_arrays):
                    arr3[:] = arr1
                exe3.forward(is_train=True)
                exe3.backward(exe3.outputs[0])

                for arr1, arr3 in zip(exe1.outputs + exe1.grad_arrays,
                                      exe3.outputs + exe3.grad_arrays):
                    assert_almost_equal(arr1, arr3)


@with_seed()
def test_deconvolution_grouping():
    supported_dims = [1, 2, 3] if default_context().device_type == "gpu" else [1, 2]
    for dim in supported_dims:
        ...
        for num_group in [1, 2, 4]:
            ...
            w = mx.sym.Variable('w')
            b = mx.sym.Variable('b')
            y1 = mx.sym.Deconvolution(data=x, weight=w, bias=b, num_filter=num_filter, num_group=num_group,
                                      kernel=kernel, no_bias=False)
            xslice = mx.sym.SliceChannel(data=x, num_outputs=num_group, axis=1)
            wslice = mx.sym.SliceChannel(data=w, num_outputs=num_group, axis=0)
            bslice = mx.sym.SliceChannel(data=b, num_outputs=num_group, axis=0)
            y2 = mx.sym.Concat(*[mx.sym.Deconvolution(data=xslice[i], weight=wslice[i], bias=bslice[i],
                                                      num_filter=num_filter_per_group, kernel=kernel, no_bias=False)
                            for i in range(num_group)])
            ...
            exe2 = y2.simple_bind(default_context(), x=xshape, w=wshape, b=(num_filter,))
            for arr1, arr2 in zip(exe1.arg_arrays, exe2.arg_arrays):
                scale = 0.5 if dim == 3 else 1.0
                arr1[:] = np.float32(np.random.uniform(-1.0, 1.0, size=arr1.shape)) * scale
                arr2[:] = arr1
            exe1.forward(is_train=True)
            exe1.backward(exe1.outputs[0])
            exe2.forward(is_train=True)
            ...
            if default_context().device_type == "gpu":
                ...
                y3 = mx.sym.Deconvolution(data=mx.sym.transpose(x, axes=axes1),
                                          weight=mx.sym.transpose(w, axes=axes1),
                                          bias=b, num_filter=num_filter, num_group=num_group, kernel=kernel,
                                          layout=transposed_layouts[ndim], no_bias=False)
                y3 = mx.sym.transpose(y3, axes=axes2)
                exe3 = y3.simple_bind(default_context(), x=xshape, w=wshape, b=(num_filter,))
                for arr1, arr3 in zip(exe1.arg_arrays, exe3.arg_arrays):
                    arr3[:] = arr1
                    ...
                    for pad_x in [0, 1]:
                        for in_size in [7, 32]:
                            ...
                            for arr1, arr2 in zip(exe1.outputs, exe2.outputs):
                                assert_allclose(arr1, arr2, rtol=0.01, atol=0.4)
                            ...
                            # dx
                            assert_allclose(exe1.grad_arrays[0],
                                            exe2.grad_arrays[0], rtol=0.02, atol=0.8)
                            # dw
                            assert_allclose(exe1.grad_arrays[1],
                                            exe2.grad_arrays[1], rtol=0.02, atol=0.8)
                            # db
                            assert_allclose(exe1.grad_arrays[2],
                                            exe2.grad_arrays[2], rtol=0.001, atol=0.001)


@with_seed()
def test_convolution_independent_gradients():
    ctx = default_context()
    (rtol, atol) = (1.0e-3, 2.0e-3) if ctx.device_type == "gpu" else (1.0e-3, 1.0e-3)
    ...
    for dim, num_base, kernel_x, stride_x, pad_x , in_size, no_bias in \
        ...
        base_args = {
            'x': mx.nd.random.uniform(-1.0, 1.0, shape=x_shape, ctx=ctx),
            'w': mx.nd.random.uniform(-1.0, 1.0, shape=w_shape, ctx=ctx),
            'b': mx.nd.random.uniform(-1.0, 1.0, shape=(num_filter, ), ctx=ctx) if not no_bias else None}
        for req_kind in reqs:
            ...
            exe1.forward(is_train=True)
            exe1.backward(exe1.outputs[0])
            if exe1_output0 is None:
                exe1_output0 = exe1.outputs[0]
            else:
                assert_allclose(exe1_output0, exe1.outputs[0], rtol=rtol, atol=atol)
            exe1_grads[req_kind] = grad1
            # Verify that the exe1 grads with null grad_req remain identically zero
            if req_kind == "null":
                for var_name in var_names:
                    if var_name == "b" and no_bias:
                        continue
                    assert_allclose(mx.nd.zeros_like(grad1[var_name]),
                                    grad1[var_name], rtol=0.0, atol=0.0)

        for x_req, w_req, b_req in itertools.product(reqs, repeat=3):
            # No need to check an exe2 against an identical exe1 (which has all its reqs equal)
            if x_req == w_req and x_req == b_req:
                continue
            ...
            exe2.forward(is_train=True)
            assert_allclose(exe1_output0, exe2.outputs[0], rtol=rtol, atol=atol)

            exe2.backward(exe2.outputs[0])
            for var_name in var_names:
                if var_name == "b" and no_bias:
                    continue
                # Make sure operation hasn't altered its inputs
                assert_allclose(base_args[var_name], args2[var_name], rtol=rtol, atol=atol)
                # An exe2 grad with a null grad_req must match the exe1 grad zero array exactly
                assert_almost_equal(exe1_grads[grad_req2[var_name]][var_name], grad2[var_name],
                                    rtol=0.0 if grad_req2[var_name] == "null" else None,
                                    atol=0.0 if grad_req2[var_name] == "null" else None)
...
def test_reshape():
    ...
    assert_allclose(exe.grad_arrays[0], out_grad_npy.reshape((5, 4, 3, 7)))
...
def test_transpose():
    for ndim in range(1, 7):
        for t in range(5):
            ...
            assert_allclose(np.transpose(x.asnumpy(), axes=axes), y)

            y = mx.nd.transpose(x)
            assert_allclose(np.transpose(x.asnumpy()), y)
...
def test_crop():
    for ndim in range(1, 6):
        for t in range(5):
            ...
            idx = tuple(idx)
            x = mx.nd.array(np.random.normal(size=dims))
            y = mx.nd.crop(x, begin=begin, end=end)
            assert_allclose(x[idx], y)

            vx = mx.sym.Variable('x')
            vy = mx.sym.crop(vx, begin=begin, end=end)
            check_numeric_gradient(vy, [x])


def get_grad(x, idx):
    xx = np.zeros(shape=x.shape, dtype=x.dtype)
...
def test_slice_axis():
    for ndim in range(1, 6):
        ...
        for t in range(ndim):
            ...
            assert_allclose(x[idx], y)
            exec1.backward([y])
            assert_allclose(get_grad(x, idx), xgrad)
            ...
            assert_allclose(get_grad(x, idx) + x_grad_npy, xgrad, atol=1E-5)
...
def test_slice_like():
    for ndim in range(1, 6):
        ...
        for t in range(ndim):
            ...
            exec1.backward([y])
            assert_allclose(get_grad(x, idx), xgrad)
            assert_allclose(xgrad1, mx.nd.zeros_like(xgrad1))
        ...
        for c in [1, 2, 3, 4]:
            for h in [5, 9, 13, 17]:  # for convenience test, this third and forth input dim should be 4x + 1
                for w in [5, 9, 13, 17]:
                    ...
                    out = exe.outputs[0]
                    # check forward
                    assert_almost_equal(out, args['data'][:, :, h//4:h-h//4, w//4:w-w//4], rtol=1e-2, atol=1e-4)
                    out_grad = mx.nd.ones(out.shape, ctx=dev)
                    exe.backward([out_grad])
                    # check backward
                    assert_almost_equal(out_grad, grad_grad[0][:, :, h//4:h-h//4, w//4:w-w//4], rtol=1e-2, atol=1e-4)
...
def test_batch_dot():
    ...
    if ctx.device_type == 'gpu':
        ...
        if mx.context.gpu_sm_arch(ctx.device_id) >= 50:
...
def check_instance_norm_v2_with_shape(shape, xpu):
    # bind with label
    eps = 0.001
    X = mx.symbol.Variable('X')
    G = mx.symbol.Variable('G')
    ...
    Y = mx.symbol.InstanceNormV2(data=X, beta=B, gamma=G, eps=eps)
    x = mx.random.normal(0, 1, shape, ctx=mx.cpu()).copyto(xpu)
    gamma = mx.random.normal(0, 1, shape[1], ctx=mx.cpu()).copyto(xpu)
    beta = mx.random.normal(0, 1, shape[1], ctx=mx.cpu()).copyto(xpu)
...
@with_seed()
def test_instance_normalization_v2():
    check_instance_norm_v2_with_shape((1, 1, 1), default_context())
    check_instance_norm_v2_with_shape((2, 1, 1), default_context())
    check_instance_norm_v2_with_shape((2, 1, 2), default_context())
    check_instance_norm_v2_with_shape((2,4,5,6), default_context())
    check_instance_norm_v2_with_shape((3,3,2,3,2,1,1), default_context())
                        ...
                        for out_dtype in ['float32', 'float64']:
                            ...
                            if i < in_data_dim-1:
                                ...
                                check_symbolic_forward(norm_sym, [in_data], [npy_out.astype(dtype)],
                                                       rtol=1e-2 if dtype == np.float16 else 1e-3,
                                                       atol=1e-4 if dtype == np.float16 else 1e-5, ctx=ctx)
                                if dtype is not np.float16 and not skip_backward:
...
def test_init():
    ...
    def test_arange_like():
        ...
        for sh in shape_list:
            for axis in axis_list:
                ...
                assert_almost_equal(nd_out, np_out)
    ...
    def test_arange_like_without_axis():
        ...
        for sh in shape_list:
            ...
            assert_almost_equal(nd_out, np_out.reshape(sh))
...
def test_grid_generator():
    ...
    for target_shape in test_case:
        ...
        assert_almost_equal(output[0,0], yv.T, rtol=1e-3, atol=1e-3)
        assert_almost_equal(output[0,1], xv.T, rtol=1e-3, atol=1e-3)
...
def test_quantization_op():
    ...
    assert_almost_equal(a_,  a_real, rtol=1e-2)
...
@with_seed()
def test_gemm():
    _gemm_test_helper(np.float64, True, atol_bw=2e-7)
...
# Adding back seed that failed on TU104 https://gitlab-master.nvidia.com/dl/dgx/mxnet/-/jobs/14427342
@with_seed(398291261)
def test_laop():
    dtype = np.float64
    rtol_fw = 1e-7
...
def test_softmax_output_normalization():
    def _softmaxoutput_normalization(multi_output, use_ignore, normalization):
        ...
        assert_almost_equal(out, softmax_data)
        ...
        assert_almost_equal(data.grad, data_grad)
...
@unittest.skip("This could interfere with some other tests, which should NOT be launched with AMP, "
               "because as of 03/31/2020 AMP cannot be deactivated.")
@with_seed()
def test_softmax_AMP():
    from scipy.special import softmax
    from mxnet.contrib import amp

    performance_test = 0   # Change it, to run performance test
    if performance_test:
        repeat = 100
        warming_percent = 2
        ctxList = [mx.cpu(), mx.gpu()]
    else:
        repeat = 1
        warming_percent = 0
        ctxList = [default_context()]
    ...
    def simple_test():
        ...
        for dtype in ["float32", "float16"]:
            for dshape in [(m,)]:
                np_x = np.ones(dshape, dtype=dtype)
                mx_x = mx.nd.array(np_x, ctx=ctx)

                for axis in range(len(dshape)):
                    np_softmax = softmax(np_x, axis=axis)
                    mx_softmax = mx.nd.softmax(mx_x, axis=axis)
                    assert_almost_equal(np_softmax.astype('float32'), mx_softmax, rtol=1e-03, atol=1e-4)
        return

    def usingMKLDNNSoftmax(usingGPU, inputType, outputType, dimShape):
        return not usingGPU and inputType == "float32" and outputType == inputType and dimShape >= 1 and dimShape <= 4

    def perf_test_param(usingGPU, inputType, dimShape, repeat=100, warming_percent=2):
        if usingGPU or inputType=="float16" and dimShape == 2:
            N = 50
        else:
            N = 100 if dimShape == 1 else (10 if dimShape < 5 else 1)

        N = N * repeat
        first = N * warming_percent / 100
        return N, first, N - first


    def run_softmaxTest(inputType = "float32", ctxList = None, usingAMP=False, repeat=1, warming_percent=2):
        if usingAMP:
            amp.init()
        outType = "float32" if "softmax" in amp.lists.symbol.FP32_FUNCS else "float16"

        if inputType=="float32":
            (s, p) = (m, k)
            (rtol, atol) = (1e-03, 1e-4)
        else:
            # For float16 sizes should be smaller, because numpy has problems calculating softmax
            (s, p) = (int(m/10), int(k/100))
            (rtol, atol) = (1e-03, 1e-3)
        ...
        for ctx in ctxList:
            usingGPU = ctx == mx.gpu()
            context = "GPU" if usingGPU else "CPU"
            print("Context: {}: Types: {} ==> {}   Using AMP: {}".format(context, inputType, outType, "yes" if usingAMP else "no"))
            if usingAMP and not usingGPU:
                continue

            for dshape in [(s,), (s, p), (p, s), (l, l, l, l, l)]:
                np_x = np.random.uniform(size=dshape).astype(inputType)
                mx_x = mx.nd.array(np_x, ctx=ctx)
                dimShape = len(dshape)
                mklDNNsoftmax = usingMKLDNNSoftmax(usingGPU, inputType, outType, dimShape)
                if repeat > 1:
                    N, iFirst, iLast = perf_test_param(usingGPU, inputType, dimShape, repeat=repeat)
                else:
                    N = 0

                for axis in range(dimShape):
                    np_softmax = softmax(np_x, axis=axis)
                    mx_softmax = mx.nd.softmax(mx_x, axis=axis)
                    assert_almost_equal(np_softmax.astype('float32'), mx_softmax.astype('float32'), rtol=rtol, atol=atol)
                    if N == 0:
                        continue

                    for i in range(N):
                        b = mx.nd.softmax(mx_x)
                        b.wait_to_read()
                        if i == iFirst:
                            begin = time.time()
                        if i == iLast:
                    ...
                    str = "%6d launches:  %6.3f sec. " % (N, (end - begin))
                    comment = " ***mklDNNsoftmax" if axis == dimShape - 1 and mklDNNsoftmax else ""
                    print(str + " for axis {} of shape = {} {}".format(axis, dshape, comment))

    simple_test()

    for usingAMP in [False, True]:
        for inputType in ["float16", "float32"]:
            run_softmaxTest(inputType=inputType, ctxList=ctxList, usingAMP=usingAMP, repeat=repeat, warming_percent=warming_percent)
...
def test_assert_allclose():
    numbList = [False, True, 0, 1, 56]
    funcList = [np.int, np.int16, np.int32, np.int64, np.float16, np.float32, np.float64]
    # Run all tests with no exceptions expected
    for numb in numbList:
        # Pairwise comparison of the numbers from numbList represented in different formats
        for atype in funcList:
            a = atype(numb)
            for btype in funcList:
                assert_allclose(a, btype(numb))

        # Strings and bool comparison
        for atype in [np.bool, np.str]:
            a = atype(numb)
            assert_allclose(a, a)

    # Run all tests with expected exceptions:
    for numb in numbList:
        # Pairwise comparison of the numbers from numbList represented in different formats
        for atype in funcList:
            a = atype(numb)
            for btype in funcList:
                try:
                    assert_allclose(a, btype(numb + 1))
                    assert False
                except: pass

    for val in [True, False]:
        try:
            assert_allclose(np.bool(val), np.bool(not val))
            assert False
        except: pass
...
def check_name(op_sym, expected_names, monitor_all=True):
    output_names = []
    ...
    op_exe.set_monitor_callback(get_output_names_callback, monitor_all=monitor_all)
    try:
        op_exe.forward()
        mx.nd.waitall()
    ...
    for output_name, expected_name in zip(output_names, expected_names):
        ...
        if ret:
            ret = False
            print("TEST FAILED: for {}  monitor_all = {}".format(op_sym, monitor_all))
            print("   %20s   %20s" % ("OUTPUT_NAME:", "EXPECTED_NAME:"))
        print("   %20s   %20s" % (output_name, expected_name))
    return ret
...
def test_op_output_names_monitor():
    ...
    if (is_windows):
        ...
        res = True
        res &= check_name(conv_sym, ['conv_output'], False)

        deconv_sym = mx.sym.Deconvolution(data, kernel=(2, 2), num_filter=1, name='deconv')
        res &= check_name(deconv_sym, ['deconv_output'], False)

        fc_sym = mx.sym.FullyConnected(data, num_hidden=10, name='fc')
        res &= check_name(fc_sym, ['fc_output'], False)

        lrn_sym = mx.sym.LRN(data, nsize=1, name='lrn')
        res &= check_name(lrn_sym, ['lrn_output', 'lrn_tmp_norm'], False)

        act_sym = mx.sym.Activation(data, act_type='relu', name='act')
        res &= check_name(act_sym, ['act_output'], False)

        cc_sym = mx.sym.concat(data, data, dim=0, name='concat')
        res &= check_name(cc_sym, ['concat_output'], False)

        sm_sym = mx.sym.softmax(data, name='softmax')
        res &= check_name(sm_sym, ['softmax_output'], False)

        sa_sym = mx.sym.SoftmaxActivation(data, name='softmax')
        res &= check_name(sa_sym, ['softmax_output'], False)

        us_sym = mx.sym.UpSampling(data, scale=2, sample_type='nearest', name='upsampling')
        res &= check_name(us_sym, ['upsampling_output'], False)

        us_sym = mx.sym.Pooling(data, kernel=(2, 2), pool_type='avg', name='pooling')
        res &= check_name(us_sym, ['pooling_output'], False)
        assert(res)
...
def test_op_all_names_monitor():
    ...
    if (is_windows):
        ...
        data = mx.sym.Variable('data', shape=(10, 3, 10, 10))
        conv_sym = mx.sym.Convolution(data, kernel=(2, 2), num_filter=1, name='conv')
        res &= check_name(conv_sym, ['data', 'conv_data', 'conv_weight', 'conv_weight', 'conv_bias', 'conv_bias', 'conv_output'])

        deconv_sym = mx.sym.Deconvolution(data, kernel=(2, 2), num_filter=1, name='deconv')
        res &= check_name(deconv_sym, ['data', 'deconv_data', 'deconv_weight', 'deconv_weight', 'deconv_output'])

        fc_sym = mx.sym.FullyConnected(data, num_hidden=10, name='fc')
        res &= check_name(fc_sym, ['data', 'fc_data', 'fc_weight', 'fc_weight', 'fc_bias', 'fc_bias', 'fc_output'])

        lrn_sym = mx.sym.LRN(data, nsize=1, name='lrn')
        res &= check_name(lrn_sym, ['data', 'lrn_data', 'lrn_output', 'lrn_tmp_norm'])

        act_sym = mx.sym.Activation(data, act_type='relu', name='act')
        res &= check_name(act_sym, ['data', 'act_input0', 'act_output'])

        cc_sym = mx.sym.concat(data, data, dim=0, name='concat')
        res &= check_name(cc_sym, ['data', 'concat_arg0', 'data', 'concat_arg1', 'concat_output'])

        sm_sym = mx.sym.softmax(data, name='softmax')
        res &= check_name(sm_sym, ['data', 'softmax_data', 'softmax_output'])

        length = mx.sym.Variable("length", shape=(10, 10, 10))
        sm_sym = mx.sym.softmax(data, length, axis=1, use_length=True, name='softmax')
        res &= check_name(sm_sym, ['data', 'softmax_data', 'length', 'softmax_length', 'softmax_output'])

        sa_sym = mx.sym.SoftmaxActivation(data, name='softmax')
        res &= check_name(sa_sym, ['data', 'softmax_input0', 'softmax_output'])

        us_sym = mx.sym.UpSampling(data, scale=2, sample_type='nearest', name='upsampling')
        res &= check_name(us_sym, ['data', 'upsampling_arg0', 'upsampling_output'])

        us_sym = mx.sym.Pooling(data, kernel=(2, 2), pool_type='avg', name='pooling')
        res &= check_name(us_sym, ['data', 'pooling_data', 'pooling_output'])
        assert(res)
        del os.environ['MXNET_SUBGRAPH_BACKEND']
