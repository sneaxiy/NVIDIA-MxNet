...
def test_instancenorm():
    ...
    class Net(gluon.HybridBlock):
        def __init__(self,
            ...
            super(Net, self).__init__(**kwargs)
            with self.name_scope():
                self.instnorm = gluon.nn.InstanceNorm(axis=axis)

        def hybrid_forward(self, F, x):
            out = self.instnorm(x)
            return out

    x = mx.nd.random.uniform(-1.0, 1.0, shape=(2, 10, 10, 10))
...
@with_seed()
def test_instancenorm_v2():
    shape = (2, 12, 10, 12)
    default_axis = 1
    in_channels = shape[default_axis]
    layer = nn.InstanceNormV2(in_channels=in_channels)
    ...
    class Net(gluon.HybridBlock):
        def __init__(self,
            ...
            super(Net, self).__init__(**kwargs)
            with self.name_scope():
                self.instnorm = gluon.nn.InstanceNormV2(axis=axis, act_type=act_type)
    ...
    for dtype in [np.float32, np.float16]:
        x = mx.nd.random.uniform(-1.0, 1.0, shape=shape).astype(dtype)
        for axis in axis_list:
            # np.float16 not supported on cpu
            if dtype == np.float16 and default_context().device_type == 'cpu':
                continue
            print('dtype = {}, axis = {}, act_type = None'.format(dtype, axis))
            net = Net(axis)
            check_layer_forward_withinput(net, x)

            # integrated relu support only within a narrow domain
            act_type = 'relu'
            if (dtype == np.float16 and default_context().device_type == 'gpu'
                                    and axis != 1
                                    and shape[axis] % 4 == 0):
                print('dtype = {}, axis = {}, act_type = {}'.format(dtype, axis, act_type))
                net = Net(axis, act_type=act_type)
                check_layer_forward_withinput(net, x)
...
def check_split_data(x, num_slice, batch_axis, **kwargs):
    ...
    if not is_np_array():
        mx.test_utils.assert_almost_equal(mx.nd.concat(*res, dim=batch_axis), x)
    else:
        mx.test_utils.assert_almost_equal(_mx_np.concatenate(res, axis=batch_axis), x)
...
def check_hybrid_static_memory(**kwargs):
    ...
    assert_almost_equal(y1, y2, rtol=1e-3, atol=1e-5)
    for key in grads1:
        assert_almost_equal(grads1[key], grads2[key], rtol=1e-3, atol=1e-4)
