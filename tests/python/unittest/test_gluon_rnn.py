...
from common import setup_module, assert_raises_cudnn_not_satisfied, with_seed


def check_rnn_states(fused_states, stack_states, num_layers, bidirectional=False, is_lstm=True, rtol=1e-4, atol=1e-4):
    ...
    for f, s in zip(fused_states, stack_states):
        ...
        assert_almost_equal(f, s, atol=atol, rtol=rtol)
...
def check_rnn_consistency(fused_layer, stack_layer, loss, input_size, hidden_size, bidirectional=False, rtol=1e-2, atol=1e-4):
    ...
    check_rnn_states(fused_states, stack_states, num_layers, bidirectional, len(fused_begin_state) == 2, rtol=rtol, atol=atol)
...
def check_rnn_unidir_layer_gradients(mode, input_size, hidden_size, num_layers, loss):
    ...
    stack_layer.initialize()
    check_rnn_consistency(fused_layer, stack_layer, loss, input_size, hidden_size, atol=3e-2)
...
def check_rnn_bidir_layer_gradients(mode, input_size, hidden_size, num_layers, loss):
    ...
    with stack_layer.name_scope():
        ...
        stack_layer.initialize()
    check_rnn_consistency(fused_layer, stack_layer, loss, input_size, hidden_size, bidirectional=True, atol=1e-1)
