    ...
    def update(self, index, weight, grad, state):
        ...
        if not use_multi_precision:
            if self.momentum == 0.0:
                ...
                mom = state
                if self.clip_gradient is not None:
                    mom[:] = (self.momentum*mom - wd*weight -
                        mx.nd.clip(grad*self.rescale_grad, -self.clip_gradient, self.clip_gradient))
                    weight += lr * mom
                else:
                    mom[:] = self.momentum*mom - wd*weight - self.rescale_grad*grad
                    weight += lr * mom
        else:
            ...
            if self.momentum == 0.0:
                ...
                if self.clip_gradient is not None:
                    mom[:] = (self.momentum*mom - wd*weight32 -
                        mx.nd.clip(grad32*self.rescale_grad, -self.clip_gradient, self.clip_gradient))
                    weight32 += lr * mom
                else:
                    mom[:] = self.momentum*mom - wd*weight32 - self.rescale_grad*grad32
                    weight32 += lr * mom
        ...
        if self.momentum == 0.0:
            ...
            for row in range(num_rows):
              ...
              if all_zeros:
                  continue
              if self.clip_gradient is not None:
                  mom[row] = (self.momentum*mom[row] - wd*weight[row] -
                      mx.nd.clip(grad[row]*self.rescale_grad, -self.clip_gradient, self.clip_gradient))
                  weight[row] += lr * mom[row]
              else:
                  mom[row] = self.momentum*mom[row] - wd*weight[row] - self.rescale_grad*grad[row]
                  weight[row] += lr * mom[row]
...
def _test_multilamb_helper(dtype):
    opt1 = PyLAMB
    opt2 = mx.optimizer.LAMB
...
# Temporarily break up test_multilamb into 3 subtests to understand
# runtime variability across GPU architectures and dtypes.
@with_seed()
def test_multilamb_float16():
    _test_multilamb_helper(np.float16)

@with_seed()
def test_multilamb_float32():
    _test_multilamb_helper(np.float32)

@with_seed()
def test_multilamb_float64():
    _test_multilamb_helper(np.float64)
