...
def _test_cuda_graphs(seed):
    ctx = default_context()

    def create_exec(size, depth, ctx):
        (n, c, h, w) = (1, 1, size, size)
        for i in range(depth):
            sym = mx.sym.Convolution(data=None if i == 0 else sym,
                                     layout='NCHW', num_filter=10, kernel=(3,3), pad=(1,1),
                                     cudnn_algo_fwd=7, cudnn_algo_bwd_data=0,
                                     cudnn_algo_bwd_filter=0, name='conv{}'.format(i))

        exe = sym.simple_bind(conv0_data=(n, c, h, w), ctx=ctx, grad_req='write')
        return exe

    # Note that CUDA Graph creation is normally triggered on the 2nd iteration
    def exercise_exec(exe, iterations=2):
        for i in range(iterations):
            is_train = i % 2 == 0
            exe.forward(is_train=is_train)
            data = exe.outputs[0].asnumpy()
            exe.backward(exe.outputs[0])
            dgrad = exe.grad_arrays[0].asnumpy()
            wgrad = exe.grad_arrays[1].asnumpy()
        return (data, dgrad, wgrad)

    exe1 = create_exec(100, 20, ctx)
    exercise_exec(exe1, 4)
    exe2 = create_exec(200, 20, ctx)
    exercise_exec(exe2, 4)

    # Captured temp workspaces of exe1 are now invalid, because the temp workspace size
    # grew with exe2.  Running exe1 may still work since the exe1 workspace was not really
    # freed from the gpu driver's perspective- it's in MXNet's memory pool.
    exercise_exec(exe1)

    # Trigger the freeing of the memory pool due to memory exhaustion.  This should
    # invalidate any captured temp space areas that were part of the memory pool.
    (free_mem_bytes, total_mem_bytes) = mx.context.gpu_memory_info(ctx.device_id)
    init_size_MB = 50
    max_size_MB = 1000
    for size_MB in range(init_size_MB, max_size_MB, 2):
        last_free_mem_bytes = free_mem_bytes
        (free_mem_bytes, total_mem_bytes) = mx.context.gpu_memory_info(ctx.device_id)
        # When the free memory area jumps higher, it means the MXNet memory pool was released.
        if (free_mem_bytes > last_free_mem_bytes):
            break
        data_shape = (size_MB * 1024 * 1024 // np.dtype(np.float32).itemsize,)
        # Allocate and free an ndarray to ultimately force MXNet to 'ReleaseAll' its pool.
        x = mx.nd.zeros(data_shape, dtype='float32', ctx=ctx)
        x = None

    # Now that the MXNet-maintained free pool has been released, the captured stale tempspace
    # pointers in exe1 are even more likely to cause a crash.
    exercise_exec(exe1)
    # This should always work since exe2 tempspace pointers are still valid
    exercise_exec(exe2)
...
def test_cuda_graphs():
    for num_threads in ['1', '2']:
        for num_streams in ['1', '2']:
            run_in_spawned_process(_test_cuda_graphs,
                               {'MXNET_GPU_WORKER_NTHREADS' : num_threads,
...
def test_report_sm_archs():
    sys.stderr.write('GPU SM archs = {} ... '.format(
        [mx.context.gpu_sm_arch(i) for i in range(mx.context.num_gpus())] ))
...
def _batchnorm_versions_helper_fp16(batchnorm_op_list, data_shape, fix_gamma, use_global_stats, use_relu):
    ctx_list = []
    sym_list = []
    elements_per_c = np.prod(data_shape) / data_shape[1]
    # Set up an atol of 0.2 for the largest test case (128, 56, 56, c). Otherwise scale.
    atol = 0.2 * elements_per_c / (128 * 56 * 56)
    rtol = 1e-2
    atol_mult = atol / rtol
    # BatchNorm gpu cudnn (if cudnn is enabled)
    if 'batchnorm_cudnn' in batchnorm_op_list:
        ctx_list.append({'ctx': mx.gpu(0), 'batchnorm_data': data_shape,
    ...
    check_consistency(sym_list, ctx_list, use_uniform=True, scale=0.4, rtol=rtol, atol=atol)
...
def test_batchnorm_with_type():
    ...
    if cuda_arch < 60:
        ...
        print('Cudnn-cuda features-last batchnorm test')
        ...
        print('Cudnn-cuda vs. MXNet-cuda features-last batchnorm test on RN50 sizes, et. al.')
        for shape in features_first_shapes:
            features_last_shape = (shape[0],) + shape[2:] + (shape[1],)
            for fix_gamma in [False, True]:
                print('Testing features-last shape {} with fix_gamma {}.'.format(features_last_shape, fix_gamma))
...
def test_batchnorm_relu_fusion_heuristic():
    test_batchnorm_relu_fusion();
...
def test_batchnorm_add_relu_fusion_heuristic():
    test_batchnorm_add_relu_fusion();


@with_seed()
def test_batchnorm_relu_fusion(nchw_shapes=None):
    ctx = mx.gpu(0)
    cuda_arch = mx.context.gpu_sm_arch(ctx.device_id)
    if cuda_arch < 60:
        ...
        if nchw_shapes == None:
            nchw_shapes = default_nchw_shapes
        print('Cudnn-cuda vs. MXNet-cuda NHWC batchnorm+relu test on RN50 sizes')
        for shape in nchw_shapes:
            features_last_shape = (shape[0],) + shape[2:] + (shape[1],)
            for fix_gamma in [False, True]:
                print('Testing BN+relu, features-last shape {} with fix_gamma {}.'.format(features_last_shape, fix_gamma))
                _batchnorm_versions_helper_fp16(batchnorm_op_list=['batchnorm_cudnn',
                                                                   'batchnorm_cudnn_transposed'],
                                                data_shape=shape,
                                                fix_gamma=fix_gamma, use_global_stats=False,
...
def test_batchnorm_add_relu_fusion():
    ...
    if cuda_arch < 60:
        ...
        nchw_shapes = [
            ...
            ( 1,  8,  4,  8),
            ( 4,  8,  4,  4),
            ( 1,  8,  1,  nhw_size(56, 1)), # p100
            ( 1,  8,  1,  nhw_size(60, 2)), # p40
            ( 1,  8,  1,  nhw_size(80, 2)), # v100
            ( 1,  8,  1,  nhw_size(36, 1)), # turing
            ( 1,  8,  1,  nhw_size(46, 1)), # turing
            ( 1,  8,  1,  nhw_size(68, 1)), # turing
            ( 64,  256,  56,  56),
            ( 64,  128,  28,  28),
            ( 64,  512,  28,  28),
            ( 64,  256,  14,  14),
...
# Helper function to append a convolution symbol and associated context to lists that are
# then checked for consistency.  Transposes are inserted to allow comparison of models
# independent of data layout.
def _add_conv_model(sym_list, ctx_list, ctx, dtype, indata_shape, **kwargs):
    data = mx.sym.Variable('conv_data')
    w = mx.sym.Variable('conv_weight')
    b = None if kwargs.get('no_bias', False) else mx.sym.Variable('conv_bias')
    indata_features = indata_shape[1]
    num_filter = kwargs['num_filter']
    weight_shape = (num_filter, indata_features) + kwargs['kernel']
    bias_shape = (num_filter,)
    transposed_layouts = {'NWC', 'NHWC', 'NDHWC'}
    do_transpose = kwargs.get('layout') in transposed_layouts
    ndim = len(indata_shape)
    if do_transpose:
        # NCW -> NWC axes=(0,2,1); NCHW -> NHWC axes=(0,2,3,1); NCDHW -> NDHWC axes=(0,2,3,4,1);
        axes = (0,) + tuple(range(2,ndim)) + (1,)
        data = mx.sym.transpose(data, axes=axes)
        w = mx.sym.transpose(w, axes=axes)
    sym = mx.sym.Convolution(data, w, b, name='conv', **kwargs)
    if do_transpose:
        # NWC -> NCW axes=(0,2,1); NHWC -> NCHW axes=(0,3,1,2); NDHWC -> NCDHW axes=(0,4,1,2,3);
        axes = (0, ndim-1) + tuple(range(1,ndim-1))
        sym = mx.sym.transpose(sym, axes=axes, name='conv')
    sym_list.append(sym)
    ctx_list.append({'ctx': ctx,
                     'conv_data': indata_shape,
                     'conv_weight': weight_shape,
                     'conv_bias': bias_shape,
                     'type_dict': {'conv_data': dtype, 'conv_weight': dtype, 'conv_bias': dtype}})
...
def test_convolution_with_type():
    ...
    test_cases = [((2, 2, 10, 10), (3,3), 3, 'NHWC'),
                  ((2, 3, 9, 11), (4,5), 6, 'NHWC'),
                  ((2, 3, 21), (5,), 3, 'NWC'),
                  ((4, 2, 16, 15, 14), (4,3,3), 5, 'NDHWC'),
                  ((2, 2, 5, 7, 12), (2,3,4), 3, 'NDHWC'), ]
    for (indata_shape, kernel_shape, num_filter, addtl_layout) in test_cases:
        sym_list = []
        ctx_list = []
        # Add gpu models
        for dtype in [np.float64, np.float32, np.float16]:
            _add_conv_model(sym_list, ctx_list, mx.gpu(0), dtype, indata_shape,
                            num_filter=num_filter, kernel=kernel_shape)
        # Add cpu models
        for dtype in [np.float64, np.float32]:
            _add_conv_model(sym_list, ctx_list, mx.cpu(0), dtype, indata_shape,
                            num_filter=num_filter, kernel=kernel_shape)
        # Optionally add gpu models with 'features last' layouts, surrounded by transposes
        if addtl_layout is not None:
            for dtype in [np.float32, np.float16]:
                _add_conv_model(sym_list, ctx_list, mx.gpu(0), dtype, indata_shape,
                                num_filter=num_filter, kernel=kernel_shape, layout=addtl_layout)
        ...
        tol = {np.dtype(np.float16): 0.5,
               ...
               np.dtype(np.uint8): 0,
               np.dtype(np.int32): 0}
        check_consistency(sym_list, ctx_list, rtol=tol, atol=tol)
        # test ability to turn off training on bias
        check_consistency(sym_list, ctx_list, grad_req={'conv_data': 'write',
                                                        'conv_weight': 'write',
                                                        'conv_bias': 'null'}, rtol=tol, atol=tol)
...
def test_convolution_cudnn_flags():
    ...
    # forces algo version = 7, 0, 0
    sym700 = mx.sym.Convolution(num_filter=3, kernel=(3,3), name='conv', cudnn_algo_verbose=1,
                                cudnn_algo_fwd=7, cudnn_algo_bwd_data=0, cudnn_algo_bwd_filter=0)
...
def check_consistency_NxM(sym_list, ctx_list, scale=0.5, rtol=None, atol=None):
    ...
    check_consistency(np.repeat(sym_list, len(ctx_list)), ctx_list * len(sym_list), scale, rtol=rtol, atol=atol)


@with_seed()
def test_convolution_options():

    test_cases = [((2, 7, 7), (3,), 3, 'NWC', {'pad': (1,)}),
                  ((2, 7, 7), (3,), 3, 'NWC', {'stride': (2,)}),
                  ((2, 7, 7), (3,), 3, 'NWC', {'dilate': (2,)}),
                  ((2, 7, 7), (3,), 3, 'NWC', {'no_bias': True}),
                  ((2, 7, 7), (1,), 3, 'NWC', {'pad': (0,)}),
                  ((2, 2, 7, 7), (3,3), 3, 'NHWC', {'pad': (1,1)}),
                  ((2, 2, 7, 7), (3,3), 3, 'NHWC', {'stride': (2,2)}),
                  ((2, 2, 7, 7), (3,3), 3, 'NHWC', {'dilate': (2,2)}),
                  ((2, 2, 7, 7), (3,3), 3, 'NHWC', {'no_bias': True}),
                  ((2, 2, 7, 7), (1,1), 3, 'NHWC', {'pad': (0,0)}),
                  ((2, 2, 5, 7, 7), (2,3,3), 3, 'NDHWC', {'pad': (1,1,1)}),
                  ((2, 2, 5, 7, 7), (2,3,3), 3, 'NDHWC', {'stride': (2,2,2)}),
                  ((2, 2, 5, 7, 7), (2,3,3), 3, 'NDHWC', {'no_bias': True}),
                  ((2, 2, 5, 7, 7), (1,1,1), 3, 'NDHWC', {'pad': (0,0,0)}),  ]

    for (indata_shape, kernel_shape, num_filter, addtl_layout, kwargs) in test_cases:
        sym_list = []
        ctx_list = []
        # Add gpu models, with both settings of cudnn_off
        for dtype in [np.float64, np.float32, np.float16]:
            for cudnn_off in [False, True]:
                _add_conv_model(sym_list, ctx_list, mx.gpu(0), dtype, indata_shape,
                                num_filter=num_filter, kernel=kernel_shape, cudnn_off=cudnn_off, **kwargs)
        # Add cpu models, with both setting of cudnn_off (which should be ignored)
        for dtype in [np.float64, np.float32]:
            for cudnn_off in [False, True]:
                _add_conv_model(sym_list, ctx_list, mx.cpu(0), dtype, indata_shape,
                                num_filter=num_filter, kernel=kernel_shape, **kwargs)
        # Optionally add cudnn gpu models with 'features last' layouts, surrounded by transposes
        if addtl_layout is not None:
            for dtype in [np.float32, np.float16]:
                _add_conv_model(sym_list, ctx_list, mx.gpu(0), dtype, indata_shape,
                                num_filter=num_filter, kernel=kernel_shape, layout=addtl_layout, **kwargs)
        # wider tolerance needed for true-fp16 NCHW test above
        tol = {np.dtype(np.float16): 0.5,
               np.dtype(np.float32): 1e-3,
               np.dtype(np.float64): 1e-5,
...
@unittest.skip("Test fails intermittently. Spawned process fails in mshadow::NewStream<mshadow::gpu>()")
@with_seed()
...
def test_convolution_options_verbose():
    ...
    check_consistency_NxM([sym, sym_no_cudnn], ctx_list, scale=0.1)
    # Stride > 1
...
def test_convolution_versions():
    ...
    check_consistency(syms, ctx_list, scale=0.1)
...
# Helper function to append a deconvolution symbol and associated context to lists that are
# then checked for consistency.  Transposes are inserted to allow comparison of models
# independent of data layout.
def _add_deconv_model(sym_list, ctx_list, ctx, dtype, indata_shape, **kwargs):
    data = mx.sym.Variable('deconv_data')
    w = mx.sym.Variable('deconv_weight')
    b = None if kwargs.get('no_bias', True) else mx.sym.Variable('deconv_bias')
    indata_features = indata_shape[1]
    num_filter = kwargs['num_filter']
    weight_shape = (indata_features, num_filter) + kwargs['kernel']
    bias_shape = (num_filter,)
    transposed_layouts = {'NWC', 'NHWC', 'NDHWC'}
    do_transpose = kwargs.get('layout') in transposed_layouts
    ndim = len(indata_shape)
    if do_transpose:
        ...
        data = mx.sym.transpose(data, axes=axes)
        w = mx.sym.transpose(w, axes=axes)
    sym = mx.sym.Deconvolution(data, w, b, name='deconv', cudnn_algo_verbose=True, **kwargs)
    if do_transpose:
        # NWC -> NCW axes=(0,2,1); NHWC -> NCHW axes=(0,3,1,2); NDHWC -> NCDHW axes=(0,4,1,2,3);
        axes = (0, ndim-1) + tuple(range(1,ndim-1))
        sym = mx.sym.transpose(sym, axes=axes, name='deconv')
    sym_list.append(sym)
    ctx_list.append({'ctx': ctx,
                     'deconv_data': indata_shape,
                     'deconv_weight': weight_shape,
                     'deconv_bias': bias_shape,
                     'type_dict': {'deconv_data': dtype, 'deconv_weight': dtype, 'deconv_bias': dtype}})


@with_seed()
def test_deconvolution_options():

    test_cases = [((2, 2, 7), (3,), 3, 'NWC', {'pad': (1,)}),
                  ((2, 2, 7), (3,), 3, 'NWC', {'stride': (2,)}),
                  ((2, 2, 7), (3,), 3, 'NWC', {'dilate': (2,)}),
                  ((2, 2, 7), (3,), 3, 'NWC', {'no_bias': False}),
                  ((2, 2, 7), (1,), 3, 'NWC', {'pad': (0,)}),
                  ((2, 8, 10, 10), (3,3), 2, 'NHWC', {'pad': (1,1)}),
                  ((2, 8, 10, 10), (3,3), 2, 'NHWC', {'stride': (2,2)}),
                  ((2, 8, 10, 10), (3,3), 2, 'NHWC', {'dilate': (2,2)}),
                  ((2, 8, 10, 10), (3,3), 6, 'NHWC', {'no_bias': False}),
                  ((2, 8, 10, 10), (1,1), 3, 'NHWC', {'pad': (0,0)}),
                  ((2, 2, 5, 7, 7), (2,3,3), 3, 'NDHWC', {'pad': (1,1,1)}),
                  ((2, 2, 5, 7, 7), (2,3,3), 3, 'NDHWC', {'stride': (2,2,2)}),
                  ((2, 2, 5, 7, 7), (2,3,3), 8, 'NDHWC', {'no_bias': False}),
                  ((2, 2, 5, 7, 7), (1,1,1), 3, 'NDHWC', {'pad': (0,0,0)}),  ]
    ...
    for (indata_shape, kernel_shape, num_filter, addtl_layout, kwargs) in test_cases:
        ...
        for dtype in [np.float64, np.float32, np.float16]:
            for cudnn_off in [False, True]:
                # 3D deconv not supported by mxnet's cuda implementation
                if len(kernel_shape) == 3 and cudnn_off:
                    continue
                _add_deconv_model(sym_list, ctx_list, mx.gpu(0), dtype, indata_shape,
                                num_filter=num_filter, kernel=kernel_shape, cudnn_off=cudnn_off, **kwargs)
        # Add cpu models, with both setting of cudnn_off (which should be ignored)
        for dtype in [np.float64, np.float32]:
            for cudnn_off in [False, True]:
                # 3D deconv not supported by mxnet's cpu implementation
                if len(kernel_shape) == 3:
                    continue
                _add_deconv_model(sym_list, ctx_list, mx.cpu(0), dtype, indata_shape,
                                num_filter=num_filter, kernel=kernel_shape, **kwargs)
        # Optionally add cudnn gpu models with 'features last' layouts, surrounded by transposes
        if addtl_layout is not None:
            for dtype in [np.float32, np.float16]:
...
def test_deconvolution_with_type():
    ...
    test_cases = [((2, 2, 7), (3,), 3, 'NWC', {}),
                  ((2, 2, 7), (3,), 3, 'NWC', {'no_bias': False}),
                  ((2, 2, 10, 10), (3,3), 6, 'NHWC', {}),
                  ((2, 2, 10, 10), (3,3), 6, 'NHWC', {'no_bias': False}),
                  ((2, 2, 5, 7, 7), (2,3,3), 3, 'NDHWC', {}),
                  ((2, 2, 5, 7, 7), (2,3,3), 8, 'NDHWC', {'no_bias': False})]
        ...
        if addtl_layout is not None:
            for dtype in [np.float32, np.float16]:
                _add_deconv_model(sym_list, ctx_list, mx.gpu(0), dtype, indata_shape,
                                num_filter=num_filter, kernel=kernel_shape, layout=addtl_layout, **kwargs)
        check_consistency(sym_list, ctx_list)
        check_consistency(sym_list, ctx_list, grad_req="add")


# Helper function to append a deconvolution symbol and associated context to lists that are
# then checked for consistency.  Transposes are inserted to allow comparison of models
# independent of data layout.
def _add_instancenorm_model(sym_list, ctx_list, S, ctx, dtype, indata_shape, axis, act_type):
    data = mx.sym.Variable('norm_data')
    g = mx.sym.Variable('norm_gamma')
    b = mx.sym.Variable('norm_beta')
    gb_shape = (indata_shape[axis],)
    ndim = len(indata_shape)
    # The mx.sym.InstanceNorm impl assumes axis == 1
    do_transpose = S == mx.sym.InstanceNorm and not axis == 1
    if do_transpose:
        # NWC -> NCW axes=(0,2,1); NHWC -> NCHW axes=(0,3,1,2); NDHWC -> NCDHW axes=(0,4,1,2,3);
        axes = tuple(range(0,axis)) + tuple(range(axis+1,ndim))
        axes = (axes[0], axis) + axes[1:]
        data = mx.sym.transpose(data, axes=axes, name='norm')
    if S == mx.sym.InstanceNorm:
        sym = S(data, g, b, name='norm')
        if act_type == 'relu':
            sym = mx.sym.Activation(data=sym, act_type='relu', name='norm')
    else:
        sym = S(data, g, b, axis=axis, act_type=act_type, name='norm')
    if do_transpose:
        # NCW -> NWC axes=(0,2,1); NCHW -> NHWC axes=(0,2,3,1); NCDHW -> NDHWC axes=(0,2,3,4,1);
        axes = (0,) + tuple(range(2,ndim))
        axes = axes[0:axis] + (1,) + axes[axis:ndim]
        sym = mx.sym.transpose(sym, axes=axes, name='norm')
    sym_list.append(sym)
    param_type = np.float32 if dtype is np.float16 else dtype
    ctx_list.append({'ctx': ctx,
                     'norm_data': indata_shape,
                     'norm_gamma': gb_shape,
                     'norm_beta': gb_shape,
                     'type_dict': {'norm_data': dtype, 'norm_gamma': param_type, 'norm_beta': param_type}})

@with_seed()
def test_instancenorm_options():
    ...
    test_cases = [(1, 2, 3),
                  ...
                  (2, 2, 5, 7, 7),
                  (3, 2, 5, 7, 8),
                  # 3D UNET Test Cases, DHW product reduced to permit std tols
                  (2, 4, 4, 4, 32),
                  (2, 4, 4, 4, 64),
                  (2, 4, 4, 4, 128),
                  (2, 4, 4, 4, 256),
    ...
    # override too-small 1e-20 atol for pure float64 test
    atol = {np.dtype(np.float64): 1e-6, }

    for indata_shape in test_cases:
        print('Testing of IN shape {}'.format(indata_shape))
        for axis in [1, len(indata_shape)-1]:
            for dtype in [np.float64, np.float32, np.float16]:
                for act_type in [None, 'relu']:
                    sym_list = []
                    ctx_list = []
                    for ctx in [mx.cpu(0), mx.gpu(0),]:
                        _add_instancenorm_model(sym_list, ctx_list, mx.sym.InstanceNorm, ctx,
                                                dtype, indata_shape, axis, act_type)
                        # InstanceNormV2 not supported on cpu with dtype == np.float16
                        if dtype == np.float16 and ctx == mx.cpu(0):
                            continue
                        # InstanceNormV2 'relu' only supported with fp16 on gpu with axis = -1
                        if act_type == 'relu' and not (dtype == np.float16 and ctx == mx.gpu(0) and
                                                       axis == len(indata_shape)-1 and indata_shape[axis] % 8 == 0):
                            continue
                        _add_instancenorm_model(sym_list, ctx_list, mx.sym.InstanceNormV2, ctx,
                                                dtype, indata_shape, axis, act_type)
                    try:
                        check_consistency(sym_list, ctx_list, atol=atol, use_uniform=True, scale=0.1)
                    except:
                        print('Testing of InstanceNorm fails: shape {} with axis {}, dtype {}, and act_type {}.'.format(
                            indata_shape, axis, np.dtype(dtype).name, act_type))
                        raise
...
def test_grid_generator_with_type():
    ...
    ctx_list = [{'ctx': mx.cpu(0), 'data': (3, 2, 20, 20), 'type_dict': {'data': np.float32}},
                {'ctx': mx.gpu(0), 'data': (3, 2, 20, 20), 'type_dict': {'data': np.float32}}]
    check_consistency(sym, ctx_list)
    check_consistency(sym, ctx_list, grad_req="add")
...
def test_fullyconnected_options():
    ...
    ctx_list = [{'ctx': mx.gpu(0), 'inner_data': (2, 10), 'type_dict': {'inner_data': np.float64}},
                ...
                {'ctx': mx.cpu(0), 'inner_data': (2, 10), 'type_dict': {'inner_data': np.float64}},
                {'ctx': mx.cpu(0), 'inner_data': (2, 10), 'type_dict': {'inner_data': np.float32}}]
    check_consistency(sym3, ctx_list, scale=0.1)
...
@with_seed()
@unittest.skip("Incompatible with CUDA 11.1+ if run in enhanced compatibility mode.")
def test_cuda_rtc():
    source = r'''
...
@with_seed()
def test_batchnorm_relu_heuristic_explicitly():
    for p, c in [(32,256),(16,128),(8,64),(4,32)]:
        for r in range(1,6):
            for o in range (0,1):
                with environment({
                    'MXNET_CUDNN_NHWC_BN_HEURISTIC_BWD':'0',
                    'MXNET_CUDNN_NHWC_BN_HEURISTIC_FWD':'0',
                    'MXNET_CUDNN_NHWC_BN_THREADS_PER_PIXEL_FWD':str(p),
                    'MXNET_CUDNN_NHWC_BN_THREADS_PER_PIXEL_BWD':str(p),
                    'MXNET_CUDNN_NHWC_BN_C_ELEMENTS_PER_CTA_FWD':str(c),
                    'MXNET_CUDNN_NHWC_BN_C_ELEMENTS_PER_CTA_BWD':str(c),
                    'MXNET_CUDNN_NHWC_BN_PIXELS_PER_THREAD_IN_REGISTERS_FWD':str(r),
                    'MXNET_CUDNN_NHWC_BN_PIXELS_PER_THREAD_IN_REGISTERS_BWD':str(r),
                    'MXNET_CUDNN_NHWC_BN_MAX_OCCUPANCY_FWD':str(o),
                    'MXNET_CUDNN_NHWC_BN_MAX_OCCUPANCY_BWD':str(o),
                    }):
                    test_batchnorm_relu_fusion([
                        (64, 512, 7, 7),
                        (64, 256, 7, 7),
