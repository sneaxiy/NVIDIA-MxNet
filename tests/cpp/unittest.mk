...
build/tests/cpp/%.o : tests/cpp/%.cc $(CPP_PACKAGE_OP_H_FILE) | mkldnn
	@mkdir -p $(@D)
...
build/tests/cpp/operator/%.o : tests/cpp/operator/%.cc $(CPP_PACKAGE_OP_H_FILE) | mkldnn
	@mkdir -p $(@D)
...
build/tests/cpp/storage/%.o : tests/cpp/storage/%.cc $(CPP_PACKAGE_OP_H_FILE) | mkldnn
	@mkdir -p $(@D)
...
build/tests/cpp/engine/%.o : tests/cpp/engine/%.cc $(CPP_PACKAGE_OP_H_FILE) | mkldnn
	@mkdir -p $(@D)
...
build/tests/cpp/thread_safety/%.o : tests/cpp/thread_safety/%.cc $(CPP_PACKAGE_OP_H_FILE) | mkldnn
	@mkdir -p $(@D)
...
# We want $(TEST) to find libmxnet.so via LD_LIBRARY_PATH, so leave it off the link command
$(TEST): $(TEST_OBJ) lib/libmxnet.so $(TEST_LIB_DEP)
	$(CXX) -std=c++11 $(TEST_CFLAGS) -I$(GTEST_INC) -o $@ $(TEST_OBJ) $(TEST_LIB_DEP) $(TEST_LDFLAGS)
