...
# CFLAGS for debug
ifeq ($(DEBUG), 1)
	CFLAGS += -g -O0 -D_GLIBCXX_ASSERTIONS -mcmodel=medium
...
ifeq ($(USE_CUDA), 1)
ifeq ($(CUDA_ARCH),)
	KNOWN_CUDA_ARCHS := 50 52 60 61 70 75 80
...
all: lib/libmxnet.so $(BIN) extra-packages extension_libs
...
EXT_LIBS = lib/libcustomop_lib.so lib/libtransposecsr_lib.so lib/libtransposerowsp_lib.so lib/libsubgraph_lib.so lib/libpass_lib.so
ifeq ($(USE_CUDA), 1)
        EXT_LIBS += lib/libcustomop_gpu_lib.so
endif
...
# Link libs into a separate directory so they can be selectively copied to a container by Dockerfile.base
	@mkdir -p $(@D)/extensions
	(cd $(@D)/extensions; ln -f -s ../$(@F) .)
